...
- **Breaking:** the vestigial `_backend` and `_into` suffixes are dropped from every api trait and method, the matching OEP methods and the `poulpy-cpu-ref` default bodies: `vec_znx_add_into_backend` is `vec_znx_add`, `VecZnxAddIntoBackend` is `VecZnxAdd`, `vec_znx_rotate_backend_default` is `vec_znx_rotate_default`. Purely lexical; bench ids follow. The rename table is generated from the api and OEP sources by the script in the PR description.
