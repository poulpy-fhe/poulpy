...
- **Breaking:** `GLWEMaskFill::fill_glwe_mask_from_source(res, source)` and `fill_glwe_mask_from_seed(res, seed)` fill the mask columns `1..=rank` of `res` at its own radix and `k`; the `base2k`, `res_col` and `rank` parameters are gone from the api, the `EncryptionImpl` methods, the delegates and `GLWEMaskFillReference`. Every library caller passed `res.base2k()`, `1` and `res.rank()`. A fixture that fills every column of a GLWE draws the body with `vec_znx_fill_uniform_source` and then the mask with this op, from the same stream.
...
- **Breaking:** the `core::fill` module is removed: the core runners sample their operands on the tested backend and require `GLWEMaskFill` and `VecZnxFillUniformSource`, the blind rotation runners also `LWEFillMask`, and the core runners and suites drop the `OwnedBuf: CopyFromHost` and `*AtBackendMut` bounds the host staging needed.
