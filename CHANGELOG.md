...
- **Breaking:** `Backend` requires `PartialEq + Eq` (all marker types are ZSTs deriving them), which makes the containers' and prepared types' equality derives live; byte-buffer-backed prepared containers can derive `Eq` even when their logical DFT word is `f64`, while higher-level prepared wrappers retain only `PartialEq`. `VecZnxBig::stats` is an inherent impl on `VecZnxBig<D, i64, B>`.
