...
- **Breaking:** the DFT/big-domain layouts are re-keyed from backend to word: `VecZnxDft<D, B: Backend>` → `VecZnxDft<D, W: DftWord>`, and likewise `VecZnxBig<D, W: BigWord>`, `SvpPPol<D, W: DftWord>`, `VmpPMat<D, W: DftWord>`, `CnvPVecL/R<D, W: DftWord>`. The `*Owned` / `*BackendRef` / `*BackendMut` aliases keep their backend parameter and project the word (`VecZnxDftOwned<B> = VecZnxDft<B::OwnedBuf, B::DftWord>`), which carries the whole api/oep/delegates op layer unchanged — and backends sharing a representation now share buffer types: `VecZnxDft<_, f64>` is one type across `FFT64Ref`/`Avx`/`Avx512`/`Neon`, `VecZnxDft<_, Q120bScalar>` one type across the NTT4x30 family.
...
- **Breaking:** backend-bound constructors move to method-level generics (the struct no longer names a backend): `VecZnxDft::alloc::<B>(…)` / `from_bytes::<B>(…)`, `VecZnxBig::from_bytes::<B>`, `SvpPPol::alloc::<B>`, `VmpPMat::alloc::<B>`, `CnvPVec*::alloc::<B>` / `from_bytes::<B>`, and the zero-copy DFT→big re-tag is now witnessed by a backend: `VecZnxDft::into_big::<B>()`.
- Since the backend is no longer recoverable from a DFT/big-family value's type, `to_backend_ref()` / `to_backend_mut()` cannot infer it; all six types gain inherent `x.to_backend_ref::<B>()` / `x.to_backend_mut::<B>()` for owned buffers (delegating to the `*ToBackendRef/Mut` traits, which remain the bound vocabulary for generic code; scratch-view wrappers such as `VecZnxDftViewMut<'_, B>` still infer without a turbofish).
- `f64`-word containers are `PartialEq` but not `Eq` (the previous backend-keyed derives were dead impls, as backend markers implement neither), and `VecZnxBig::stats` is now an inherent impl on `VecZnxBig<D, i64>` (was bounded `B: Backend<ScalarBig = i64>`).
...
- The prepared layouts (`GLWEPrepared`, `GGLWEPrepared`, `GGSWPrepared`, `GLWESecretPrepared`, `PreparedDiagonal`, …) follow the HAL re-key transparently — inner buffers are now keyed by `B::DftWord` — and their derived `Eq` is dropped (`PartialEq` retained), since `f64`-word backends cannot satisfy it.
