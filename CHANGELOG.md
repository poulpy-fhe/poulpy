...
- `Diagonals` carries an optional period tag per diagonal: `set_periodic(index, values, period)` stores a diagonal that repeats every `period` slots (checked, `period` a power-of-two divisor of `slots`) and `period(index)` reads it back, `slots` for an untagged diagonal; `set` stores an untagged, dense diagonal; `transpose` moves the tags with their diagonals. A scheme encodes a tagged diagonal on `period` slots, its plaintext polynomial being sparse by the factor `slots / period`.
...
- Homomorphic DFT factors are stored at their true sparsity. The S2C and C2S factors are merged radix-2 butterflies: in the natural slot order a layer's three diagonals repeat over its butterfly width (two diagonals on the widest layer) and a merge keeps the larger period, so every diagonal of a factor repeats over the widest butterfly the factor merges. The generator tags each diagonal with that period as it merges (`ComplexDiagonals::diagonal_period`), `ComplexDiagonals::build_transform` hands the encoder one period of every diagonal (`ComplexDiagonals::period`, the map's largest tag, `slots` untagged, one degree per transformation), and the diagonal encoder allocates the compact plaintext for the slots it receives: on the full-packing presets the four S2C factors are stored at degrees 16, 256, 4096 and N (C2S mirrored) where every factor was N. `compose`, `conjugate` and the new `ComplexDiagonals::scale` keep the tags, so PaCo's blockwise factors carry their tile replication. `DFTPlan::diagonal_log_sparsity` and `DFTPlan::factor_log_sparsity` replay the same rule from the schedule without generating a value, like `diagonal_indexes` (the sparse-repack seed and the half-zeroed last `Encode` factor keep only the packing sparsity, bit-reversed layers count at their full width); the generator's tests pin the replay, the tags and the generated values against each other per diagonal, and the compiled matrices' plaintext degrees per factor.
