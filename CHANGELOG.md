...
- **Breaking:** the host-side `FillUniform` trait is removed, with its `VecZnx`, `ScalarZnx` and `MatZnx` impls and the `poulpy-core` (GLWE, LWE, GGLWE, GGSW, keys, compressed) and `poulpy-bin-fhe` (blind rotation keys) impls; test and bench fixtures sample on the backend through `vec_znx_fill_uniform_source`, the new `vec_znx_fill_uniform_source_all` and the LWE mask sampler `fill_lwe_mask_from_source`.
- `VecZnxFillUniformSourceAll::vec_znx_fill_uniform_source_all(base2k, k, res, source)`, a derived operation, fills every column of a `VecZnx` in increasing order, drawing exactly what `vec_znx_fill_uniform_source` draws column by column; it takes any `VecZnxToBackendMut`, and its default body on `HalVecZnxImpl` is pinned by `test_vec_znx_fill_uniform_source_all_derived` on every backend.
...
- **Breaking:** `GLWEMaskFill::fill_glwe_mask_from_source(res, source)` and `fill_glwe_mask_from_seed(res, seed)` fill the mask columns `1..=rank` of `res` at its own radix and `k`; the `base2k`, `res_col` and `rank` parameters are gone from the api, the `EncryptionImpl` methods, the delegates and `GLWEMaskFillReference`. Every library caller passed `res.base2k()`, `1` and `res.rank()`. A fixture that fills every column of a GLWE uses `vec_znx_fill_uniform_source_all` on its data.
...
- **Breaking:** the `core::fill` module is removed: the core runners sample their operands on the tested backend and require `VecZnxFillUniformSourceAll`, the blind rotation runners also `LWEFillMask`, and the core runners and suites drop the `OwnedBuf: CopyFromHost` and `*AtBackendMut` bounds the host staging needed.
