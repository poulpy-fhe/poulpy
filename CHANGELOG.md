...
- A key with fewer digits than its input binds to the digits it has, matching what the dense kernels already did; `GGLWEActiveUse::covers_input` records the shortfall so policy dispatch refuses such a key while execution does not.
