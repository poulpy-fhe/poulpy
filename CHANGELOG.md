...
- `DFTPlan::diagonal_log_sparsity` and `DFTPlan::factor_log_sparsity` give the sparsity of every factor diagonal of a homomorphic DFT, replayed from the schedule like `diagonal_indexes` without generating a value: in the natural slot order each radix-2 layer's three diagonals repeat over the layer's butterfly width, a merge keeps the larger period, so every diagonal of a factor repeats over the widest butterfly it merges and stores at `2^(log_n - s)`; the sparse-repack seed and the half-zeroed last `Encode` factor keep only the packing sparsity, a bit-reversed plan is reported dense. Pinned against the generated diagonals, exact on the natural order.
