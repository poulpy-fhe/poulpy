...
- Sparse operands ([#266](https://github.com/poulpy-fhe/poulpy/issues/266)): the slots a contract's `sparse` line names accept a degree-`n` operand, `n` a power-of-two divisor of the module degree `N`, which stands for its ring embedding `switch_ring_{n->N}`. The coefficient-domain `add` / `sub` families of `VecZnx` and `VecZnxBig`, including the small-operand Big forms, read such an operand with a stride. `cnv_prepare_left`, `cnv_prepare_right` and `cnv_prepare_self` take a degree-`n` input into a degree-`n` `CnvPVecL` / `CnvPVecR` allocated with `CnvPVecL::alloc(n, ..)`, and `cnv_apply_dft`, `cnv_apply_dft_add`, `cnv_apply_dft_sum` and `cnv_pairwise_apply_dft` take such an operand in either slot or both, at independent degrees. Nothing is expanded: the compact operand is read through the backend's slot correspondence, which on every in-tree transform is `slot_N(i) = slot_n(i >> log2(N/n))`. `cnv_by_const_apply` keeps dense operands (a degree-`n` `a` is embedded by the caller). Pinned by `test_vec_znx_sparse_add_sub`, `test_vec_znx_big_sparse_add_sub` and `test_convolution_sparse` on every backend, each against the dense operation on the `switch_ring` embedding.
...
- Sparse operands (#266) on every CPU family. Coefficient domain: one strided kernel set in `poulpy_cpu_ref::reference::vec_znx` behind the reference add/sub kernels, which every backend uses; the NTT4x30 Big kernels, which zipped operands of different degrees silently, now reject a degree that does not embed. DFT domain: a degree-`n` prepare runs the family's degree-`n` transform (FFT64 and NTT4x30 already held a plan per power of two; the IFMA handle gains a forward table per sparse degree, one per power of two from 8 to N/2 built at module creation, about doubling the forward-table setup), and every `cnv_prepare_self` asserts `right.n() == left.n()` at entry, pinned by `test_convolution_sparse`. The apply kernels gather a sparse operand's block rows into their per-task staging: reim4 blocks of four slots, with `poulpy-cpu-rayon`'s FFT64 wrapper as a fourth `Reim4Convolution` implementor updated alongside the other three; NTT4x30 x2 rows, the reference kernel keeping its fused dense pairwise path and only staging the gathered operand; IFMA groups of eight. The AVX2 and AVX-512 NTT4x30 kernels instead read each slot from a per-operand base. Dense paths keep their instructions, except the NTT4x30 reference sum, whose row pair now goes through the shared `x2_row` accessor (reference and NEON backends only, no bench runner exists for that form). The sparse degree floor is 8 (the block widths); `sparse_log_gap` asserts it once at kernel entry. Scratch: `convolution_apply_dft_tmp_bytes` (FFT64) grows by `8 * b_size` f64, `ntt4x30_cnv_apply_dft_tmp_bytes` by the `a1`, `b`, `b1` staging rows (its pairwise size is now equal to it), the IFMA apply and sum sizes by the gathered rows, the sum's scratch taken as `max(fused, unfused)` through one shared helper.
