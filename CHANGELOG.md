...
- Derived operations now carry a backend-generic default body on their OEP family trait, composed only from other OEP methods of the same backend; the bodies live as `#[doc(hidden)] pub` free functions in `poulpy_hal::oep::derived` (`<op>_derived` / `<op>_tmp_bytes_derived`, generic over the implementing type and the backend) so that `test_suite::derived` can pin them. A backend implements the basis and inherits the rest. **Breaking:** the OEP family traits carry supertraits (`HalVecZnxDftImpl: HalVecZnxBigImpl`; `HalSvpImpl`, `HalVmpImpl`, `HalConvolutionImpl: HalVecZnxDftImpl`; `HalConvolutionImpl: HalVecZnxBigImpl`); the `oep` module docs give the composition graph and the implementation order a new backend follows.
- The derived set: `vmp_apply_dft`; `vec_znx_lsh` / `rsh` and their `_add` / `_sub` / `_assign` forms (each a `vec_znx_normalize` call with an offset); `vec_znx_mul_xp_minus_one` (+ `_assign`); `vec_znx_add_scalar_assign`; `vec_znx_big_add_small`, `vec_znx_big_sub_small_a`, `vec_znx_big_sub_small_b` (scratch-free, signature unchanged); `vec_znx_idft_normalize_consume`; `vec_znx_dft_automorphism_add_with_plan`; `svp_apply_dft`; `vmp_apply_dft_to_dft_add`; `cnv_prepare_self`, `cnv_apply_dft_add`, `cnv_pairwise_apply_dft`, `cnv_by_const_apply_add`. Fused CPU kernels stay as explicit backend overrides, each pairing its body with its own `_tmp_bytes`: on the NTT and Rayon backends that had one, and on every CPU backend for `lsh_assign`, `mul_xp_minus_one_assign` and the three small-operand Big products. `cnv_by_const_apply`, `vec_znx_idft_apply_tmpa`, `svp_apply_dft_to_dft_assign` and the three `vec_znx_big_*_small_assign` forms stay required, with rustdoc explaining why.
- **Breaking:** `vec_znx_lsh_tmp_bytes(res_size)` / `vec_znx_rsh_tmp_bytes(res_size)` take the destination size and size their whole direction family (`_add`/`_sub`/`_assign` share it), so they now report a full `res_size`-limb `VecZnx` plus the normalize carry instead of the single carry buffer the CPU kernels needed; every arena sized through them, including the `poulpy-core` and `poulpy-ckks` helpers below, grows accordingly; `vec_znx_mul_xp_minus_one_assign_tmp_bytes(size)` takes the destination size; `svp_apply_dft` and `vec_znx_dft_automorphism_add_with_plan` take a trailing `scratch` and gain `svp_apply_dft_tmp_bytes(b_size)` / `vec_znx_dft_automorphism_add_with_plan_tmp_bytes(res_size, a_size)`; `Convolution` gains `cnv_apply_dft_add_tmp_bytes` and `cnv_by_const_apply_add_tmp_bytes`, and `cnv_apply_dft_sum_tmp_bytes` defaults to the max of the two steps its default body runs.
- **Breaking, behaviour:** on the CPU backends `lsh`, `rsh` and their accumulating forms lose their dedicated kernels and run through `vec_znx_normalize` with an offset of `±k` (`lsh_assign` keeps a fused kernel that is bit-exact with that composition). A shift whose destination is shorter than its source now rounds the discarded tail jointly (one unit in the last limb can differ from before), and `rsh` by more than the destination precision now yields zero (it previously returned `a >> (k mod base2k)`).
- **Breaking:** `cnv_prepare_left_lazy`, `cnv_prepare_right_lazy`, `cnv_apply_dft_lazy` and their `_tmp_bytes` are removed: they forwarded to the eager forms, which callers now use directly. `vec_znx_dft_add_scaled_assign` and its five kernels go with them, having had no caller anywhere in the workspace. Their OEP methods, delegates and backend overrides go with them.
- **Breaking:** `vec_znx_normalize_assign` takes a leading `a_offset: i64`, matching the `res_offset` its out-of-place twin already had, so an in-place normalization can shift the column at the same time. Only a negative offset is accepted: a positive one would write a limb before reading it. `vec_znx_rsh_assign` is now a single call into it, with no temporary and no copy back, which measured 13.6 microseconds against 20.4 for the previous default body and 24.4 for the fused kernel PR4 deleted, at degree 4096 over eight limbs on FFT64Avx512. The in-place kernel takes the same exactness dispatch as the out-of-place one: the per-limb carry chain only where at most one limb is dropped, the i128 exact path otherwise. Both deleted right-shift kernels used the carry chain unconditionally, which is what put them one unit in the last limb away from the composition.
- `test_suite::derived` gains one parity test per derived operation (default body against an explicit api oracle, and against the backend's override where one exists), registered on every backend; `test_svp_apply_dft` and `test_vmp_apply_dft` are now registered on every backend (they were missing on most); `cnv_prepare_self` gets its first test.
- `poulpy-bench` gains a HAL runner for every derived operation that lacked one: the shift `_add` / `_sub` forms, `vec_znx_add_scalar_assign`, `vec_znx_idft_normalize_consume`, `vmp_apply_dft_to_dft_add`, `cnv_prepare_self` and `cnv_by_const_apply_add`. `NTT3x42Ifma` gains a HAL sweep, having had none.
- **Fix:** the derived `vec_znx_lsh`, `vec_znx_rsh` and `vec_znx_rsh_assign` cast `k` to `i64` unclamped, so a `k` of `2^63` or more wrapped and reversed the shift where the contract says the destination is zeroed. `k` is now clamped before the cast to a width past which the shift zeroes the destination for any source the normalization accepts: the source width for a left shift, the destination width plus 64 bits for a right shift. The api suite runs every shift at `k` equal to the width, one past it and `usize::MAX`, and checks for zero past the width.
- **Fix:** the derived `_tmp_bytes` formulas that add a nested call's scratch to a temporary of their own (`vmp_apply_dft`, `vmp_apply_dft_to_dft_add`, `vec_znx_lsh`, `vec_znx_rsh`, `vec_znx_idft_normalize_consume`, `cnv_apply_dft_add`, `cnv_by_const_apply_add`) did not round the temporary to `SCRATCH_ALIGN`, so at a degree where it is not a multiple of 64 bytes (`n < 8` with 8-byte words) the advertised scratch was short by one alignment step and the nested take panicked. `Backend::scratch_aligned` is added and applied, and `poulpy-cpu-ref` runs the composites at `n = 4` with exactly the advertised scratch.
- Every api trait now carries a structured contract beside its prose: `op`, `class` (basis / variant / derived / support), `mutation` (out-of-place / in-place / accumulate / none), `domain`, `ensures`, `exact` and `test` on every operation, plus `definition`, `requires`, `fallback`, `override` and `sparse` where they say something. The `api` module docs define the vocabulary and add the degree-embedding paragraph; the `sparse` line records, per operation, which operand slots will take a degree other than the module's ([#266](https://github.com/poulpy-fhe/poulpy/issues/266)). A test reads `src/api/*.rs`, holds every `pub trait` to a block, checks that its class brings the lines it needs and resolves every name on the `test` line against `src/test_suite`.
...
- `vec_znx_dft_automorphism` joins the OEP as a derived operation: `HalVecZnxDftImpl::vec_znx_dft_automorphism` carries the default body (build the plan, apply it, drop it) and the api trait forwards to it, so a backend can override it as its contract said. The provided body on the api trait, which no backend could replace, is removed. `test_vec_znx_dft_automorphism_derived` pins the default body and the api entry against the plan form on every backend.
...
- Qualified paths whose trait was the only candidate are written as `Type::method`: `<Self as HalXImpl>::` and `<Base as HalXImpl>::` in the backends, `<Self as XDefault<BE>>::` in `poulpy-core`. The `<Self as $defaults>::` form in the `poulpy-cpu-ref` `hal_impl_*` macros stays, since the FFT64 and NTT4x30 default traits share method names and both apply to one backend.
...
- **Fix:** every unary op stamps the destination's metadata before writing it, and stamps the final metadata: `ckks_copy`, `ckks_neg_into`, `ckks_mul_i_into`, `ckks_mul_pow2_into`, `ckks_div_pow2_into` and the plaintext add/sub into-forms join `ckks_rotate_into` and `ckks_conjugate_into`. `div_pow2` moves its charged bits under `log_delta` inside the stamp instead of after the shift, which would otherwise have the shift round `bits` too low now that it honours `k`. With the `poulpy-core` shift change, every CKKS operation leaves its output canonical at the `k` it reports; the test suite's decryption helpers assert it on every ciphertext they decrypt, so each operation test pins it on every backend, and the `cnv_prepare_*` masks are no-ops on every operand.
