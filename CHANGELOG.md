...
- **Breaking:** remove `max_size` from the layout family. It never diverged from `size` — every allocator passed the same value twice — so it was a stored duplicate that could only go wrong by mistake. `size` is now both the working and the allocated width, fixed at construction, and operating on fewer limbs is exclusively a borrowed view: `VecZnxDftBackendMut::with_size_mut`, plus the new free-standing `vec_znx_backend_mut_with_size` for `VecZnx`. Removed: `VecZnxShape::max_size`, the inherent `max_size()` on all three containers, `from_data_with_max_size`, and `vec_znx_alloc_with_max_size`. The `VecZnx` wire format drops one `u64`, which also removes a latent hazard: `WriterTo` wrote `max_size` but only `n·cols·size` limbs of payload, so a deserialized value could hand out a view over limbs that were never read.
- **Breaking:** split `ZnxInfos` into a family-neutral shape plus two family traits. `ZnxInfos` keeps `n()`, `log_n()`, `size()` and `poly_count()`; `VecZnxInfos` adds `cols()`; `MatZnxInfos` adds `rows()`, `cols_in()` and `cols_out()`. One trait served both families dishonestly: `cols()` returned `cols_in()` on the matrix containers and `rows()` was a hardcoded `1` on the vector ones, so no trait bound could read a prepared key's output rank. `ZnxView` now requires `VecZnxInfos` and the matrix containers expose inherent `raw()`/`raw_mut()` instead. Migration is small: the concrete types keep inherent accessors that shadow the traits, so only fully-qualified `<X as ZnxInfos>::cols/rows` spellings need renaming.
...
- The core layouts become generic over their HAL payload: `GLWECore<P>`, `GGLWECore<P>`, `GGSWCore<P>`, `GLWESecretCore<P>`, `GLWESecretTensorCore<P>`, `GLWEPublicKeyCore<P>`, and the seven key-role wrappers (`GLWESwitchingKeyCore<P>`, `GLWEAutomorphismKeyCore<P>`, `GLWETensorKeyCore<P>`, `GLWEToLWEKeyCore<P>`, `LWESwitchingKeyCore<P>`, `LWEToGLWEKeyCore<P>`, `GGLWEToGGSWKeyCore<P>`). Every coefficient-domain and prepared spelling is now an alias of the same struct, so each `LWEInfos` / `GLWEInfos` / `GGLWEInfos` / `GGSWInfos` impl is written once against the payload instead of twice against two structs. Source-compatible with one exception: a tuple-struct *constructor* cannot be spelled through a type alias, so the four newtype wrappers now construct as `GLWETensorKeyCore(…)` — crate-internal only, since the fields are `pub(crate)`. Two asymmetries are preserved: secrets keep their own `LWEInfos` (`base2k` reported as `0`, panicking `k()`, `rank` the raw column count), and `Eq` stays coefficient-domain only.
- Add the big/accumulator domain to the GLWE family: `GLWEBig<D, B>` with its borrow traits and scratch carves. It is the deferred-normalization intermediate — an inverse DFT lands there with un-carried magnitude. Note the domain contract: `base2k` and `k` on a non-coefficient value describe the accumulator, not a precision claim.
- Split GLWE keyswitching into two composable stages: `GLWEKeyswitchIntoBig::glwe_keyswitch_into_big` performs the mask product, inverse transform and body fold, stopping at an un-normalized big-domain accumulator, and `GLWEFinalizeBig::glwe_finalize_big_into` carries it back to the coefficient domain at the *destination's* precision. `glwe_keyswitch` and `glwe_keyswitch_assign` are now that composition, removing what had been two independent copies of the same algorithm; a conformance test pins the pair bit-identical to the single-shot path. `glwe_mask_dft_apply` and `glwe_keyswitch_from_mask_into_big` expose the hoisted form, which the linear-transformation baby steps now use instead of hand-rolling the same four steps. Both traits route through `api`/`oep`/`delegates` like every other operation family, so a backend can substitute a stage. **Breaking:** they live in `poulpy_core::api`, and the accumulator layout is the free function `poulpy_core::layouts::glwe_keyswitch_big_layout`. `glwe_keyswitch_tmp_bytes` is restated for the new allocation order.
- **Fixed:** the multi-digit gadget product read uninitialized scratch. Its single *overwriting* pass ran through a view narrowed by `dsize - 2` limbs, so for `dsize >= 3` the top limbs of the destination were reachable only by the later *accumulating* passes and summed whatever the arena happened to hold — `take_region` hands back raw bytes. Callers compensated by pre-zeroing, an obligation that was undocumented, invisible at the call site, and not met on the automorphism path. The overwriting pass now runs at full width, so the buffer is self-initializing and the three pre-zeroing loops are gone. New test `glwe_keyswitch_ignores_dirty_scratch` requires bit-identical results over a zeroed and a `0xFF`-poisoned arena.
