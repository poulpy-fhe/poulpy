...
- Every operation that computes also carries a `definition` line, the mathematical statement of its result in the notation the `api` module docs define once (limb and coefficient indexing with zero-extension, the ring reduction of `X^e`, `tau_p`, `switch_ring`, `rnd` and `mod 1` for the normalizations, `idft` for the DFT domain, prepared operands read as the objects their prepare read): a formula on a basis operation, the composition it stands for on a variant or a derived one. The contract test requires it on every class but `support`, which carries none.
