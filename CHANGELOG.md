...
- Add crate-private working-scale adjustment for CKKS internals: `ckks_scale_down_assign` trades `bits` of precision for budget (right-shift, `log_delta -= bits`, `log_budget += bits`) and `ckks_scale_up_assign` does the inverse (left-shift into the budget headroom). Both keep `effective_k` constant and report the new `InsufficientScalePrecision` error when `scale_down` would drop more than `log_delta` bits. `scale_down` is not exposed through `CKKSRescaleOps`; it is only used under the internal flush discipline required by adaptive polynomial evaluation.
