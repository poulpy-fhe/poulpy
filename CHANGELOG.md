...
- Sparse operands ([#266](https://github.com/poulpy-fhe/poulpy/issues/266)): the slots a contract's `sparse` line names accept a degree-`n` operand, `n` a power-of-two divisor of the module degree `N`, which stands for its ring embedding `switch_ring_{n->N}`. The coefficient-domain `add` / `sub` families of `VecZnx` and `VecZnxBig`, including the small-operand Big forms, read such an operand with a stride. In the convolution the prepared right operand is the sparse-capable slot, and the prepares stay dense: a degree-`n` right operand is prepared under a degree-`n` module, exactly like any other prepare, and `cnv_apply_dft`, `cnv_apply_dft_add`, `cnv_apply_dft_sum` and `cnv_pairwise_apply_dft` of a degree-`N` module read it there; the result and the left operand take the module degree, so every prepare rejects an operand whose degree is not the module's. Nothing is expanded: the compact operand is read through the backend's slot correspondence, which on every in-tree transform is `slot_N(i) = slot_n(i >> log2(N/n))`. `cnv_by_const_apply` keeps dense operands (a degree-`n` `a` is embedded by the caller). Pinned by `test_vec_znx_sparse_add_sub`, `test_vec_znx_big_sparse_add_sub` and `test_convolution_sparse` on every backend, each against the dense operation on the `switch_ring` embedding.
