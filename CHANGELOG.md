...
- **Breaking:** the per-row compositions leave the `api`, with their OEP traits, `impl_*_reference_full!` macros, reference bodies and noise tests: `GGLWEKeyswitch`, `GGSWKeyswitch`, `GGLWEExternalProduct`, `GGSWExternalProduct`, `GGSWAutomorphism`, `GGSWRotate` and `GGSWFromGGLWE` (twenty methods). Each reference body was a loop of the GLWE operation over the rows, the GGSW forms followed by `ggsw_expand_row`, which stays; a caller writes that loop. `AutomorphismImpl` and `ConversionImpl` lose the GGSW methods, `test_gglwe_keyswitch_parity` and `log2_std_noise_ggsw_keyswitch` go with the operations. None had a caller outside the crate.
