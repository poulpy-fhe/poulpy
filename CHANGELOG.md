...
- Every kernel picks its transform table by the operand degree and asserts the module serves it (`check_degree` at entry, the operand equalities beside it); the IFMA handle holds forward and inverse tables for every degree from 16 to `N`; the four module-degree table accessors are gone; the suites sweep the floor degree on every backend. Floors: 8 on every backend except `FFT64Avx512` (16, its complex multiply kernels need `m` a multiple of 8) and `NTT3x42Ifma` (16); the rayon twins inherit theirs.
