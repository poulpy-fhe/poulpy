...
This release adds **PaCo** (Coron & Seuré, [ePrint 2025/886](https://eprint.iacr.org/2025/886)) — bootstrapping for CKKS via **Pa**rtial **Co**effToSlot — as a native, backend-generic CKKS operation with a bounded multi-threaded evaluator; moves CKKS encoding off the host-side codec onto a backend-resident op family backed by a new HAL plan cache and multi-ring FFT/NTT plan sets; and lands a full production-readiness pass over `poulpy-ckks` (typed errors, constructor-validated plans, scratch-carved intermediates, binary128-exact table generation, and a broad consolidation of the four-layer api/oep/delegates/default wiring). It also generalizes the HAL's scalar words: every polynomial layout is now keyed by an explicit **word type** (`ZnxWord` / `BigWord` / `DftWord`) naming its byte-layout convention — with the DFT/big-domain containers additionally keyed by the **backend** that produced them — and the multi-prime NTT scalar becomes the prime-set-parameterized `CrtWord<P, T>`, opening the path to `i32` coefficient words and new CRT lane geometries without further layout changes. That path is now open end to end: the coefficient layouts lose their `i64` default, `poulpy-core`, `poulpy-ckks` and `poulpy-bin-fhe` thread the word through their own containers, and buffer/word pins in generic code are replaced by the backend's associated types, so a narrower-word or device-resident backend is expressible without touching the op layers. Cross-backend buffer movement is explicit: layout-identical backend pairs declare per-container `*LayoutCompatible` markers (validated by a new word-compat test suite) unlocking a zero-copy `into_backend` re-tag. Finally, it adds **SHIP** (Cheon, Hanrot, Kim & Stehlé, [ePrint 2025/784](https://eprint.iacr.org/2025/784)) — a shallow CKKS half bootstrap built on sparse-secret mux blind rotations — as a second native bootstrapping family.

### `poulpy-hal`: word-keyed layouts

- Add `layouts::word` with the word traits `ZnxWord` (coefficient domain; `i64`, carrying `const BITS`), `BigWord` (accumulator domain; `i64`, `i128`) and `DftWord` (prepared/DFT domain; `f64`, backend CRT blocks). A word type names a **byte-layout convention** (element packing, lane count, prime set, ordering, reduction form) and governs sizing, element views, and serialization interpretation; it does not by itself grant cross-backend interchangeability (that is the layout-compat markers' job, below). `Backend::bytes_of_*` overrides remain authoritative for total buffer sizes (a word is a sizing + identity contract, not necessarily an element view — e.g. `Q126Scalar` over planar storage).
- **Breaking:** `Backend`'s scalar associated types are renamed — `ScalarPrep` → `DftWord`, `ScalarBig` → `BigWord` (`size_of_scalar_prep`/`size_of_scalar_big` → `size_of_dft_word`/`size_of_big_word`) — and every backend additionally declares its coefficient word `type ZnxWord` (currently `i64` everywhere; not yet consumed by the op layer). External `Backend` implementations must apply both renames and add the new type.
- **Breaking:** the DFT/big-domain layouts are re-keyed by word **and** backend: `VecZnxDft<D, B: Backend>` → `VecZnxDft<D, W: DftWord, B: Backend<DftWord = W>>`, and likewise `VecZnxBig<D, W: BigWord, B>`, `SvpPPol` / `VmpPMat` / `CnvPVecL/R<D, W: DftWord, B>` (struct-level bound; inconsistent word/backend combinations are unnameable). The word names the byte-layout convention, the backend pins provenance: containers of different backends are distinct types even at equal words, so accidental cross-backend interchange is a compile error. The `*Owned` / `*BackendRef` / `*BackendMut` aliases absorb the change (`VecZnxDftOwned<B> = VecZnxDft<B::OwnedBuf, B::DftWord, B>`; `CnvPVecLOwned` / `CnvPVecROwned` are new), carrying the whole api/oep/delegates op layer unchanged.
- `VecZnx` / `ScalarZnx` / `MatZnx` gain a defaulted word parameter (`VecZnx<D, W: ZnxWord = i64>`); every existing spelling keeps compiling, and `ReaderFrom`/`WriterTo` become word-generic over the unchanged, untagged wire format — lengths validate against `size_of::<W>()` and the receiver's declared word interprets the bytes (the `i64` format is byte-identical to before). Value-level `i64` code (encoding, `FillUniform`, ternary/binary samplers, `bytes_of` sizing) stays pinned to the `i64` default.
- **Breaking:** backend-bound constructors spell the backend through the owned alias — `VecZnxDftOwned::<B>::alloc(…)` / `from_bytes(…)` and likewise for the other five — and the zero-copy DFT→big re-tag infers everything from the value: `x.into_big()` (its size invariant is a hard assert).
- With the backend recoverable from every DFT/big-family value's type, `x.to_backend_ref()` / `x.to_backend_mut()` infer it everywhere — no turbofish, no UFCS spelling (the `*ToBackendRef/Mut` traits remain the bound vocabulary for generic code).
- New `layouts::layout_compat`: per-container-family `unsafe` markers (`VecZnxDftLayoutCompatible`, `SvpPPolLayoutCompatible`, `VmpPMatLayoutCompatible`, `VecZnxBigLayoutCompatible`, `CnvPVecLayoutCompatible`) that a backend pair declares to assert byte-identical layouts, unlocking the zero-copy `x.into_backend::<B2>()` re-tag on all six containers (reflexive for every backend; emitted automatically both ways by `impl_backend_from!`). The new `test_suite::word_compat` validates each declared marker with byte-equality and cross-consumption tests — and immediately earned its keep: the accelerated NTT4x30 backends prepare `VmpPMat` in a prime-major planar layout while the reference backend uses block-interleaved q120c, so no `VmpPMat` marker exists between them and the mismatch is unrepresentable instead of latent.
- `ZnxView::raw()`/`at()` (and mut variants) now assert the element view fits the underlying buffer (word-sized span vs actual allocation) and debug-assert word alignment, turning "word is sizing-only here" situations (e.g. the packed IFMA `VmpPMat`) into panics instead of out-of-bounds reads; the coefficient-domain delegate macros bound `Backend<ZnxWord = i64>`, making a backend's declared coefficient word load-bearing.
- **Breaking:** `Backend` requires `PartialEq + Eq` (all marker types are ZSTs deriving them), which makes the prepared types' equality derives live. The DFT/big containers define `PartialEq`/`Eq` (and `Hash` where provided) manually on the representation — same shape, same buffer bytes — bounding neither the word nor the backend, so `Eq` holds even for `f64`-word containers (byte equality is a total equivalence); higher-level prepared wrappers retain only `PartialEq`. `VecZnxBig::stats` is an inherent impl on `VecZnxBig<D, i64, B>`.

### `poulpy-cpu-ref` / CPU backends: prime-set-parameterized NTT words

- **Breaking:** `PrimeSet` is lane-generic: `type PrimeElem` (`u32` for the ~30-bit family, `u64` for ~42-bit), and the GAT `type Lanes<T> = [T; N]` — the lane count's single source of truth via `LaneArray::LEN`, blanket-implemented for every `N` (bytemuck's `min_const_generics` feature supplies `Pod for [T; N]`) — keeping `Q` / `OMEGA` / `LOG_Q`. The family-neutral machinery — `PrimeSet`, `LaneElem`, `LaneArray`, and `CrtWord<P, T>` itself — lives in `poulpy_hal::layouts` (module `layouts::crt`, next to the word traits), so the 42-bit family no longer reaches into the ntt4x30 module for its prime-set vocabulary; `ntt4x30::primes`/`types` re-export it for compatibility and keep what is genuinely the family's: `Primes29/30/31`, `PrimeSetCrt4`, the `Q120*` aliases and `Q_SHIFTED`. CRT *reconstruction* constants leave the base trait for family-specific extensions: full-CRT on `PrimeSetCrt4` (implemented by `Primes29/30/31`; the ntt4x30 kernels' `P: PrimeSet` bounds become `P: PrimeSetCrt4`) and Garner on `PrimeSetNtt3x42Ifma`, whose `Primes42` now also implements the unified `PrimeSet`.
- **Breaking:** `Q120bScalar` becomes the alias `CrtWord<Primes30, u64>` of the new `#[repr(transparent)]` block type `CrtWord<P: PrimeSet, T: LaneElem>` (a `DftWord` for any `(P, T)`): prime set, lane element, and exact lane count are all part of the type, so distinct CRT conventions cannot unify accidentally. Lane access (`.0[k]`) is unchanged; tuple construction must spell `CrtWord([…])` since type aliases cannot act as constructors. `Q126Scalar` stays a standalone sizing-only `DftWord` for the planar IFMA layout.
- All nine CPU backends (and `HostBytesBackend`) declare `type ZnxWord = i64` alongside the renamed `DftWord` / `BigWord`, and every backend marker ZST derives `PartialEq, Eq, Hash` (required by the new `Backend` supertraits).
- Each accelerated crate gains a `layout_compat` module declaring `VecZnxDftLayoutCompatible` / `SvpPPolLayoutCompatible` / `VecZnxBigLayoutCompatible` (both directions) between the reference backend and its accelerated sibling — `FFT64Ref`↔`FFT64Avx`/`FFT64Avx512`/`FFT64Neon` and `NTT4x30Ref`↔`NTT4x30Avx`/`NTT4x30Avx512`/`NTT4x30Neon` — validated by `word_compat` suite instantiations in each crate's tests. `VmpPMat` and `CnvPVec` markers are intentionally absent (the accelerated NTT4x30 `VmpPMat` layout diverges from Ref's; `CnvPVec` layouts are unverified). `NTT3x42Ifma` pairs with nobody (unique `Q126Scalar` word).
- `poulpy-cpu-arm`: the NEON ntt4x30 modules (`ntt4x30_convert`, `ntt4x30_ntt`) are repaired — they are gated on `enable-neon` **and** `target_arch = "aarch64"` and had silently rotted out of every check lane; the `neon` CI job (`.github/workflows/ci.yml`, native aarch64 runner) checks, lints, and tests the crate with `enable-neon,enable-ckks`.

### `poulpy-core`

- The prepared layouts (`GLWEPrepared`, `GGLWEPrepared`, `GGSWPrepared`, `GLWESecretPrepared`, `PreparedDiagonal`, …) follow the HAL re-key transparently — inner buffers are keyed by `(B::DftWord, B)` — and their derived `Eq` is dropped (`PartialEq` retained, and now live via `Backend: PartialEq`); the HAL containers themselves implement `Eq` unconditionally (equality is byte equality and never compares a word value).
...
- **Breaking:** remove `max_size` from the layout family, completing the move to view-based narrowing above. `VecZnxShape` (shared by `VecZnx`, `VecZnxDft` and `VecZnxBig`) carried both `size` and `max_size`, but no library allocation path ever set them apart: every constructor passed `size` twice and `vec_znx_alloc_with_max_size` was only ever called with equal arguments, so the capacity field was a stored duplicate that could only ever diverge by mistake. `size` is now both the working and the allocated width, fixed at construction, and operating on fewer limbs is exclusively a borrowed view: the existing `VecZnxDftBackendMut::with_size_mut`, plus the new free-standing `vec_znx_backend_mut_with_size` for `VecZnx` (free-standing rather than inherent because `VecZnxBackendMut<'_, B>` reaches `B` only through associated types, which leaves `B` unconstrained in an `impl` header). Removed: `VecZnxShape::max_size`, the inherent `max_size()` on all three containers, `from_data_with_max_size` (folded into `from_data`), and `vec_znx_alloc_with_max_size` (trait method, inherent impl, and delegate); `VecZnxShape::new` loses its fourth argument and `with_size` now asserts `size <= self.size()`, since views only ever shrink. The `VecZnx` wire format drops one `u64` field, which also removes a latent hazard: `WriterTo` wrote `max_size` but only `n·cols·size` limbs of payload, so a deserialized value could hand out a narrowing view over limbs that were never read. The one place the two widths genuinely diverged was the HAL test suite, where nine `vec_znx` tests allocate four limbs and declare a smaller working size to catch kernels writing past `size`; they now allocate at full width and narrow at the kernel call through the new `test_suite::vec_znx_backend_mut_sized`, preserving the out-of-range check.
- **Breaking:** split `ZnxInfos` into a family-neutral shape plus two family traits. `ZnxInfos` keeps `n()`, `log_n()`, `size()` and `poly_count()`; the new `VecZnxInfos: ZnxInfos` adds `cols()` (`VecZnx`, `VecZnxDft`, `VecZnxBig`, `ScalarZnx`, `SvpPPol`, `CnvPVecL/R`) and `MatZnxInfos: ZnxInfos` adds `rows()`, `cols_in()` and `cols_out()` (`MatZnx`, `VmpPMat`). One trait previously served both families dishonestly: `cols()` returned `cols_in()` on the matrix containers, `rows()` was a hardcoded `1` on every vector container, and `cols_out()` existed only as an inherent method, so no trait bound could read a prepared matrix's output rank (the blocker for making the gadget-key layouts generic over their payload). `poly_count()` becomes required rather than defaulted: the old `rows · cols · size` was correct only for the vector family and both matrix containers already overrode it, so a new layout now gets a compile error instead of a silent under-count; every implementor's value is unchanged. Consequently `ZnxView` requires `VecZnxInfos` (its `at_ptr` strides by `cols()`) and the matrix containers no longer implement it: they never used the columnar accessors (`MatZnx` shadowed `at` with its own `(row, col)` entry view, `VmpPMat` only ever took `raw()`), and instead expose inherent `raw()` / `raw_mut()` routed through the new shared `raw_scalars` / `raw_scalars_mut` / `element_view_span` helpers, so the alignment and buffer-length assertions live in one place rather than four. Migration is small in practice because the concrete layouts keep inherent `cols` / `rows` / `cols_in` / `cols_out` that shadow the traits: only fully-qualified `<X as ZnxInfos>::cols/rows` spellings (the four backends' `vmp_apply_dft`) need renaming, and generic code bounds on the family trait it actually uses.
...
### `poulpy-core`
- **Breaking:** the limb-width model collapses to two quantities — the claimed precision `k()` (with the derived `size()` = `ceil(k/base2k)`) and the allocation `max_size()`/`max_k()` — and the *stored* width disappears as a semantic concept. Previously three widths hid behind one name: `LWEInfos::max_size()` meant the buffer's current stored width on most types, the meta-derived width on layout types, and the allocated capacity on `GLWETensor`, while an inherent `GLWE::max_size()` (capacity) shadowed the trait impl, so the same call returned different numbers depending on whether it resolved concretely or through a trait bound. Now `LWEInfos::max_size()` has exactly one meaning — the allocated width, `size() <= max_size()` — implemented off the backing buffer's allocation for buffer-backed types and meta-derived for layout/spec types. Consequences: the `Compact` and `SetSize` traits are **removed** (nothing shrinks a buffer below its allocation; shedding memory or narrowing a result means allocating/carving the destination at exactly the `k` you want, which was already the mul-family contract), `GLWEInfos`-bound signatures across api/oep/delegates/default drop their `Compact` bounds, buffers always serialize at their allocated width, and processing width is governed by effective `k` — with one *typed* exception: plaintext operands are **integer polynomials, not Torus elements** — LSB-anchored, every encoded limb carries data — and declare their consuming width through the new `IntPolyInfos::encoded_k()` trait, implemented across the whole plaintext family — `GLWEPlaintext`/`LWEPlaintext` (+ their layout specs and scratch-view wrappers), `PreparedDiagonal`, and the CKKS plaintext types (currently equal to the allocated width, an encoder property rather than a vocabulary one). `GLWEMulPlain` and every other plaintext-consuming op bound their plain operand on `IntPolyInfos`, so a type that cannot state its encoded width cannot reach a convolution, and `max_k()` is never consumed by compute anywhere.
- Fixed, uncovered by the width-model collapse: the inherent `GLWE::max_size()` shadowed the `LWEInfos` impl with different semantics (allocation vs stored width), so the same call returned different values depending on resolution path, and the CKKS info forwarders inherited opposite semantics from it; `GLWETensor` reported its allocation as the trait width where every sibling reported the stored width; the linear-transformation eval scratch budget claimed to size off allocated capacity but read the stored width (the trait could not express capacity); `SetSize::set_size` wrote a width that `size()` did not read back; and `Compact::compact()` carried an undocumented `+ log_n` retention margin with no measurable effect.
- `LWEInfos::max_size()` keeps both its meaning (the allocation) and its distinction from `size()` (derived from the claimed `k`), but buffer-backed types now read it off the payload's `size()`, since a HAL layout's size *is* its allocation after the `max_size` removal above. The two `LWE::validate_shape` checks asserting `size <= max_size` on the body and mask buffers are dropped: the condition is no longer representable.
...
- **Breaking:** every gadget-key layout (`GGLWE`, `GGSW`, `GLWESwitchingKey`, `GLWEAutomorphismKey`, `GLWETensorKey`, `GGLWEToGGSWKey`, and the LWE-switching / blind-rotation keys built on them) now stores an **auxiliary guard** `k_aux: TorusPrecision` in place of the total torus precision `k`. The total is derived — `k() = dnum·dsize·base2k + k_aux` — so the gadget region (`dnum·dsize·base2k`) and the below-gadget guard (`k_aux`) are parameterized independently, and keys enforce the invariant `k_aux ≥ dsize·base2k` (at least one full gadget digit of guard). Migrating a former `Dnum(d)` key that carried an implicit zero guard gives `Dnum(d−1)` with `k_aux = dsize·base2k` — its last digit was drowned in the noise anyway (`Dnum(d)/k_aux=0 ≡ Dnum(d−1)/k_aux=base2k`).
- **Breaking:** the gadget-key operations (`glwe_keyswitch`, `glwe_external_product[_assign]`, `glwe_automorphism`, `glwe_tensor_relinearize`, and their bin-fhe/ckks compositions) no longer take an explicit output/working **size** argument. The working limb width is now derived internally from the input ciphertext's `k` and the key's `(dsize, k_aux)`, replacing the unreliable `ct.size() + key.dsize()` heuristics that could truncate a gadget digit mid-decomposition and mismanage the output noise.
- **Breaking:** key allocation and byte-sizing signatures are reordered to a uniform `(…, dnum, dsize, k_aux, rank…)` parameter order (`dsize` now precedes `k_aux`) across the api/compressed/prepared layers and their `*_alloc` / `bytes_of` / `*_prepared_alloc` families.
...
- The direct bootstrap entry points (`ckks_paco_bootstrap_direct_into` and the parallel variant) produce a **leveled output**: the caller allocates the output at any final torus width `k ≤ k_out` — query the maximum with the new `PaCoContext::max_output_k(&keys)` — and the branch evaluates the whole circuit at the correspondingly narrower working width `k + circuit_depth`, so a lower requested level runs genuinely cheaper (fewer limbs through every stage). The circuit no longer runs in-place on the caller's output: it evaluates in a private accumulator and writes the result out (matching the classic `ckks_bootstrap` convention), so the output no longer doubles as the working buffer. An output level above `k_out` is rejected up front. Works for any `k`, including non-`base2k`-aligned widths and the wide-scale `Quad` parameterization.
...
- Add SHIP as a native CKKS half bootstrap: a one-limb bottom ciphertext holding gap-`gamma` cleartexts in its coefficients is refreshed into a slots-domain ciphertext at the raised precision, **without ModUp or EvalMod**. The pipeline is one dense -> sparse encapsulation keyswitch at the bottom modulus, then per support slot the theta-column masking (keygen-prepared selector-mask operands accumulated in the DFT domain through the fused `cnv_accumulate_dft`, one IDFT + normalize per column) followed by hoisted base-B mux blind rotations (one input DFT per digit position, gadget-decomposed VMPs, DFT-domain output automorphisms whose plans are built once per bootstrap), and a binary product tree over the `h + 1` factors (depth `1 + ceil(log2(h + 1))`), closed by a single homomorphic conjugation.
- Evaluation is exposed through `api::CKKSShipOps<BE, F>` on `Module<BE>`, all outputs caller-allocated: `ckks_ship_bootstrap_into` (real cleartexts in the first `N/2` coefficients), `ckks_ship_bootstrap_complex_into` (`Re`/`Im` coefficient halves recombined with one conjugation; requires the `omega_2` mask set), the validating scratch query `ckks_ship_bootstrap_tmp_bytes`, and `ckks_ship_coeff_encodings` (+ `_tmp_bytes`) building the input-dependent plaintext material. The scalar is sealed behind `api::ShipScalar` (`f64`, `Quad`), which bounds the one-limb bottom radix by the mantissa.
- Invariant-bearing data lives in `layouts`: `ShipPlan` (constructor-validated gap, working precision, output budget, and sparse-secret/mux dimensions, with derived `raised_k`, mixed-radix `mux_bases`, and tree depth), `ShipSecretSpec` (samples and validates the regularly-spaced sparse support), and `ShipKeySet` -> `ShipKeysPrepared` (per-slot mux key groups and `4*theta` selector masks prepared as left convolution operands, the dense -> sparse encapsulation key kept at the bottom modulus, plus tensor and conjugation keys), fingerprinted by `ShipKeyParameters`. Key generation is integrated (`ShipKeySet::generate`, host-side): the mux keys are non-standard rank-2 -> 1 switching keys with automorphism-twisted output secrets that no core keygen produces.
- The only SHIP-specific backend hook is coefficient encoding via `oep::CKKSShipCoeffEncodingImpl<BE>`; the scheme-defining host reference `encoding::ship_coeff_encodings_host` is public and `poulpy-cpu-ref` exports `impl_ckks_ship_coeff_encoding!`, invoked by all CPU backends. Everything else composes existing CKKS, core, and HAL operations.
- Backend-generic tests cover a cleartext replica of the paper's Algorithm 1 (pinning the phase, mask, and conjugation conventions before any homomorphic code runs), the HMuxRot primitive, and end-to-end real and complex bootstraps gated on recovered precision, running on every CPU backend at `f64` and `Quad` precision.
...
- **Breaking:** `CKKSMeta` gains `slots: SlotsKind` (`Real` / `Complex`, defaulting to `Complex`) recording which subfield the slots are known to live in; every `CKKSMeta` literal must name it. Ops compose it with `SlotsKind::join`, which keeps `Real` only when every operand is `Real`; `mul_i` and linear transformations yield `Complex`, and the conjugate folds (`z + conj(z)`) restamp `Real`. Read with `CKKSInfos::slots`, set with `SetCKKSInfos::set_slots`. Replaces the private `SlotsKind` of the functional-bootstrapping module.
...
- `CKKSModuleAlloc` is fully default-bodied over `ModuleCoreAlloc` (backends get it from an empty blanket impl) and gains `ckks_ciphertext_alloc_with_rank`, with `ckks_ciphertext_alloc` as the rank-1 convenience. The `CKKSImpl` one-stop bundle now also covers `DFTImpl` and `CKKSEvalModImpl`; bootstrapping is a direct no-OEP composition whose only two traits are the execution API (`CKKSBootstrappingOps`) and prepared-key provider (`BootstrappingKeys`). The `oep` module docs record the wiring-pattern taxonomy (opt-in marker defaults, unconditional composite blankets, scalar-generic backend-side seams, no-OEP composite families).
...
- Add `poulpy-cpu-ref/src/ckks_ship.rs` with the `impl_ckks_ship_coeff_encoding!` macro (same staging pattern for the SHIP coefficient encoding), invoked by the same backends.
- Fix `poulpy-cpu-avx512` compiling with `enable-ckks,enable-avx512f` but without `enable-ifma`: the `NTT3x42Ifma` CKKS wiring macros are now individually feature-gated.

### `poulpy-hal`: word genericity delivered

- **Breaking:** `VecZnx`, `ScalarZnx` and `MatZnx` lose the `= i64` default on their word parameter (`VecZnx<D, W: ZnxWord>`): a coefficient layout can no longer be named without stating its word. Sizing follows (`bytes_of`, `rsh_tmp_bytes` are word-generic), and `ZnxWord` gains `from_i64` so `FillUniform` and the ternary/binary secret samplers become word-generic too (`FillUniform` now asserts `log_bound <= W::BITS`). `layouts::encoding` stays bound to `ZnxWord = i64`: the integer encode/decode family is genuinely i64-shaped.
- `Backend` gains provided, overridable sizing methods `size_of_znx_word`, `bytes_of_vec_znx`, `bytes_of_scalar_znx` and `bytes_of_mat_znx`, completing the existing DFT/big pair. Scratch and buffer sizing now routes through the backend that owns the memory instead of the host's word.
- Fixed: the `MatZnx` backend entry-view helpers computed row/column offsets from the dense host formula (`n * cols * size * size_of::<W>()`, pinned to `i64`) while allocation goes through the overridable `Backend::bytes_of_mat_znx`. Both halves are corrected: the stride now calls `B::bytes_of_vec_znx`, so a backend with a narrower coefficient word *or* a padded buffer addresses the right entries. The inherent `MatZnx::bytes_of` / `entry_bytes` stay dense, describing the host layout they are named for. The same class of defect is fixed in the test suite's `upload_scalar_znx` / `upload_vec_znx` / `upload_mat_znx`, which accepted an `i64` host layout and returned a `BE::ZnxWord` device layout in one signature, and in `test_suite::word_compat`, whose backend pair asserted matching `OwnedBuf` and `DftWord` but not `ZnxWord`.
- The generic test suites drop 11 `Backend<OwnedBuf = Vec<u8>>` bounds: host reference operands are allocated on the host (`VecZnx::alloc`) instead of through the device module, and the new `alloc_host_vec_znx::<BE>` carves a host template sized for the destination backend's word. `TestBackend` remains the single documented `ZnxWord = i64` pin (the suites drive `encode_*`/`decode_*`).

### `poulpy-core`: backend-routed sizing

- **Breaking:** the cross-backend layout transfers (`ModuleTransfer::upload_*` / `download_*` and the `to_backend` wrappers) now require both backends to share a coefficient word (`To: Backend<ZnxWord = From::ZnxWord>`). `TransferFrom::transfer_buf` sees an opaque buffer with no shape and no `base2k`, so it cannot re-decompose limbs, and the helpers re-attach the source's shape to the destination; without the bound, threading the word through these signatures would have permitted a raw byte re-tag across differing words. Moving a value between backends whose limb axis differs is a re-encoding with a caller-allocated destination, and is left to a follow-up.
- Fixed: `lwe_bytes_of_from_infos` (and the `LWE::bytes_of` formula it inherited) summed the body and mask sizes with no padding, but an LWE is two separate buffers and `ScratchArena::take_region` aligns every carve to `SCRATCH_ALIGN`. The reservation under-counted the gap before the mask by up to `SCRATCH_ALIGN - 1` bytes, which surfaces as a scratch-exhaustion panic on small limb counts (an 8-byte body consumes 64). The body is now rounded up to the arena's alignment.

- Add `api::GLWEBytesOf<BE>` on `Module<BE>` (`glwe_bytes_of`, `glwe_secret_bytes_of_from_infos`, `gglwe_bytes_of_from_infos`, `ggsw_bytes_of_from_infos`, and friends), replacing host-pinned `Type::<Vec<u8>, i64>::bytes_of` calls at scratch-sizing sites. Sizing is keyed by domain rather than by word, because one `DftWord` spans five layouts and a backend may share `ZnxWord` with `BigWord`.
- **Breaking:** `ModuleCoreAlloc` gains `type ZnxWord`, and `ModuleCoreCompressedAlloc` gains both `type OwnedBuf: Data` and `type ZnxWord`, returning `Self::OwnedBuf` across its 40+ constructors instead of `Vec<u8>`. Compressed key allocation is no longer host-pinned.
- The layout family is word-generic throughout: `GLWE`, `GLWEPlaintext`, `LWE`, the gadget keys and their compressed/prepared forms all thread `W`, and the api/oep/delegates/default layers name `BE::ZnxWord` rather than `i64`. A handful of methods stay i64 by construction (`fill_binary_coeffs`, the integer encoders) and say so.

### `poulpy-ckks`: word-generic containers

- **Breaking:** `CKKSCiphertext<D, W, S = Normalized>` and `CKKSPlaintext<D, W>` carry the coefficient word; the latter also drops its `D = Vec<u8>` default. New aliases `CKKSCiphertextOwned<BE>` and `CKKSPlaintextOwned<BE>` name the backend instead of its representation and absorb the change at most call sites.
- The two container definitions were the crate's whole i64 dependency: threading them collapses 316 `Backend<ZnxWord = i64>` bounds to 10, all in the host float codec path. `CKKSPlaintextVecHostCodec` stays implemented only for `CKKSPlaintext<D, i64>` (it quantizes to i64/i128 limbs), the two host reference coefficient encoders (`paco_coeff_encodings_host`, `ship_coeff_encodings_host`) state the i64 requirement they inherit from `decode_vec_i64`, and `TestContextBackend` carries one documented pin for the suites.
- `CKKSConjugateImpl` no longer pins the word: its requirement was `GLWEShift<BE>`, which bottoms out in the HAL's i64 coefficient-domain delegates, so the bound is now stated as the trait it needs.
- Fixed: `delegates::eval_mod` sized backend scratch with host `VecZnx::<Vec<u8>, i64>::bytes_of` rather than `BE::bytes_of_vec_znx`, so the reservation could disagree with what `take_vec_znx_scratch` allocates.

### `poulpy-bin-fhe`: word-generic containers and host de-pinning

- **Breaking:** the storage types carry the coefficient word: `FheUint<D, T, W>`, `FheUintPreparedDebug<D, T, W>`, `LookupTable<D, W>` (dropping its `D = Vec<u8>` default), `BlindRotationKey<D, BRT, W>`, `BlindRotationKeyCompressed`, `CircuitBootstrappingKey` (both forms), `BDDKey<D, BRA, W>` and `GLWEBlindRetriever<D, W>` (also dropping a host default). `LookupTableFactory<D, W>` is parameterized to match, so the LUT vocabulary is no longer written in terms of `Vec<u8>`.
- Trait declarations and impls now state what they actually need. Where a body requires host-visible bytes it says `Backend<OwnedBuf: HostDataMut + HostDataRef>` instead of `OwnedBuf = Vec<u8>`, which any host-visible buffer type satisfies: 75 bounds across `Cmux`, `Cswap`, `ExecuteBDDCircuit`, `GetGGSWBit`, the blind-rotation and circuit-bootstrapping layers, and the key encryption/preparation traits. Key allocation moves off host storage entirely (`BlindRotationAlgo::alloc_key` returns `BlindRotationKey<M::OwnedBuf, Self, M::ZnxWord>`).
- What remains pinned is the part that is genuinely i64: the BDD and CGGI evaluation path writes message bits straight into i64 coefficients (`encode_vec_i64`, `encode_coeff_i64`, i64 GGSW selectors, `mod_switch_2n` over `&[i64]`), so those items keep `ZnxWord = i64` where the requirement is created rather than copied crate-wide.
- Fixed: the compressed `CircuitBootstrappingKey<D, BRA>` was generic over its buffer but held `GLWETensorKeyCompressed<Vec<u8>, i64>` and `GLWEAutomorphismKeyCompressed<Vec<u8>, i64>` fields, so a device instantiation would have compiled while silently holding host key material. Fixed: the CGGI blind rotation sized backend scratch with `VecZnx::<Vec<u8>, i64>::bytes_of` instead of `BE::bytes_of_vec_znx`.
...
- Add `docs/ship.md` (user-facing SHIP guide: API, construction outline, key and ciphertext contract, security discussion); link it from `docs/README.md`.
- Update `docs/bootstrapping.md` for the constructor-validated plans (corrected `1/K` CoeffsToSlots pre-scale documentation, encapsulation weight moved to `EncapsulationKeysLayout`, power-of-two `f_mod_interval` requirement) and add a Toolchain section to the `poulpy-ckks` README documenting the nightly requirement (`#![feature(f128)]`), the x86_64 Linux GNU-only `libquadmath` fast path, and the portable binary128 fallback elsewhere.
- Add native Intel macOS and Apple Silicon CI smoke coverage for the portable binary128 math surface and `Quad` CKKS encoding; the Intel lane also exercises the Darwin AVX FFT assembly path.
