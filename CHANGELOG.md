...
- **Fix:** `ckks_rotate_into` and `ckks_conjugate_into` stamped the destination's metadata after the key switch, so the key switch normalized its output at the destination's stale, wider `k` and the result carried key-switch noise below the width it was then labelled with; a convolution consumer rescales by `2^k`, so those bits were harmless only because the `cnv_prepare_*` mask clamped them away. The stamp now precedes the write, there and in every unary op (`ckks_copy`, `ckks_neg_into`, `ckks_mul_i_into`, `ckks_mul_pow2_into`, `ckks_div_pow2_into` and the plaintext add/sub into-forms), and stamps the final metadata. `offset` is computed against the pre-stamp destination, so the alignment and the decrypted values are unchanged; `div_pow2` moves its charged bits under `log_delta` inside the stamp instead of after the shift, which would otherwise have the shift round `bits` too low now that it honours `k`. With the `poulpy-core` shift change, every CKKS operation leaves its output canonical at the `k` it reports; the test suite's decryption helpers assert it on every ciphertext they decrypt, so each operation test pins it on every backend.
