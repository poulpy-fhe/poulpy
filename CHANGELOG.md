...
- **Breaking:** every GGLWE product binds its key for the operation's exact input precision. `GGLWEInfos::bind_for` returns a `GGLWEUse` (`Empty` at zero precision, otherwise the logical layout, the physical row origin and step, the readable limb prefix and the storage pitch), and `gglwe_product_dft_default` takes that bound beside the complete physical key. A stored key binds through its own `dsize`, a `with_dsize` wrapper through the one it requests, so `gglwe_product_dft_selected` and the native/selected fork are gone.
- A GGLWE product validates the prepared key against the physical shape its bound was resolved from before dispatch: degree, rows, both column counts, the stored limb pitch, the selected rows and prefix, and that the backing covers `bytes_of_vmp_pmat`. A key restored with metadata that disagrees with its buffer fails here rather than inside a kernel.
- Every automorphism variant sizes its product from the bound; only `add_assign` did, so the add/sub forms under-sized a coarsened key the same way the linear transformation did.
- Raw binding remains intentionally permissive: `GGLWEBind::bind_for` may return the available low digits with `GGLWEActiveUse::covers_input == false`. Registry selection, linear-transformation preflight and CKKS ciphertext multiplication dispatch use `bind_covering_for`, so they never select a key whose effective digits and auxiliary padding do not cover the requested precision.
- Fix the linear-transformation baby- and giant-step sizing under-reporting a coarsened key: a `with_dsize` wrapper forwards the physical `dnum`/`k_aux`, so both now size from the resolved logical layout.
- `LinearTransformationBabySteps` records the source precision it was allocated for, and evaluation rejects a cache prepared at another precision. Baby rotations resolve at that source precision; giant rotations resolve at the post-PROD destination precision, so one factor may intentionally use different effective decompositions on its two sides.
...
- **Breaking:** the core tensor batch OEP scratch queries take one `GGLWEActiveUse` per active lane, and execution takes aligned `GLWETensorKeyPreparedBound` values. The typed bound can only be constructed from a prepared tensor-key view plus its active use. The provided default validates every lane and the shared scratch before its first write; an overriding backend receives the active frontier as one call and owns any fusion or splitting.
...
- Add `GGLWEProductDefault::gglwe_product_dft_tmp_bytes_upper_default`, the bound-product scratch over every input precision at or below the queried one. The exact requirement is not monotone in that precision, so whole-chain/proxy linear-transformation queries use this upper form for baby products, lazy giant rotations and fallback automorphisms; exact transform queries keep the exact bound.
...
- The resolver, product-term counts and registry material scores use checked widened arithmetic and report overflow instead of saturating. Registry ordering remains deterministic, and layouts whose total precision cannot be represented are rejected rather than truncated.
...
- **Breaking:** the exact linear-transformation scratch queries take the destination, source, transform and key layout helper used by evaluation, resolve baby rotations at source precision and giant rotations at destination precision, and follow every rotation the transform visits. `glwe_prepare_linear_transformation_baby_steps_tmp_bytes` takes the rotation list for the same reason.
- Add `glwe_eval_linear_transformation{,_unprepared_rhs}_bound_tmp_bytes`, the conservative upper bound over any transform from one representative key, for callers that allocate one arena before knowing which transforms or input precisions will use it. The two share the per-key half of the budget, so they cannot drift.
...
- The homomorphic (I)DFT chain ping-pongs `ct` with one scratch ciphertext instead of copying back per factor. Each factor stamps its natural post-factor destination metadata while retaining the reusable physical ping-pong buffers, after cache and key-bound preflight succeeds.
...
- **Breaking:** a CKKS batch resolves every lane at its exact precision, validates all semantic plans, prepared bounds and scratch before writing, and sends one active frontier to the batch OEP. Scratch queries receive aligned `GGLWEActiveUse` values and execution receives aligned `GLWETensorKeyPreparedBound` values; the backend owns any fusion or splitting, and distinct same-shaped physical keys are legal in one call.
...
- Empty or zero-precision lanes take the scalar no-product path and do not read a product key; active lanes remain in their original frontier order.
...
- PaCo uses one checked, evolving branch-key schedule for both runtime validation and scratch planning: linear-transformation babies resolve at each factor's source precision and giants at its destination, a ψ pair advances precision once after both branches, product rotations and relinearization keys resolve at the product source, and later conjugations resolve at the then-current precision. Every prepared key is checked for covering bounds before execution.
...
- **Breaking:** `ckks_eval_linear_transformation{,_streamed}_into_tmp_bytes` take the destination and source explicitly, as well as the transform and key source. The assign/self-assign queries plan and allocate the transform's natural lower-precision destination before copying it back; `ckks_prepare_linear_transformation_baby_steps_tmp_bytes` takes the rotation list. `ckks_dft_evaluate_tmp_bytes` stays transform-free and uses the conservative bound form for a whole chain.
- Linear-transformation execution preflights the baby-cache precision, every required key lookup and every prepared covering bound before destination writes or DFT metadata stamping.
