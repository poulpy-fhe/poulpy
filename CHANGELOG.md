...
- Add crate-private working-scale adjustment for CKKS internals: `ckks_scale_down_assign` trades `bits` of precision for budget (right-shift, `log_delta -= bits`, `log_budget += bits`) and `ckks_scale_up_assign` does the inverse (left-shift into the budget headroom). Both keep the torus width `k` constant — so `log_budget = k − log_delta` follows the `log_delta` relabel for free — and report the new `InsufficientScalePrecision` error when `scale_down` would drop more than `log_delta` bits. `scale_down` is not value-preserving over the full modulus; it is only used under the internal flush discipline required by adaptive polynomial evaluation.
- Add adaptive Chebyshev polynomial evaluation (`CKKSAdaptivePolynomialEvaluation::ckks_eval_poly_real_const_coeffs_adaptive` with the `AdaptiveEvalMode` speed/modulus-saving selector, built from `Polynomial::encode_bsgs_adaptive` and the `AdaptiveBSGS` split): splits a Chebyshev approximation at the BSGS baby-step boundary into a full-precision low branch and a `2^drop`-compensated high branch evaluated at reduced scale, trading polynomial precision for saved modulus.
