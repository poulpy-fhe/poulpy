...
- **Breaking:** add scratch-free `VecZnxCanonicalize` to restore the canonical representation at a requested precision; `HalVecZnxImpl` gains the matching hook, implemented by every built-in CPU backend.
- **Breaking:** `VecZnx::data` is private and `VecZnx` is no longer `Copy`; use `data()`, invalidating `data_mut()`, or consuming `into_data()`. Canonical state is shared safely across mutable backend reborrows.
...
- Track whether `VecZnx` values are known canonical, restore canonical output after key-switch and convolution operations, and account for the resulting bounded rounding in noise estimates.
