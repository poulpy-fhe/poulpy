...
- **Breaking:** `Backend` requires `PartialEq + Eq` (all marker types are ZSTs deriving them), which makes the containers' and prepared types' `PartialEq` derives live; `f64`-word containers remain `PartialEq` but not `Eq`. `VecZnxBig::stats` is an inherent impl on `VecZnxBig<D, i64, B>`.
