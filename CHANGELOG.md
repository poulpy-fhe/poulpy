...
- The `vmp_apply_dft_to_dft` contract states `pmat.cols_out() == res.cols()` and `pmat.cols_in() == a.cols()` as preconditions instead of "the largest valid dimension is used": the flat row law needs them, and the NTT4x30 kernel now asserts them as the FFT64 one did. The `cnv_prepare_self` default body asserts `right.size() == left.size()`, which its contract already required. The unreachable FFT64 reference `vmp_apply_dft`, which read the wrong column and one limb, is gone; every backend inherits the derived body.
