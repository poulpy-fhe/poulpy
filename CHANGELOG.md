...
- **Breaking:** owned host storage is `AlignedBuf` (`poulpy_hal::AlignedVec<u8>`): `alloc_aligned` and `alloc_aligned_custom` return an `AlignedVec<T>`, which dereferences to a slice, is zero-initialized, padded to a 64-byte multiple by `alloc_aligned`, and frees its allocation with the layout that made it. `VecZnxOwned`, `MatZnxOwned`, `ScalarZnxOwned`, `HostBytesBackend::OwnedBuf` and every CPU backend's `OwnedBuf` are built on it; every layout `from_bytes` takes `impl Into<AlignedBuf>`, so a `Vec<u8>` argument still compiles and is copied into aligned storage; `ToOwnedDeep` and `to_host_owned` copy into aligned storage, so an owned layout can no longer sit on unaligned bytes. `Backend::from_bytes` copies its `Vec<u8>` instead of adopting an aligned one. The twiddle tables of the CPU backends hold `AlignedVec<u64>` and `AlignedVec<R>`.
