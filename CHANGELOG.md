...
- Add CKKS functional bootstrapping (`ckks_functional_bootstrap` and the shared-power-basis `ckks_functional_bootstrap_multi`) on the standard S2C-first `BootstrappingPlan` / `BootstrappingContext`. The implementation reuses slim bootstrapping's S2C, ModUp, C2S, key generation, scratch layout, and recipe budget accounting, replacing EvalMod with encoded general or binary LUT evaluation. LUT arity automatically determines `log_msg_ratio`, which is validated against the input after accounting for the bottom S2C cost; backend-generic tests cover the general, multi-LUT, binary, and mismatched-ratio paths.
