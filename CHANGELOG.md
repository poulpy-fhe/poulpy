...
- **Breaking:** arithmetic, encryption and polynomial-evaluation reference families now require explicit opt-in. `impl_core_reference_full!` forwards a complete backend, while per-family macros and callable `*Composition` traits allow individual overrides. Public delegates require the implementation they call, and compressed-mask reconstruction dispatches through the public API.
...
- Core contracts now have a compiler-checked method inventory and executable reference parity across portable FFT/NTT, AVX, AVX-512/IFMA, NEON and Rayon implementations. Coverage includes mutation variants, conversions, prepared keys, linear transformations, BSGS phases, encryption/decryption and compressed forms. Randomized parity injects the tested backend's realized samples into an independent reference composition; sampling distributions and source advancement are checked separately.
- CI verifies successful execution of every supported core contract group, including guaranteed AVX/AVX-512 coverage in the native/SDE lane and optional QEMU NEON runs. Reference compositions are tested with independently sized poisoned scratch, and strict core rustdoc is validated.
...
- **Breaking:** the `default` module is `reference` ([#234](https://github.com/poulpy-fhe/poulpy/issues/234), item 2): `poulpy_core::reference` holds the reference implementation of every operation, the composition traits are `*Reference` (`GLWEKeyswitchReference`, `EncryptionReference`, ...), their methods `*_reference`, and the backend wiring macros `impl_*_reference_full!` and `impl_core_reference!`. Every `poulpy_core::default` path, `*Default` trait, `*_default` method and `impl_*_defaults*!` macro renames one for one.
