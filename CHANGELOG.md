...
- **Breaking:** `cnv_prepare_left_lazy`, `cnv_prepare_right_lazy`, `cnv_apply_dft_lazy` and their `_tmp_bytes` are removed — they forwarded to the eager forms, which callers now use directly — along with `vec_znx_dft_add_scaled_assign` and its five kernels, which had no caller anywhere in the workspace. Their OEP methods, delegates and backend overrides go with them.
