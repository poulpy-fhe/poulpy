...
- Introduce the sealed coefficient-state algebra of `docs/spec/normalization_typestate.md` (§3) without touching any kernel or layout: `CoefficientState` with the forms `Unwritten`, `Raw` and `Coeff<N: Normalization, C: Canonicality>`, the canonicality markers `Canonical`/`NonCanonical`, the axis relation `CanonicalityFitsIn` and product relation `CoeffFitsIn`, and the conservative compatibility aliases `CoeffNormalized`/`CoeffUnnormalized`. The existing `NormalizationState` gains the bridge `type AsCoeff` mapping `Normalized`/`Unnormalized` onto those aliases, and the `Normalization` axis trait names the sealed subset usable inside `Coeff`. Crate-private groundwork for the later migration PRs: the immutable `CoeffContext` representation context (constructor enforces `represented_k > 0` and the live-limb arithmetic), the conservative `CarryCert` digit-bound certificate with its parallel-join and reservation rules (explicitly never consulted by normalization), and the reference canonical projection `P_p` with property tests (idempotence, grid alignment, normalized-interval preservation). Roots, operations and behavior are unchanged; the docs also gain `docs/spec/normalization_inventory.md` (frozen normalization contracts, workspace site inventory, baselines) enforced by API-shape tests in `poulpy-hal`/`poulpy-core` and the `scripts/normalization_denylist.sh` CI ratchet.
- **Breaking:** the root state parameter moves onto the coefficient-state algebra (spec PR 2): `VecZnx<D, W, S: CoefficientState = CoeffNormalized>` (and `GLWE`, `CKKSCiphertext`, the scratch views) replace the `NormalizationState` parameter, with `CoeffNormalized`/`CoeffUnnormalized` (`Coeff<Normalized, NonCanonical>` / `Coeff<Unnormalized, NonCanonical>`) as the conservative images of the old `Normalized`/`Unnormalized` root states. Operation signatures take `impl ArithmeticState` where they took `impl NormalizationState`, `CoeffFitsIn` replaces `FitsIn` in op bounds (with `CoeffFitsIn<CoeffUnnormalized>` as an `ArithmeticState` supertrait, restoring the "everything fits an unnormalized destination" elaboration), and the `State` associated type of every to-backend trait is bounded by `ArithmeticState`, so backend views exist only for arithmetic states until `Unwritten`/`Raw` roots become reachable in later migration steps. `Normalized`/`Unnormalized` remain as the sealed normalization-axis markers inside `Coeff<N, C>`. This is a pure vocabulary switch: no kernel, no state transition, and no runtime behavior changes, and `Unwritten`/`Raw` are not yet producible by any constructor or scratch take.
- **Breaking:** safe mutable word access is now state-gated and borrowed views cannot be relabelled (spec PR 2 exit gate, invariants 2 and 4). `DataViewMut` (and through its blanket impl `ZnxViewMut`: `at_mut`, `raw_mut`, `as_mut_ptr`) exists only for `Coeff<Unnormalized, NonCanonical>`; `ZnxZero` stays available for every state (zero preserves every claim) and `VecZnx::from_data_unnormalized` provides weakest-label raw ingestion. Root state transitions (`into_unnormalized`, `into_state`, consuming `normalize`) require the new `DataOwned` storage marker (carried by `Backend::OwnedBuf`), so a mutable borrow of a typed owner can no longer change label and leave the owner stale; the arena view wrappers (`VecZnxViewMut`, `GLWEViewMut`) keep their transitions as authoritative scratch roots. Backend kernels write through the sealed unsafe capability trait `poulpy_hal::oep::KernelWordsMut` (`unsafe fn kernel_words_mut`, wrapped safely as `poulpy_cpu_ref::reference::kernel_words_mut` and, for test harnesses, `poulpy_hal::test_suite::harness_words_mut`); storage transfer/view plumbing uses the documented `VecZnx::transfer_data_mut`. The inventoried §6.4-B call sites that relabel a borrowed view for a carry write and re-normalize in place go through the named transitional bridge traits `BorrowedCarryView` (and the always-sound `WeakenBackendRef` for read-only use), implemented for `VecZnx` and `GLWE`; because these relabels never name a backend, the trait methods need no turbofish, and every call is counted by the deny-list ratchet and scheduled for removal by the PR 5 scratch-transaction migration. Four compile-fail doctests in `poulpy-hal` pin the guarantees: no `at_mut`/`data_mut` on a normalized root, no relabel of a mutable borrow, no weakening into a normalization proof.
- **Breaking:** `VecZnx`'s storage field is now private (spec PR 2). All external access goes through the named accessors: `DataView::data`, `DataViewMut::data_mut`, and the new consuming `VecZnx::into_data` (the state label dies with the value; re-wrapping goes back through the `from_data` trust boundary). The 20 direct field-access sites across `poulpy-core` layouts (LWE/plaintext storage re-shaping), `poulpy-bin-fhe` LUT/blind-rotation transfer helpers, `poulpy-ckks`, `poulpy-cpu-ref` raw-pointer kernels, and the Rayon normalize splitter migrated to the accessors. This funnels every raw-storage touch through nameable methods so the next step can gate mutable access by state and hand backend kernels a sealed capability instead.
- **Breaking:** close the remaining safe raw-mutation and state-forwarding constructors on `VecZnx` (spec PR 2): `as_scalar_znx_mut` is deleted (it had no callers; `as_scalar_znx_ref` stays), and `from_data_like`/`map_data_mut` move from safe public inherent methods to the backend-implementor extension-point trait `poulpy_hal::oep::ReborrowData`, whose contract restricts them to same-digit reborrows inside backend kernels (delegating-backend view casts, host views). Scheme code can no longer wrap arbitrary storage under an inherited normalization label.
- **Breaking:** `VecZnx` gains a type-level normalization state, `VecZnx<D, W, S: NormalizationState = Normalized>`, with sealed markers `Normalized` and `Unnormalized` and the `FitsIn<S>` relation between them. Carry-producing ops (`add`, `sub`, `add_scalar`, `mul_xp_minus_one`, `lsh_add`/`rsh_add`, `add_normal`, ...) only accept `Unnormalized` destinations, bound-preserving ops (`copy`, `negate`, `rotate`, `automorphism`, `switch_ring`, ...) map an input state into any state it fits in, normalizing ops (`normalize`, `big_normalize`, `lsh`, `rsh`) accept any destination, and DFT-domain inputs (`vec_znx_dft_apply`, VMP, SVP, convolution) require `Normalized`. Feeding un-normalized digits to the DFT pipeline is now a compile error. `VecZnxToBackendRef`/`VecZnxToBackendMut` carry the state through a `State` associated type; `into_unnormalized` is the free relabel and `normalize(self, ..)` the only way back: there is no `assume_normalized` or any other API that turns an `Unnormalized` value into a `Normalized` one without a normalization pass (the relabel behind `normalize` is crate-private, the state field is private, and `from_data` plus every allocator produce `Normalized` values). `VecZnx::from_data` is accordingly pinned to `Normalized` (raw ingestion is a trust boundary, like deserialization); the new state-preserving `VecZnx::from_data_like` and `VecZnx::map_data_mut` re-wrap an existing value's storage without touching its label. For backend implementors only, the `poulpy_hal::oep::SetNormalizationState` trait (safe `set_unnormalized()`, unsafe `set_normalized()`) sets the state directly, `set_normalized` skipping the pass, so a fused kernel that guarantees the digit bound by construction can bypass normalization before a DFT; it is implemented for every state-carrying container (`VecZnx`, `VecZnxViewMut`, and `GLWE`/`CKKSCiphertext` through their crates' `oep` modules) and must never appear in scheme code. A scratch destination that will accumulate carries is `take_vec_znx_scratch(..)` followed by `into_unnormalized()`.

### `poulpy-core`

- **Breaking:** `GLWE<D, W, S = Normalized>` propagates the HAL normalization state; `GLWEToBackendRef`/`GLWEToBackendMut` expose it as `State`. `glwe_add`/`glwe_sub`/`glwe_mul_xp_minus_one`/`glwe_lsh_add`/`glwe_lsh_sub` require `State = Unnormalized` destinations, keyswitching, external products, automorphisms, convolutions, tensoring and decryption require `State = Normalized` inputs, and encryption produces `Normalized` ciphertexts. `GLWE::into_unnormalized` (also on `take_glwe_scratch` views) and `GLWE::normalize` cover the fusion pattern. `GLWE::normalize` (and the `GLWEViewMut`/`CKKSCiphertext` variants) now delegate to `VecZnx::normalize` and bound on `VecZnxNormalizeAssignBackend<BE>` instead of `GLWENormalize<BE>`, so the state relabel exists only inside `poulpy-hal`; `GLWE::assume_normalized` does not exist; the backend-implementor bypass is the `poulpy_hal::oep::SetNormalizationState` trait, implemented for `GLWE`/`GLWEViewMut` in `poulpy_core::oep` (unsafe, reserved for fused kernels inside backend crates).
...
- The CMUX family and `FheUint` byte splicing normalize their fused differences before the next external product.
