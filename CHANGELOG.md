...
- Since the backend is no longer recoverable from a DFT/big-family value's type, `to_backend_ref()` / `to_backend_mut()` cannot infer it; all six types gain inherent `x.to_backend_ref()` / `x.to_backend_mut()` for owned buffers (delegating to the `*ToBackendRef/Mut` traits, which remain the bound vocabulary for generic code; scratch-view wrappers such as `VecZnxDftViewMut<'_, B>` still infer without a turbofish).
