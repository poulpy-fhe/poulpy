...
- **Breaking:** nine dead `api` methods are removed with their OEP methods, delegates and reference bodies: `glwe_mul_const_assign`, `glwe_sub_negate_assign`, the out-of-place `glwe_mul_xp_minus_one` (its `_assign` form stays), `glwe_encrypt_zero_pk`, `glwe_mul_plain_assign` (its bench runner goes with it), the traits `GLWESwitchingKeyEncryptPk` and `GLWEAutomorphismKeyEncryptPk`, which each held a `_tmp_bytes` and no operation, and the explicit-parameter `glwe_bytes_of` and `glwe_secret_bytes_of`, whose `_from_infos` forms are now the required methods of `GLWEBytesOf`. None had a caller outside the crate.
- **Breaking:** the per-row compositions leave the `api`, with their OEP traits, `impl_*_reference_full!` macros, reference bodies and noise tests: `GGLWEKeyswitch`, `GGSWKeyswitch`, `GGLWEExternalProduct`, `GGSWExternalProduct`, `GGSWRotate` and `GGSWFromGGLWE` (seventeen methods). Each reference body was a loop of the GLWE operation over the rows, the GGSW forms followed by `ggsw_expand_row`, which stays; a caller writes that loop. `ConversionImpl` loses `ggsw_from_gglwe`, `test_gglwe_keyswitch_parity` goes with `gglwe_keyswitch`. None had a caller outside the crate. `GGSWAutomorphism` stays: a GGSW of `X^k` under the Galois element `-1` is a GGSW of `X^{-k}`, the inverse rotation.
- **Breaking:** `glwe_trace_galois_elements` and `glwe_pack_galois_elements` are removed; the one free function `poulpy_core::trace_galois_elements(log_n, cyclotomic_order)` answers both (it was already the body of each, and `poulpy-bin-fhe` carried a third copy). The mask-fill traits `GLWEMaskFill` and `LWEFillMask` leave the `api`: the encryptions fill masks through the OEP `EncryptionImpl` methods, nothing called the public forms.
- **Breaking:** the trace is in place only: the out-of-place `glwe_trace` is removed (its reference body was `glwe_copy` into a temporary, `glwe_trace_assign`, `glwe_copy` out), and `glwe_trace_tmp_bytes(res_infos, key_infos)` sizes `glwe_trace_assign`; a caller that traces into another ciphertext copies first and adds `glwe_copy_tmp_bytes`. `glwe_pack` and the `poulpy-bin-fhe` circuit bootstrapping do so.
