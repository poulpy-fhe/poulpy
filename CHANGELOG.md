...
- **Breaking:** every fallible op-trait method returns `CKKSResult<T>` instead of `anyhow::Result<T>`. `CKKSError` is `#[non_exhaustive]` with `Composition` / `Internal` variants and a `From<anyhow::Error>` bridge; `CKKSCompositionError` gains `InvalidPlan` and `PreparedOperandLayoutMismatch`.
- **Breaking:** api naming sweep onto the `CKKS*Ops` convention: `CKKSEncrypt` → `CKKSEncryptOps`, `CKKSDecrypt` → `CKKSDecryptOps`, `DFTOps` → `CKKSDFTOps`.
- **Breaking:** `ckks_encrypt_sk` and `BootstrappingContext::generate_keys` take the randomness sources as `(source_xe, source_xa)`, a positional swap the compiler cannot catch.
- **Breaking:** `ckks_decrypt` decrypts into the destination plaintext's preset frame instead of stamping the ciphertext's metadata onto it; preset `pt.set_meta(ct.meta())` for the old behavior.
- **Breaking:** the CKKS multiplications produce the result at the destination's requested `dst.k()`, rounding the low bits, instead of filling to `dst.max_k()`. Allocate the destination at exactly the `k` you want.
- **Breaking:** `DFTPlan` is constructor-validated: private fields, `DFTPlan::new` + `with_scaling` + `with_bit_reversed`, the new `FactorSchedule` of `FactorStep`, `meta` renamed `coeffs_meta` and carrying the lightweight `CoeffsMeta`, `check` removed, and infallible schedule queries.
- **Breaking:** `BootstrappingPlan` is a constructor-validated recipe requiring the pipeline and explicit techniques. Sparse-secret encapsulation is recipe-owned and `EncapsulationKeysLayout` carries only the two switching-key shapes.
- Add S2C-first (slim) bootstrapping as `BootstrappingPipeline::S2CFirst`, with `BootstrappingPlan::{pre_mod_up_consumed_bits, post_mod_up_consumed_bits, input_k, bootstrap_k}` placing the split costs and `ckks_bootstrap_real` skipping the imaginary branch.
- Add functional bootstrapping (`ckks_functional_bootstrap`, the shared-power-basis `ckks_functional_bootstrap_multi`, and `ckks_functional_bootstrap_real`) on the S2C-first recipe, replacing EvalMod with encoded general or binary LUT evaluation. LUT arity determines `log_msg_ratio`, and `functional_bootstrap_k` accounts for both LUT kinds.
- Support EvalRound+ with S2C-first bootstrapping.
- Bootstrapping, eval_mod, and the linear-transformation wrappers carve every working ciphertext from scratch instead of heap-allocating; `ckks_bootstrap_tmp_bytes` and `ckks_eval_mod_tmp_bytes` charge them.
- Generalize DFT-matrix and eval-mod table generation over the scalar via the new `DftScalar` trait, so `Quad` matrices are binary128-exact instead of f64-rounded. `Quad` is now `bytemuck::Pod`.
- Make `Quad`'s `Float` surface portable where no native binary128 libm exists, fixing Intel macOS builds without changing the public type or ABI; `libquadmath` remains the accelerated backing on x86_64 Linux GNU.
- **Breaking:** the `crate::leveled::*` re-export shim is removed.
- **Breaking:** the `test_suite` module is gated behind the new `test-utils` feature, and the dead `enable-ckks` / `enable-avx` / `enable-neon` flags are removed from `poulpy-ckks`.
- The bootstrapping conformance test asserts a measured precision floor on every configuration; the retired host reim encoder survives as the test-suite reference oracle.
- **Breaking:** CKKS ciphertexts no longer compact (the `Compact` / `SetSize` impls and bounds are removed), and the effective-`k` width rule is uniform across the ciphertext-side API: tensor intermediates are carved at `max(a.k, b.k)`, alignment offsets are computed against `dst.k()`, and `ckks_mod_up_into` widens to `dst.k()`. Plaintext operands read `IntPolyInfos::encoded_k()` instead, since masking an integer polynomial at its `k` would destroy data limbs.
- Fixed, uncovered by the width-model collapse: the tensor intermediate was sized from the physical width while its `cnv_offset` used effective `k`; `validate_backend_storage_capacity` compared the key's stored width against its backend view's allocation; and the add/sub/copy/rotate/pow2 families aligned to the destination's allocation while the mul family used `dst.k()`. Every stage's precision is bit-identical to the compacting implementation.
