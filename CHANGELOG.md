...
- `poulpy-cpu-arm`: the NEON ntt4x30 modules (`ntt4x30_convert`, `ntt4x30_ntt`) are repaired — they are gated on `enable-neon` **and** `target_arch = "aarch64"` and had silently rotted out of every check lane; the aarch64 + `enable-neon` check/clippy lane is now documented in `ci-local.md`.

### `poulpy-core`

- The prepared layouts (`GLWEPrepared`, `GGLWEPrepared`, `GGSWPrepared`, `GLWESecretPrepared`, `PreparedDiagonal`, …) follow the HAL re-key transparently — inner buffers are keyed by `(B::DftWord, B)` — and their derived `Eq` is dropped (`PartialEq` retained, and now live via `Backend: PartialEq`), since `f64`-word backends cannot satisfy `Eq`.
