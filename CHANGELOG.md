...
- Add `layouts::word` with the word traits `ZnxWord` (coefficient domain; `i64`, carrying `const BITS`), `BigWord` (accumulator domain; `i64`, `i128`) and `DftWord` (prepared/DFT domain; `f64`, backend CRT blocks). A word type names a **complete byte-layout convention** (element packing, lane count, prime set, ordering, reduction form): type equality means buffer interchangeability, a backend deviating in any aspect must mint a new word type, and `Backend::bytes_of_*` overrides remain authoritative for total buffer sizes (a word is a sizing + identity contract, not necessarily an element view — e.g. `Q126Scalar` over planar storage).
...
- All nine CPU backends (and `HostBytesBackend`) declare `type ZnxWord = i64` alongside the renamed `DftWord` / `BigWord`.
