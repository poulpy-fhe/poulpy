...
- The serialization round trips of the `poulpy-hal`, `poulpy-core` and `poulpy-bin-fhe` layouts are backend test bodies (`test_suite::serialization`), sampled through the module at 50-bit digits and run as backend tests from `poulpy-cpu-ref`; the host-only tests they replace are gone.
