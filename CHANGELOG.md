...
- **Breaking:** close the remaining safe raw-mutation and state-forwarding constructors on `VecZnx` (spec PR 2): `as_scalar_znx_mut` is deleted (it had no callers; `as_scalar_znx_ref` stays), and `from_data_like`/`map_data_mut` move from safe public inherent methods to the backend-implementor extension points `poulpy_hal::oep::vec_znx_from_data_like`/`vec_znx_map_data_mut`, whose contract restricts them to same-digit reborrows inside backend kernels (delegating-backend view casts, host views). Scheme code can no longer wrap arbitrary storage under an inherited normalization label.
