...
- Every api trait carries a structured contract beside its prose: `op`, `class` (basis / variant / derived / support), `mutation` (out-of-place / in-place / accumulate / none), `domain`, `ensures`, `exact` and `test` on every operation, plus `definition`, `requires`, `fallback`, `override` and `sparse` where they say something. The `api` module docs define the vocabulary (value model, canonical form, limb rule, mutation classes, exactness classes, the degree-embedding paragraph) and the `layouts` module docs the value model and the windows; the `sparse` line records, per operation, which operand slots will take a degree other than the module's ([#266](https://github.com/poulpy-fhe/poulpy/issues/266)). A test reads `src/api/*.rs`, holds every `pub trait` to a block, checks that its class brings the lines it needs and resolves every name on the `test` line against `src/test_suite`.
- Sparse operands ([#266](https://github.com/poulpy-fhe/poulpy/issues/266)): the slots a contract's `sparse` line names accept a degree-`n` operand, `n` a power-of-two divisor of the module degree `N`, which stands for its ring embedding `switch_ring_{n->N}`. The coefficient-domain `add` / `sub` families of `VecZnx` and `VecZnxBig`, including the small-operand Big forms, read such an operand with a stride. In the convolution the right operand is the sparse-capable slot: `cnv_prepare_right` takes a degree-`n` input into a degree-`n` `CnvPVecR` allocated with `CnvPVecR::alloc(n, ..)`, and `cnv_apply_dft`, `cnv_apply_dft_add`, `cnv_apply_dft_sum` and `cnv_pairwise_apply_dft` read it there; the result and the left operand take the module degree, so `cnv_prepare_left` and `cnv_prepare_self` reject a degree-`n` input. Nothing is expanded: the compact operand is read through the backend's slot correspondence, which on every in-tree transform is `slot_N(i) = slot_n(i >> log2(N/n))`. `cnv_by_const_apply` keeps dense operands (a degree-`n` `a` is embedded by the caller). Pinned by `test_vec_znx_sparse_add_sub`, `test_vec_znx_big_sparse_add_sub` and `test_convolution_sparse` on every backend, each against the dense operation on the `switch_ring` embedding.
