...
- Add `GLWEAutomorphismKeyHelper` / `GLWERelinearizationKeyHelper` and their layout twins: a caller names a function and its exact precision, the helper answers with a physical key and the effective `dsize`. Implemented by `GGLWEKeyRegistry` and by `GGLWESingleKey`.
