...
- `Backend::MIN_SPARSE_DEGREE` (default 8) names the smallest degree a sparse operand may have under a backend; the kernels assert it through `sparse_log_gap(module_n, n, floor)` and the sparse test suite sweeps down to it.
...
- Compact plaintexts ([#266](https://github.com/poulpy-fhe/poulpy/issues/266)): `CKKSModuleAlloc::ckks_pt_vec_alloc_compact(slots, base2k, k)` allocates a plaintext at degree `max(2 * slots, MIN_SPARSE_DEGREE)`, capped at the ring degree, and the encoders write it with gap 1; its value is the same `M(X^(N/n))` the dense sparse plaintext held. `ckks_add_pt_vec` / `sub_pt_vec` take a compact plaintext through the sparse-capable slot of the shift-add family, no expansion; the linear-transformation diagonal encoder emits compact diagonals for every sparse matrix, so the DFT and PaCo transformations prepare and hold diagonals `N/n` times smaller and run the sparse convolution sweep. `ckks_mul_pt_vec` / `_assign` return an error on a compact plaintext (they prepare per call under the ring's module); a plaintext whose degree is not a power-of-two divisor of the ring's from the backend floor is an error on `add_pt_vec` / `sub_pt_vec`. Pinned by `compact_plaintext_embedding`, `compact_plaintext_add_sub`, `compact_linear_transformation` (prepared and streamed) and `compact_diagonal_encoder` on every backend.
