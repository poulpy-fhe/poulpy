...
- **Breaking, behaviour:** on the CPU backends the dedicated shift kernels are gone; `lsh`/`rsh` and their accumulating/assigning forms run through `vec_znx_normalize` with an offset of `±k`. A shift whose destination is shorter than its source now rounds the discarded tail jointly (one unit in the last limb can differ from before), and `rsh` by more than the destination precision now yields zero (it previously returned `a >> (k mod base2k)`).
