//! Single-command sanity + perf check for `poulpy-cpu-avx` vs `poulpy-cpu-portable`.
//!
//! TEMPORARY — fold into proper `poulpy-bench` criterion benches once the
//! AVX backend is fully validated.
//!
//! Usage (native x86_64 with AVX2 + FMA):
//!
//!   RUSTFLAGS="-C target-feature=+avx2,+fma" \
//!     cargo run --release --example bench_avx_vs_ref \
//!     -p poulpy-cpu-avx --features enable-avx
//!
//! Output:
//!   - stdout (live progress)
//!   - file `bench_avx_vs_ref.txt` in the current working directory
//!     (overwritten on each run). Override the path with the
//!     `POULPY_AVX_BENCH_OUT` environment variable.
//!
//! Each kernel is run once for correctness (AVX output vs `_ref` output,
...
mod avx {
    ...
    const SEED: [u8; 32] = *b"poulpy-cpu-avx-bench-seed--01234";
    const SIZES: &[usize] = &[1024, 4096, 16384];
    const DEFAULT_OUT: &str = "bench_avx_vs_ref.txt";
    ...
    fn row(rep: &mut Reporter, name: &str, n: usize, ref_ns: u128, avx_ns: u128, speedups: &mut Vec<f64>) {
        let s = ref_ns as f64 / avx_ns.max(1) as f64;
        speedups.push(s);
        rep.line(&format!(
            "  {:36} n={:>6}   ref={:>9} ns   avx={:>9} ns   {:5.2}×",
            name, n, ref_ns, avx_ns, s
        ));
    }
    ...
    fn bench_znx_add(rep: &mut Reporter, n: usize, sp: &mut Vec<f64>) {
        ...
        <FFT64Avx as ZnxAdd>::znx_add(&mut avx, &a, &b);
        ...
        let avx_ns = time(iters, || {
            <FFT64Avx as ZnxAdd>::znx_add(&mut r, &a, &b);
            black_box(&r);
        });
        row(rep, "znx_add", n, ref_ns, avx_ns, sp);
    }
    ...
    fn bench_znx_normalize(rep: &mut Reporter, n: usize, sp: &mut Vec<f64>) {
        ...
        <FFT64Avx as ZnxNormalizeMiddleStepAssign>::znx_normalize_middle_step_assign(base2k, 0, &mut x_avx, &mut c_avx);
        ...
        let avx_ns = time(iters, || {
            x.copy_from_slice(&x_init);
            c.copy_from_slice(&c_init);
            <FFT64Avx as ZnxNormalizeMiddleStepAssign>::znx_normalize_middle_step_assign(base2k, 0, &mut x, &mut c);
            black_box((&x, &c));
        });
        row(rep, "znx_normalize_middle_step_assign", n, ref_ns, avx_ns, sp);
    }
    ...
    fn bench_znx_automorphism(rep: &mut Reporter, n: usize, sp: &mut Vec<f64>) {
        ...
        let mut avx = vec![0i64; n];
        let mut refr = vec![0i64; n];
        <FFT64Avx as ZnxAutomorphism>::znx_automorphism(p, &mut avx, &a);
        ...
        let avx_ns = time(iters, || {
            <FFT64Avx as ZnxAutomorphism>::znx_automorphism(p, &mut r, &a);
            black_box(&r);
        });
        row(rep, "znx_automorphism", n, ref_ns, avx_ns, sp);
    }
    ...
    fn bench_reim_add(rep: &mut Reporter, n: usize, sp: &mut Vec<f64>) {
        ...
        <FFT64Avx as ReimArith>::reim_add(&mut avx, &a, &b);
        ...
        let avx_ns = time(iters, || {
            <FFT64Avx as ReimArith>::reim_add(&mut r, &a, &b);
            black_box(&r);
        });
        row(rep, "reim_add", n, ref_ns, avx_ns, sp);
    }
    ...
    fn bench_reim_mul(rep: &mut Reporter, n: usize, sp: &mut Vec<f64>) {
        ...
        let mut avx = vec![0f64; n];
        let mut refr = vec![0f64; n];
        <FFT64Avx as ReimArith>::reim_mul(&mut avx, &a, &b);
        ...
        let avx_ns = time(iters, || {
            <FFT64Avx as ReimArith>::reim_mul(&mut r, &a, &b);
            black_box(&r);
        });
        row(rep, "reim_mul", n, ref_ns, avx_ns, sp);
    }
    ...
    fn bench_reim_addmul(rep: &mut Reporter, n: usize, sp: &mut Vec<f64>) {
        ...
        <FFT64Avx as ReimArith>::reim_addmul(&mut avx, &a, &b);
        ...
        let avx_ns = time(iters, || {
            r.copy_from_slice(&r0);
            <FFT64Avx as ReimArith>::reim_addmul(&mut r, &a, &b);
            black_box(&r);
        });
        row(rep, "reim_addmul", n, ref_ns, avx_ns, sp);
    }
    ...
    fn bench_fft(rep: &mut Reporter, n: usize, sp: &mut Vec<f64>) {
        ...
        <FFT64Avx as ReimFFTExecute<ReimFFTTable<f64>, f64>>::reim_dft_execute(&table, &mut avx);
        ...
        let avx_ns = time(iters, || {
            d.copy_from_slice(&data0);
            <FFT64Avx as ReimFFTExecute<ReimFFTTable<f64>, f64>>::reim_dft_execute(&table, &mut d);
            black_box(&d);
        });
        row(rep, "fft", n, ref_ns, avx_ns, sp);
    }
    ...
    fn bench_ifft(rep: &mut Reporter, n: usize, sp: &mut Vec<f64>) {
        ...
        let mut avx = data0.clone();
        let mut refr = data0.clone();
        <FFT64Avx as ReimFFTExecute<ReimIFFTTable<f64>, f64>>::reim_dft_execute(&table, &mut avx);
        ...
        let avx_ns = time(iters, || {
            d.copy_from_slice(&data0);
            <FFT64Avx as ReimFFTExecute<ReimIFFTTable<f64>, f64>>::reim_dft_execute(&table, &mut d);
            black_box(&d);
        });
        row(rep, "ifft", n, ref_ns, avx_ns, sp);
    }
    ...
    fn bench_ntt_from_znx64(rep: &mut Reporter, n: usize, sp: &mut Vec<f64>) {
        ...
        <NTT4x30Avx as NttFromZnx64>::ntt_from_znx64(&mut avx, &a);
        ...
        let avx_ns = time(iters, || {
            <NTT4x30Avx as NttFromZnx64>::ntt_from_znx64(&mut r, &a);
            black_box(&r);
        });
        row(rep, "ntt_from_znx64", n, ref_ns, avx_ns, sp);
    }
    ...
    fn bench_ntt(rep: &mut Reporter, n: usize, sp: &mut Vec<f64>) {
        ...
        let mut avx = data0.clone();
        let mut refr = data0.clone();
        <NTT4x30Avx as NttDFTExecute<NttTable<Primes30>>>::ntt_dft_execute(&table, &mut avx);
        ...
        let avx_ns = time(iters, || {
            d.copy_from_slice(&data0);
            <NTT4x30Avx as NttDFTExecute<NttTable<Primes30>>>::ntt_dft_execute(&table, &mut d);
            black_box(&d);
        });
        row(rep, "ntt", n, ref_ns, avx_ns, sp);
    }
    ...
    fn bench_intt(rep: &mut Reporter, n: usize, sp: &mut Vec<f64>) {
        ...
        let mut avx = data0.clone();
        let mut refr = data0.clone();
        <NTT4x30Avx as NttDFTExecute<NttTableInv<Primes30>>>::ntt_dft_execute(&table, &mut avx);
        ...
        let avx_ns = time(iters, || {
            d.copy_from_slice(&data0);
            <NTT4x30Avx as NttDFTExecute<NttTableInv<Primes30>>>::ntt_dft_execute(&table, &mut d);
            black_box(&d);
        });
        row(rep, "intt", n, ref_ns, avx_ns, sp);
    }
    ...
    fn bench_i128_add(rep: &mut Reporter, n: usize, sp: &mut Vec<f64>) {
        ...
        <NTT4x30Avx as I128BigOps>::i128_add(&mut avx, &a, &b);
        ...
        let avx_ns = time(iters, || {
            <NTT4x30Avx as I128BigOps>::i128_add(&mut r, &a, &b);
            black_box(&r);
        });
        row(rep, "i128_add", n, ref_ns, avx_ns, sp);
    }
    ...
    fn bench_nfc_middle_step(rep: &mut Reporter, n: usize, sp: &mut Vec<f64>) {
        ...
        let mut c_ref = c_init.clone();
        <NTT4x30Avx as I128NormalizeOps>::nfc_middle_step(base2k, 0, &mut r_avx, &a, &mut c_avx);
        ...
        let avx_ns = time(iters, || {
            c.copy_from_slice(&c_init);
            <NTT4x30Avx as I128NormalizeOps>::nfc_middle_step(base2k, 0, &mut r, &a, &mut c);
            black_box((&r, &c));
        });
        row(rep, "nfc_middle_step (i128)", n, ref_ns, avx_ns, sp);
    }
    ...
    pub(super) fn run() {
        let out_path = std::env::var("POULPY_AVX_BENCH_OUT").unwrap_or_else(|_| DEFAULT_OUT.to_string());
        let mut rep = Reporter::new(&out_path);

        rep.line("== poulpy-cpu-avx: AVX2/FMA vs cpu-ref ==");
        rep.line(&format!(
