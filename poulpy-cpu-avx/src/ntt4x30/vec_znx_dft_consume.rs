//! AVX2 override for the NTT4x30 `vec_znx_idft_apply_consume` hot path.
//!
...
use poulpy_cpu_ref::reference::ntt4x30::{ntt::NttTableInv, primes::Primes30, vec_znx_dft::NttModuleHandle};
...
pub(crate) fn vec_znx_idft_apply_consume<D: Data>(
    ...
    mut a: VecZnxDft<D, NTT4x30Avx>,
) -> VecZnxBig<D, NTT4x30Avx>
where
    VecZnxDft<D, NTT4x30Avx>: VecZnxDftToBackendMut<NTT4x30Avx>,
{
    let table = module.get_intt_table();

    let (n, n_blocks, u64_ptr) = {
        let mut a_mut: VecZnxDft<&mut [u8], NTT4x30Avx> = a.to_backend_mut();
        let n = a_mut.n();
