...
#[target_feature(enable = "avx2")]
pub(crate) unsafe fn vec_mat1col_product_blkpair_bbc_pm_x2_avx2(
    meta: &BbcMeta<Primes30>,
    ell: usize,
    res0: &mut [u64],
    res1: &mut [u64],
    ...
    unsafe {
        ...
        for p in 0..4usize {
            ...
            for row in 0..ell {
                ...
                let y0 = _mm256_and_si256(y, mask32);
                let y1 = _mm256_srli_epi64::<32>(y);

                for (x, lo, hi) in [(x0, &mut lo0, &mut hi0), (x1, &mut lo1, &mut hi1)] {
                    let xl = _mm256_and_si256(x, mask32);
                    let xh = _mm256_srli_epi64::<32>(x);
                    let prod_lo = _mm256_mul_epu32(xl, y0);
                    let prod_hi = _mm256_mul_epu32(xh, y1);
                    *lo = _mm256_add_epi64(*lo, _mm256_and_si256(prod_lo, mask32));
                    *lo = _mm256_add_epi64(*lo, _mm256_and_si256(prod_hi, mask32));
                    *hi = _mm256_add_epi64(*hi, _mm256_srli_epi64::<32>(prod_lo));
                    *hi = _mm256_add_epi64(*hi, _mm256_srli_epi64::<32>(prod_hi));
                }
            }

            let out0 = reduce_bbc(lo0, hi0, mask_h2, meta.h, s2l_pow_red, s2h_pow_red);
            let out1 = reduce_bbc(lo1, hi1, mask_h2, meta.h, s2l_pow_red, s2h_pow_red);
