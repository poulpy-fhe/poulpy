...
fn test_vec_znx_big_normalize_input_bound_integer() {
    ...
    for a_base2k in 1..=62 {
        for res_base2k in 1..=62 {
            for a_size in 0..=8 {
                let mut input = VecZnxBig::<Vec<u8>, i128, NTT4x30Oracle>::from_data(vec![0; 16 * N * a_size], N, 1, a_size);
                for j in 0..a_size {
                    for i in 0..N {
                        random ^= random << 17;
                        random ^= random >> 29;
                ...
                for res_size in 1..=8 {
                    ...
                    for offset in [
                        ...
                        ntt4x30_vec_znx_big_normalize::<_, _, NTT4x30Oracle>(
                            &mut output,
                            res_base2k,
                            res_size * res_base2k,
                            offset,
                        ...
                        vec_znx_normalize::<FFT64Oracle>(
                            &mut <VecZnx<Vec<u8>, i64> as VecZnxToBackendMut<FFT64Oracle>>::to_backend_mut(&mut small_output),
                            res_base2k,
                            res_size * res_base2k,
                            offset,
                            0,
                            &<VecZnx<Vec<u8>, i64> as VecZnxToBackendRef<FFT64Oracle>>::to_backend_ref(&small_input),
                            a_base2k,
                            0,
                            &mut [0i64; 3 * N],
                        );
...
fn test_normalize_exact_canonical_precision() {
    ...
    for &(values, a_base2k, res_base2k, res_k, offset, expected) in cases {
        ...
        let mut big = VecZnxBig::<Vec<u8>, i128, NTT4x30Oracle>::from_data(vec![0; 16 * values.len()], 1, 1, values.len());
        ...
        if a_base2k <= 62 && res_base2k <= 62 {
            vec_znx_normalize::<FFT64Oracle>(
                &mut <VecZnx<Vec<u8>, i64> as VecZnxToBackendMut<FFT64Oracle>>::to_backend_mut(&mut output),
                res_base2k,
                res_k,
                offset,
                0,
                &<VecZnx<Vec<u8>, i64> as VecZnxToBackendRef<FFT64Oracle>>::to_backend_ref(&small),
                a_base2k,
                0,
                &mut [0; 3],
            );
            ...
            vec_znx_normalize_assign::<FFT64Oracle>(
                res_base2k,
                res_k,
                0,
                &mut <VecZnx<Vec<u8>, i64> as VecZnxToBackendMut<FFT64Oracle>>::to_backend_mut(&mut small),
                0,
                &mut [0],
            );
...
fn test_vec_znx_big_normalize_assign_and_ranges() {
    ...
    let mut regression_input = VecZnxBig::<Vec<u8>, i128, NTT4x30Oracle>::from_data(vec![0; 16], 1, 1, 1);
    regression_input.at_mut(0, 0)[0] = 3;
    let mut regression_output = VecZnx::<Vec<u8>, i64>::from_data(vec![0; 24], 1, 1, 3);
    ntt4x30_vec_znx_big_normalize_assign::<SubOp, _, _, NTT4x30Oracle>(
    ...
    for a_base2k in [1, 2, 17, 50, 62] {
        for res_base2k in [1, 2, 19, 51, 62] {
            for offset in [i64::MIN, -400, -63, -1, 0, 1, 63, 400, i64::MAX] {
                let mut input = VecZnxBig::<Vec<u8>, i128, NTT4x30Oracle>::from_data(vec![0; 16 * N * 3], N, 1, 3);
                for j in 0..3 {
                    for i in 0..N {
                ...
                for (start, len) in [(0, 3), (3, 7), (10, 7)] {
                    ...
                    unsafe {
                        ntt4x30_vec_znx_big_normalize_range_raw::<_, NTT4x30Oracle>(
                            ptr,
                ...
                for sub in [false, true] {
                    ...
                    if sub {
                        ...
                        ntt4x30_vec_znx_big_normalize_assign::<AddOp, _, _, NTT4x30Oracle>(
                            &mut got, res_base2k, offset, 0, &input, a_base2k, 0, &mut carry,
                        );
                    }
                    for j in 0..3 {
    ...
    for base2k in 1..=63 {
        ...
        for lsh in 0..base2k {
            for offset in 0..a.len() {
                ...
                <NTT4x30Oracle as I128NormalizeOps>::nfc_middle_step(base2k, lsh, &mut res, &a, &mut carry);
...
fn test_i128_normalize_fused_reference() {
    ...
    for base2k in 1..=63 {
        for take in 1..=base2k {
            ...
            <NTT4x30Oracle as I128NormalizeOps>::nfc_middle_step_assign(base2k, 0, &mut want_res, &mut want_carry);
            <NTT4x30Oracle as I128NormalizeOps>::znx_extract_digit_addmul_normalize_i128::<false>(
...
fn test_vec_znx_big_normalize_wide_radices() {
    ...
    for a_base2k in [1, 63, 64, 65, 126, 127] {
        for res_base2k in [1, 17, 63, 64] {
            for size in 1..=3 {
                let mut input = VecZnxBig::<Vec<u8>, i128, NTT4x30Oracle>::from_data(vec![0; 32 * size], 2, 1, size);
                for j in 0..size {
                    input
                        .at_mut(0, j)
...
mod canonical_precision_tests {
    ...
    fn check(a: &[Vec<i128>], ka: usize, kr: usize, k: usize, size: usize, offset: i64, ranges: bool) {
        ...
        let mut wide = VecZnxBig::<Vec<u8>, i128, NTT4x30Oracle>::from_data(vec![0; 16 * N * 2 * a.len()], N, 2, a.len());
        ...
        if narrow {
            let src = <VecZnx<Vec<u8>, i64> as VecZnxToBackendRef<FFT64Oracle>>::to_backend_ref(&input);
            if ranges {
                let ptr = output.data_mut().as_mut_ptr().cast::<i64>();
                for (start, len) in [(0, 2), (2, 3), (5, 4)] {
                    unsafe {
                        vec_znx_normalize_range_raw::<FFT64Oracle>(
                            ptr,
        ...
        if ranges {
            ...
            ntt4x30_vec_znx_big_normalize::<_, _, NTT4x30Oracle>(&mut output, kr, k, offset, 1, &wide, ka, 1, &mut [73; 3 * N]);
        }
        verify("wide", &output);
        if narrow && ka == kr && size == a.len() && offset == 0 {
            if ranges {
                ...
                for (start, len) in [(0, 2), (2, 3), (5, 4)] {
                    unsafe {
                        vec_znx_normalize_assign_range_raw::<FFT64Oracle>(
                            ptr,
                            VecZnxShape::new(N, 2, size),
                            kr,
                            k,
                ...
                vec_znx_normalize_assign::<FFT64Oracle>(
                    ...
                    k,
                    0,
                    &mut <VecZnx<Vec<u8>, i64> as VecZnxToBackendMut<FFT64Oracle>>::to_backend_mut(&mut input),
                    1,
                    &mut [73; N],
                );
            }
