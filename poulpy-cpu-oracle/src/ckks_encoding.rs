...
mod tests {
    ...
    fn fft64_f64_reuses_ring_plan_family_at_every_dimension() {
        let module = Module::<FFT64Oracle>::new(32);
        roundtrip_all_dimensions::<FFT64Oracle, f64>(&module);
    }
    ...
    fn fft64_precision_families_coexist_in_one_module() {
        let module = Module::<FFT64Oracle>::new(32);
        roundtrip_all_dimensions::<FFT64Oracle, f64>(&module);
        roundtrip_all_dimensions::<FFT64Oracle, poulpy_ckks::Quad>(&module);
    }

    #[test]
    fn ntt_encoding_precisions_coexist_independently_of_ring_transform() {
        let module = Module::<NTT4x30Oracle>::new(32);
        roundtrip_all_dimensions::<NTT4x30Oracle, f64>(&module);
        roundtrip_all_dimensions::<NTT4x30Oracle, poulpy_ckks::Quad>(&module);
    }
    ...
    fn coefficient_only_encoding_accepts_scratch_plaintext_views() {
        ...
        let mut pt_scratch = ScratchOwned::<FFT64Oracle>::alloc(bytes);
        ...
        let mut values = CKKSEncodingBuffer::<Vec<u8>, f64>::from_host::<FFT64Oracle>(&values);
        module.ckks_encode_slots_assign_into(&mut pt, &mut values).unwrap();
        let coeffs = values.to_host::<FFT64Oracle>();

        let mut decoded = CKKSEncodingBuffer::<Vec<u8>, f64>::from_host::<FFT64Oracle>(&vec![0.0; 2 * slots]);
        module.ckks_decode_slots_into(&pt, &mut decoded).unwrap();
        let values = decoded.to_host::<FFT64Oracle>();
        let (got_re, got_im) = values.split_at(slots);
        assert_close(got_re, got_im, &re, &im);

        let mut coeff_scratch =
            ScratchOwned::<FFT64Oracle>::alloc(CKKSEncodingHostOps::<FFT64Oracle, f64>::ckks_reim_tmp_bytes(&module, slots));
    ...
    fn encoding_rejects_invalid_slot_shapes() {
        ...
        let three = [0.0; 3];
        let mut scratch =
            ScratchOwned::<FFT64Oracle>::alloc(CKKSEncodingHostOps::<FFT64Oracle, f64>::ckks_reim_tmp_bytes(&module, 8));
        let mut scratch = scratch.arena();
        ...
        let required = CKKSEncodingHostOps::<FFT64Oracle, f64>::ckks_reim_tmp_bytes(&module, 8);
        let mut undersized = ScratchOwned::<FFT64Oracle>::alloc(required - FFT64Oracle::SCRATCH_ALIGN);
        assert!(
            module
