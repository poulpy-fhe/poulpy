...
pub(crate) struct DelegatingFFT64Oracle;

impl poulpy_hal::execution::ScratchWorkers for DelegatingFFT64Oracle {}

poulpy_hal::impl_backend_from!(DelegatingFFT64Oracle, FFT64Oracle);

macro_rules! impl_forward_znx_trait {
    ($trait_name:ident, $method:ident($($arg:ident : $ty:ty),*)) => {
        impl $trait_name for DelegatingFFT64Oracle {
            #[inline(always)]
            fn $method($($arg : $ty),*) {
                <FFT64Oracle as $trait_name>::$method($($arg),*)
            }
        }
    };
}
    ...
    ($trait_name:ident, $method:ident($($arg:ident : $ty:ty),*)) => {
        impl $trait_name for DelegatingFFT64Oracle {
            ...
            fn $method<const OVERWRITE: bool>($($arg : $ty),*) {
                <FFT64Oracle as $trait_name>::$method::<OVERWRITE>($($arg),*)
            }
        }
    };
}
...
impl ZnxExtractDigitAddMul for DelegatingFFT64Oracle {
    #[inline(always)]
    fn znx_extract_digit_addmul(base2k: usize, lsh: usize, res: &mut [i64], src: &mut [i64]) {
        <FFT64Oracle as ZnxExtractDigitAddMul>::znx_extract_digit_addmul(base2k, lsh, res, src);
    }
}

impl crate::reference::normalization::I64NormalizeOps for DelegatingFFT64Oracle {}
...
fn test_normalization_kernels_delegating_fft64_ref() {
    crate::test_suite::normalization::test_normalization_kernels::<DelegatingFFT64Oracle>();
}

impl ReimFFTExecute<ReimFFTTable<f64>, f64> for DelegatingFFT64Oracle {
    #[inline(always)]
    fn reim_dft_execute(table: &ReimFFTTable<f64>, data: &mut [f64]) {
        <FFT64Oracle as ReimFFTExecute<ReimFFTTable<f64>, f64>>::reim_dft_execute(table, data)
    }
}

impl ReimFFTExecute<ReimIFFTTable<f64>, f64> for DelegatingFFT64Oracle {
    #[inline(always)]
    fn reim_dft_execute(table: &ReimIFFTTable<f64>, data: &mut [f64]) {
        <FFT64Oracle as ReimFFTExecute<ReimIFFTTable<f64>, f64>>::reim_dft_execute(table, data)
    }
}

impl ReimArith for DelegatingFFT64Oracle {}
impl Reim4BlkMatVec for DelegatingFFT64Oracle {}
impl Reim4Convolution for DelegatingFFT64Oracle {}
impl I64Ops for DelegatingFFT64Oracle {}
impl BigWordHadamardProduct for DelegatingFFT64Oracle {
    #[inline(always)]
...
unsafe impl HalVecZnxImpl for DelegatingFFT64Oracle {
    crate::hal_impl_vec_znx!();
}

unsafe impl HalModuleImpl for DelegatingFFT64Oracle {
    crate::hal_impl_module!(FFT64ModuleDefault);
}

unsafe impl HalVmpImpl for DelegatingFFT64Oracle {
    crate::hal_impl_vmp!(FFT64VmpDefault);
}

unsafe impl HalConvolutionImpl for DelegatingFFT64Oracle {
    crate::hal_impl_convolution!(FFT64ConvolutionDefault);
}

unsafe impl HalVecZnxBigImpl for DelegatingFFT64Oracle {
    crate::hal_impl_vec_znx_big!(FFT64VecZnxBigDefault);
}

unsafe impl HalSvpImpl for DelegatingFFT64Oracle {
    crate::hal_impl_svp!(FFT64SvpDefault);
}

unsafe impl HalVecZnxDftImpl for DelegatingFFT64Oracle {
