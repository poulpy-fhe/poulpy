...
- `VecZnxDft`: backend-specific prepared-domain representation of `VecZnx`. Its storage layout is backend-defined. The `with_limb_range_mut` helper can narrow a view only when the backend opts into `Backend::DFT_LIMBS_CONTIGUOUS`: each complete limb, including every column, must occupy a contiguous byte region, and adjacent limbs must have equal byte sizes. CPU backends satisfy this contract. Other layouts retain whole-object views, but must provide their own implementation for computations that need these narrowed views; unsupported narrowing panics before touching storage.
