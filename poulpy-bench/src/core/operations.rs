    ...
    Module<BE>: ModuleNew<BE> + GLWEAdd<BE> + GLWEMaskFill<BE> + ModuleCoreAlloc<OwnedBuf = BE::OwnedBuf, ZnxWord = i64>,
{
    ...
    module.fill_glwe_from_source(&mut res, &mut source);
    ...
    ScratchOwned<BE>: ScratchOwnedAlloc<BE> + ScratchOwnedBorrow<BE>,
{
    ...
    module.fill_glwe_from_source(&mut ct_out, &mut source);
    module.fill_glwe_from_source(&mut ct_in, &mut source);
    module.fill_glwe_from_source(&mut pt, &mut source);
    ...
    ScratchOwned<BE>: ScratchOwnedAlloc<BE> + ScratchOwnedBorrow<BE>,
{
    ...
    let mut pt: GLWEPlaintext<BE::OwnedBuf, i64> = module.glwe_plaintext_alloc_from_infos(&infos);
    module.fill_glwe_from_source(&mut ct, &mut source);
    module.fill_glwe_from_source(&mut pt, &mut source);
