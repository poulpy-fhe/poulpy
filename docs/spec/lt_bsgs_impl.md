...
- **Saving #2 (hoisting).** For the non-trivial `k`, the mask columns are DFT'd **once**
  ...
  Each rotation is then `VMP → IDFT → add body → normalize → canonicalize → automorphism`
...
- **Lazy DFT path** (the hot path) — taken when `res`, the PROD output, and the keys all
  ...
  through the whole loop; one IDFT + one normalize and canonicalization at the very end.
- **Fallback path** — taken on base mismatch. Still computes PROD in DFT, but normalizes
  and canonicalizes each giant contribution to SMALL and uses the public
  `glwe_automorphism`. It is *correct* but gives up savings 4–6; it exists only so
  mismatched bases don't break.
...
`glwe_normalize_big_into` (**saving #4 + #9, one rescale level**). The sub-limb
`cnv_offset_lo` shift PROD never applied is folded into this final normalize. The helper
then canonicalizes at `res.k()`; this is a no-op at limb-aligned precision and otherwise
adds one bounded precision-boundary round.

The incompatible-radix fallback normalizes and canonicalizes each giant contribution
before its ordinary automorphism, then canonicalizes the accumulated result. It retains
functional parity but not the lazy path's single precision-boundary round.
...
| #4 Lazy normalize across giants | DFT accumulator + single final normalize and canonicalization (lazy.rs) |
...
| #9 One rescale level | single rescaling `glwe_normalize_big_into` (lazy.rs) |
