...
- [Coefficient Typestate, Canonicality, and Buffer Provenance](spec/normalization_typestate.md) — the target owner/view/scratch model, independent normalization and canonical-padding guarantees, raw backend boundary, and staged migration plan for making both invariants enforceable by the safe API.
- [Normalization Migration Inventory](spec/normalization_inventory.md) — the PR 0 companion to the typestate specification: frozen normalization contracts, workspace-wide site inventory, baselines, and the deny-list ratchet.
