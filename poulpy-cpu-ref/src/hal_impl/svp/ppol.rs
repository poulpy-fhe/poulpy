//! `ppol` tier of the SVP backend implementation.
//!
//! Expanded inside a backend's `HalSvpPpolImpl` block.

/// Emits every `ppol`-tier method: the kernel forwarders, then the variants
/// derived from them.
#[macro_export]
macro_rules! hal_impl_svp_ppol {
    ($defaults:ident) => {
        ...
        $crate::__hal_impl_svp_ppol_derived!();
    };
...
#[doc(hidden)]
#[macro_export]
macro_rules! __hal_impl_svp_ppol_derived {
    () => {
        #[allow(clippy::too_many_arguments)]
        fn svp_apply_ppol_dft_to_small(
            ...
            Self::svp_apply_ppol_dft_to_dft(module, &mut tmp_dft.to_backend_mut(), 0, a, a_col, b, b_col);
        ...
        fn svp_apply_ppol_dft_to_big(
            ...
            Self::svp_apply_ppol_dft_to_dft(module, &mut tmp.to_backend_mut(), 0, a, a_col, b, b_col);
        ...
        fn svp_apply_ppol_small_to_small(
            ...
            let (mut tmp_dft, scratch_1) =
                poulpy_hal::api::ScratchArenaTakeBasic::take_vec_znx_dft_scratch(scratch.borrow(), module, 1, b_size);
            Self::svp_apply_ppol_small_to_dft(module, &mut tmp_dft.to_backend_mut(), 0, a, a_col, b, b_col);
            let (mut tmp_big, mut scratch_2) =
                poulpy_hal::api::ScratchArenaTakeBasic::take_vec_znx_big_scratch(scratch_1, module, 1, b_size);
            module.vec_znx_idft_apply_tmpa(&mut tmp_big.to_backend_mut(), 0, &mut tmp_dft.to_backend_mut(), 0);
            module.vec_znx_big_normalize(
        ...
        fn svp_apply_ppol_small_to_big(
            ...
            let (mut tmp, _) =
                poulpy_hal::api::ScratchArenaTakeBasic::take_vec_znx_dft_scratch(scratch.borrow(), module, 1, res_size);
            Self::svp_apply_ppol_small_to_dft(module, &mut tmp.to_backend_mut(), 0, a, a_col, b, b_col);
            module.vec_znx_idft_apply_tmpa(res, res_col, &mut tmp.to_backend_mut(), 0);
        }
    };
}
