...
fn delegating_backend_manual_family_matches_fft64_ref() {
    for squaring in [false, true] {
        assert_eq!(
            Module::<DelegatingFFT64Ref>::max_base2k(1 << 16, 32, 128, squaring),
            Module::<FFT64Ref>::max_base2k(1 << 16, 32, 128, squaring)
        );
    }
...
fn derived_ggsw_rotation_uses_selected_glwe_methods_and_scratch() {
    ...
    for row in 0..layout.dnum.as_usize() {
        for col in 0..layout.rank.as_usize() + 1 {
            ...
            reference.vec_znx_fill_uniform_source(
                17,
                entry.k().as_usize(),
                GLWEToBackendMut::<FFT64Ref>::to_backend_mut(&mut entry).data_mut(),
                0,
                &mut source,
            );
            reference.fill_glwe_mask_from_source(&mut entry, &mut source);
        }
    }
...
fn derived_trace_uses_selected_assign_and_its_larger_scratch_query() {
    ...
    let mut mask_source = Source::new([61; 32]);
    reference.vec_znx_fill_uniform_source(
        17,
        input.k().as_usize(),
        GLWEToBackendMut::<FFT64Ref>::to_backend_mut(&mut input).data_mut(),
        0,
        &mut mask_source,
    );
    reference.fill_glwe_mask_from_source(&mut input, &mut mask_source);
