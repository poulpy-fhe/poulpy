...
use crate::{
    ...
    layouts::{
        GGLWEInfos, GGLWEToBackendMut, GLWESecretLayout, GLWESwitchingKeyDegreesMut, LWEInfos, LWESecretToBackendRef, Rank,
    },
};
...
impl<BE: Backend> LWESwitchingKeyEncryptReference<BE> for Module<BE>
where
    Self: ModuleN + GLWESwitchingKeyEncryptSk<BE> + VecZnxAutomorphism<BE> + VecZnxCopy<BE> + VecZnxZero<BE>,
{
        ...
        A: GGLWEInfos,
    {
        ...
        let lvl_2: usize = self.glwe_secret_bytes_of_from_infos(&GLWESecretLayout {
            n: self.n().into(),
            rank: Rank(1),
        });
