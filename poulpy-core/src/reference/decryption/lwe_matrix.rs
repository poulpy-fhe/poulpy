...
where
    ...
    A: LWEMatrixInfos,
{
    module.bytes_of_vec_znx_big(infos.rows(), 1, infos.size())
        + module.bytes_of_vec_znx(infos.rows(), 1, infos.size())
    ...
    BE: Backend,
{
    ...
    let (mut tmp, scratch_1) = scratch.take_vec_znx_big_scratch(res.rows(), 1, res.size());
