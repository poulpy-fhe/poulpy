    ...
    ScratchOwned<BE>: ScratchOwnedAlloc<BE> + ScratchOwnedBorrow<BE>,
{
    ...
    for rank in 1_usize..3 {
        for dsize in 1..max_dsize + 1 {
            ...
            let mut pt_ggsw: ScalarZnx<BE::OwnedBuf, BE::ZnxWord> = module.scalar_znx_alloc(module.n(), 1);
            ...
            let noise = glwe_noise_checked(module, &glwe_out, &pt_out, &sk_prepared, &mut scratch.borrow())
                .std()
                .log2();
            println!(
...
#[allow(clippy::too_many_arguments)]
pub fn test_glwe_external_product_assign<BE: crate::test_suite::noise::TestBackend>(params: &TestParams, module: &Module<BE>)
where
    BE::OwnedBuf: poulpy_hal::layouts::HostDataMut,
    for<'a> BE::BufRef<'a>: poulpy_hal::layouts::HostDataRef,
    for<'a> BE::BufMut<'a>: poulpy_hal::layouts::HostDataMut,
    ...
    ScratchOwned<BE>: ScratchOwnedAlloc<BE> + ScratchOwnedBorrow<BE>,
{
    ...
    for rank in 1_usize..3 {
        for dsize in 1..max_dsize + 1 {
            ...
            let noise = glwe_noise_checked(module, &glwe_out, &pt_want, &sk_prepared, &mut scratch.borrow())
                .std()
                .log2();
            println!(
                "DBG ext_prod have={noise:.2} max={max_noise:.2} slack={:.2}",
