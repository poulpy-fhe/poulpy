...
{
    let first_baby_rot = rhs.baby_step_rotation(gs.first_baby_step_index());
    let sizing_diagonal_operand = gs.diagonal(first_baby_rot);
    let first_baby_operand = lhs.baby_step(first_baby_rot);
    ...
    for col in 0..cols {
        for (term_idx, &baby_step_idx) in gs.baby_step_indexes().iter().enumerate() {
            let baby_rot = rhs.baby_step_rotation(baby_step_idx);
