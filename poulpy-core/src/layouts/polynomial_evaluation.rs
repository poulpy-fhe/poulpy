    ...
    F: Float + FloatConst + FromPrimitive + Debug,
{
    ...
    pub fn decompose_bsgs_with<C>(
        ...
        let log_split = split_for_strategy(split_strategy, self.degree(), self.parity, self.basis);
        let split_leading = matches!(split_strategy, SplitStrategy::MinDepth);
        self.decompose_bsgs_inner(log_split, split_leading, split_strategy, encode)
    }
    ...
    pub fn decompose_bsgs_with_log_split<C>(
        ...
        // record `MinDepth` for the carried accounting metadata.
        self.decompose_bsgs_inner(log_split, false, SplitStrategy::MinDepth, encode)
    }
