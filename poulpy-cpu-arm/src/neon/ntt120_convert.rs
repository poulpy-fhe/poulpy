...
use poulpy_cpu_ref::reference::ntt120::primes::{PrimeSet, Primes30};
...
use super::ntt120_ntt::intt_neon;
...
pub(crate) fn vec_znx_idft_apply_consume<D: Data>(
    ...
    mut a: VecZnxDft<D, NTT120Neon>,
) -> VecZnxBig<D, NTT120Neon>
where
    VecZnxDft<D, NTT120Neon>: VecZnxDftToBackendMut<NTT120Neon>,
{
    let table = module.get_intt_table();
    let (n, n_blocks, u64_ptr) = {
        let mut a_mut: VecZnxDft<&mut [u8], NTT120Neon> = a.to_backend_mut();
        let n = a_mut.n();
