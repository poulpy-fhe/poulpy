...
use crate::{FFT64Neon, execution::RayonTaskExecutor};

const VMP_PARALLELISM: usize = 4;

poulpy_hal::impl_backend_from!(FFT64NeonRayon, FFT64Neon, RayonTaskExecutor);

fn base_module(module: &Module<FFT64NeonRayon>) -> &Module<FFT64Neon> {
    module.reinterpret()
}

fn base_dft_ref<'a>(a: &'a VecZnxDftBackendRef<'_, FFT64NeonRayon>) -> VecZnxDftBackendRef<'a, FFT64Neon> {
    VecZnxDft::from_data(&**a.data(), a.n(), a.cols(), a.size())
}

fn base_dft_mut<'a>(a: &'a mut VecZnxDftBackendMut<'_, FFT64NeonRayon>) -> VecZnxDftBackendMut<'a, FFT64Neon> {
    let (n, cols, size) = (a.n(), a.cols(), a.size());
    VecZnxDft::from_data(&mut **a.data_mut(), n, cols, size)
}

fn base_big_mut<'a>(a: &'a mut VecZnxBigBackendMut<'_, FFT64NeonRayon>) -> VecZnxBigBackendMut<'a, FFT64Neon> {
    let (n, cols, size) = (a.n(), a.cols(), a.size());
    VecZnxBig::from_data(&mut **a.data_mut(), n, cols, size)
}

fn base_big_ref<'a>(a: &'a VecZnxBigBackendRef<'_, FFT64NeonRayon>) -> VecZnxBigBackendRef<'a, FFT64Neon> {
    ...
    ($trait:ident, $method:ident) => {
        impl $trait for FFT64NeonRayon {
            ...
            fn $method(res: &mut [i64], a: &[i64]) {
                let Some(chunk) = parallel_chunk_len(res.len()) else {
                    return <FFT64Neon as $trait>::$method(res, a);
                };
                res.par_chunks_mut(chunk)
                    .zip(a.par_chunks(chunk))
                    .for_each(|(res, a)| <FFT64Neon as $trait>::$method(res, a));
            }
        }
    };
}
    ...
    ($trait:ident, $method:ident) => {
        impl $trait for FFT64NeonRayon {
            ...
            fn $method(res: &mut [i64]) {
                let Some(chunk) = parallel_chunk_len(res.len()) else {
                    return <FFT64Neon as $trait>::$method(res);
                };
                res.par_chunks_mut(chunk)
                    .for_each(|res| <FFT64Neon as $trait>::$method(res));
            }
        }
    };
}
    ...
    ($trait:ident, $method:ident) => {
        impl $trait for FFT64NeonRayon {
            ...
            fn $method(k: i64, res: &mut [i64], a: &[i64]) {
                let Some(chunk) = parallel_chunk_len(res.len()) else {
                    return <FFT64Neon as $trait>::$method(k, res, a);
                };
                res.par_chunks_mut(chunk)
                    .zip(a.par_chunks(chunk))
                    .for_each(|(res, a)| <FFT64Neon as $trait>::$method(k, res, a));
            }
        }
    };
}
    ...
    ($trait:ident, $method:ident($($arg:ident: $ty:ty),* $(,)?)) => {
        impl $trait for FFT64NeonRayon {
            ...
            fn $method<const OVERWRITE: bool>($($arg: $ty),*) {
                <FFT64Neon as $trait>::$method::<OVERWRITE>($($arg),*)
            }
        }
    };
}
...
impl ZnxMulPowerOfTwoAssign for FFT64NeonRayon {
    #[inline(always)]
    fn znx_mul_power_of_two_assign(k: i64, res: &mut [i64]) {
        let Some(chunk) = parallel_chunk_len(res.len()) else {
            return <FFT64Neon as ZnxMulPowerOfTwoAssign>::znx_mul_power_of_two_assign(k, res);
        };
        res.par_chunks_mut(chunk)
            .for_each(|res| <FFT64Neon as ZnxMulPowerOfTwoAssign>::znx_mul_power_of_two_assign(k, res));
    }
}
...
impl ReimFFTExecute<ReimFFTTable<f64>, f64> for FFT64NeonRayon {
    #[inline(always)]
    fn reim_dft_execute(table: &ReimFFTTable<f64>, data: &mut [f64]) {
        <FFT64Neon as ReimFFTExecute<ReimFFTTable<f64>, f64>>::reim_dft_execute(table, data)
    }
}

impl ReimFFTExecute<ReimIFFTTable<f64>, f64> for FFT64NeonRayon {
    #[inline(always)]
    fn reim_dft_execute(table: &ReimIFFTTable<f64>, data: &mut [f64]) {
        <FFT64Neon as ReimFFTExecute<ReimIFFTTable<f64>, f64>>::reim_dft_execute(table, data)
    }
}

impl ReimArith for FFT64NeonRayon {
    #[inline(always)]
    fn reim_from_znx(res: &mut [f64], a: &[i64]) {
        <FFT64Neon as ReimArith>::reim_from_znx(res, a)
    }
    #[inline(always)]
    fn reim_from_znx_masked(res: &mut [f64], a: &[i64], mask: i64) {
        <FFT64Neon as ReimArith>::reim_from_znx_masked(res, a, mask)
    }
    #[inline(always)]
    fn reim_to_znx(res: &mut [i64], divisor: f64, a: &[f64]) {
        <FFT64Neon as ReimArith>::reim_to_znx(res, divisor, a)
    }
    #[inline(always)]
    fn reim_to_znx_assign(res: &mut [f64], divisor: f64) {
        <FFT64Neon as ReimArith>::reim_to_znx_assign(res, divisor)
    }
    #[inline(always)]
    fn reim_add(res: &mut [f64], a: &[f64], b: &[f64]) {
        <FFT64Neon as ReimArith>::reim_add(res, a, b)
    }
    #[inline(always)]
    fn reim_add_assign(res: &mut [f64], a: &[f64]) {
        <FFT64Neon as ReimArith>::reim_add_assign(res, a)
    }
    #[inline(always)]
    fn reim_sub(res: &mut [f64], a: &[f64], b: &[f64]) {
        <FFT64Neon as ReimArith>::reim_sub(res, a, b)
    }
    #[inline(always)]
    fn reim_sub_assign(res: &mut [f64], a: &[f64]) {
        <FFT64Neon as ReimArith>::reim_sub_assign(res, a)
    }
    #[inline(always)]
    fn reim_sub_negate_assign(res: &mut [f64], a: &[f64]) {
        <FFT64Neon as ReimArith>::reim_sub_negate_assign(res, a)
    }
    #[inline(always)]
    fn reim_negate(res: &mut [f64], a: &[f64]) {
        <FFT64Neon as ReimArith>::reim_negate(res, a)
    }
    #[inline(always)]
    fn reim_negate_assign(res: &mut [f64]) {
        <FFT64Neon as ReimArith>::reim_negate_assign(res)
    }
    #[inline(always)]
    fn reim_mul(res: &mut [f64], a: &[f64], b: &[f64]) {
        <FFT64Neon as ReimArith>::reim_mul(res, a, b)
    }
    #[inline(always)]
    fn reim_mul_assign(res: &mut [f64], a: &[f64]) {
        <FFT64Neon as ReimArith>::reim_mul_assign(res, a)
    }
    #[inline(always)]
    fn reim_addmul(res: &mut [f64], a: &[f64], b: &[f64]) {
        <FFT64Neon as ReimArith>::reim_addmul(res, a, b)
    }
    #[inline(always)]
    fn reim_copy(res: &mut [f64], a: &[f64]) {
        <FFT64Neon as ReimArith>::reim_copy(res, a)
    }
    #[inline(always)]
    fn reim_zero(res: &mut [f64]) {
        <FFT64Neon as ReimArith>::reim_zero(res)
    }
}

impl Reim4BlkMatVec for FFT64NeonRayon {
    #[inline(always)]
    fn reim4_extract_1blk_contiguous(m: usize, rows: usize, blk: usize, dst: &mut [f64], src: &[f64]) {
        <FFT64Neon as Reim4BlkMatVec>::reim4_extract_1blk_contiguous(m, rows, blk, dst, src)
    }
    #[inline(always)]
    fn reim4_save_1blk_contiguous(m: usize, rows: usize, blk: usize, dst: &mut [f64], src: &[f64]) {
        <FFT64Neon as Reim4BlkMatVec>::reim4_save_1blk_contiguous(m, rows, blk, dst, src)
    }
    #[inline(always)]
    fn reim4_save_1blk<const OVERWRITE: bool>(m: usize, blk: usize, dst: &mut [f64], src: &[f64]) {
        <FFT64Neon as Reim4BlkMatVec>::reim4_save_1blk::<OVERWRITE>(m, blk, dst, src)
    }
    #[inline(always)]
    fn reim4_save_2blks<const OVERWRITE: bool>(m: usize, blk: usize, dst: &mut [f64], src: &[f64]) {
        <FFT64Neon as Reim4BlkMatVec>::reim4_save_2blks::<OVERWRITE>(m, blk, dst, src)
    }
    #[inline(always)]
    fn reim4_mat1col_prod(nrows: usize, dst: &mut [f64], u: &[f64], v: &[f64]) {
        <FFT64Neon as Reim4BlkMatVec>::reim4_mat1col_prod(nrows, dst, u, v)
    }
    #[inline(always)]
    fn reim4_mat2cols_prod(nrows: usize, dst: &mut [f64], u: &[f64], v: &[f64]) {
        <FFT64Neon as Reim4BlkMatVec>::reim4_mat2cols_prod(nrows, dst, u, v)
    }
    #[inline(always)]
    fn reim4_mat2cols_2ndcol_prod(nrows: usize, dst: &mut [f64], u: &[f64], v: &[f64]) {
        <FFT64Neon as Reim4BlkMatVec>::reim4_mat2cols_2ndcol_prod(nrows, dst, u, v)
    }
}
    ...
    RayonTaskExecutor::for_each_init(
        ...
        |tmp, block| {
            let (a, b, out) = if PAIRWISE {
                ...
                <FFT64Neon as ReimArith>::reim_add(
                    a,
            ...
            <FFT64Neon as Reim4Convolution>::reim4_convolution(out, min_size, offset, a, a_size, b, b_size);
            unsafe {
...
impl Reim4Convolution for FFT64NeonRayon {
    #[inline(always)]
    fn reim4_convolution_1coeff(k: usize, dst: &mut [f64; 8], a: &[f64], a_size: usize, b: &[f64], b_size: usize) {
        <FFT64Neon as Reim4Convolution>::reim4_convolution_1coeff(k, dst, a, a_size, b, b_size)
    }
    #[inline(always)]
    fn reim4_convolution_2coeffs(k: usize, dst: &mut [f64; 16], a: &[f64], a_size: usize, b: &[f64], b_size: usize) {
        <FFT64Neon as Reim4Convolution>::reim4_convolution_2coeffs(k, dst, a, a_size, b, b_size)
    }
    #[inline(always)]
    fn reim4_convolution(dst: &mut [f64], dst_size: usize, offset: usize, a: &[f64], a_size: usize, b: &[f64], b_size: usize) {
        <FFT64Neon as Reim4Convolution>::reim4_convolution(dst, dst_size, offset, a, a_size, b, b_size)
    }
    #[inline(always)]
    fn reim4_convolution_apply(
        ...
        if !RayonTaskExecutor::is_parallel() {
            return <FFT64Neon as Reim4Convolution>::reim4_convolution_apply(
    ...
    fn reim4_convolution_apply_accumulate(
        ...
        if !RayonTaskExecutor::is_parallel() {
            return <FFT64Neon as Reim4Convolution>::reim4_convolution_apply_accumulate(
                m, min_size, offset, dst, dst_stride, a, a_size, b, b_size, tmp,
            );
        }
    ...
    fn reim4_convolution_pairwise_apply(
        ...
        if !RayonTaskExecutor::is_parallel() {
            return <FFT64Neon as Reim4Convolution>::reim4_convolution_pairwise_apply(
    ...
    fn reim4_convolution_by_real_const_1coeff(k: usize, dst: &mut [f64; 8], a: &[f64], a_size: usize, b: &[f64]) {
        <FFT64Neon as Reim4Convolution>::reim4_convolution_by_real_const_1coeff(k, dst, a, a_size, b)
    }
    #[inline(always)]
    fn reim4_convolution_by_real_const_2coeffs(k: usize, dst: &mut [f64; 16], a: &[f64], a_size: usize, b: &[f64]) {
        <FFT64Neon as Reim4Convolution>::reim4_convolution_by_real_const_2coeffs(k, dst, a, a_size, b)
    }
}

impl I64Ops for FFT64NeonRayon {
    #[inline(always)]
    fn i64_hadamard_product(res: &mut [i64], a: &[i64], b: &[i64]) {
        <FFT64Neon as I64Ops>::i64_hadamard_product(res, a, b)
    }
    #[inline(always)]
    fn i64_extract_1blk_contiguous(n: usize, offset: usize, rows: usize, blk: usize, dst: &mut [i64], src: &[i64]) {
        <FFT64Neon as I64Ops>::i64_extract_1blk_contiguous(n, offset, rows, blk, dst, src)
    }
    #[inline(always)]
    fn i64_save_1blk_contiguous(n: usize, offset: usize, rows: usize, blk: usize, dst: &mut [i64], src: &[i64]) {
        <FFT64Neon as I64Ops>::i64_save_1blk_contiguous(n, offset, rows, blk, dst, src)
    }
    #[inline(always)]
    fn i64_convolution_by_const_1coeff(k: usize, dst: &mut [i64; 8], a: &[i64], a_size: usize, b: &[i64]) {
        <FFT64Neon as I64Ops>::i64_convolution_by_const_1coeff(k, dst, a, a_size, b)
    }
    #[inline(always)]
    fn i64_convolution_by_const_2coeffs(k: usize, dst: &mut [i64; 16], a: &[i64], a_size: usize, b: &[i64]) {
        <FFT64Neon as I64Ops>::i64_convolution_by_const_2coeffs(k, dst, a, a_size, b)
    }
}

impl BigWordHadamardProduct for FFT64NeonRayon {
    #[inline(always)]
    fn big_word_hadamard_product(res: &mut [i64], a: &[i64], b: &[i64]) {
        <FFT64Neon as BigWordHadamardProduct>::big_word_hadamard_product(res, a, b)
    }
}

unsafe impl HalVecZnxImpl<FFT64NeonRayon> for FFT64NeonRayon {
...
unsafe impl HalModuleImpl<FFT64NeonRayon> for FFT64NeonRayon {
    poulpy_cpu_ref::hal_impl_module!(FFT64ModuleDefault);
}
unsafe impl HalVmpImpl<FFT64NeonRayon> for FFT64NeonRayon {
    #[inline(always)]
    ...
    fn vmp_apply_dft_tmp_bytes(
        ...
        <FFT64Neon as HalVmpImpl<FFT64Neon>>::vmp_apply_dft_tmp_bytes(
            base_module(module),
            res_size,
            a_size,
            b_rows,
    ...
    fn vmp_apply_dft_to_dft(
        ...
        if RayonTaskExecutor::should_serialize_inner() {
            return <Self as FFT64VmpDefault<Self>>::vmp_apply_dft_to_dft_with_kernel_default::<FFT64Neon>(
                module,
                res,
                a,
                b,
    ...
    fn vmp_apply_dft_to_dft_accumulate(
        ...
        if RayonTaskExecutor::should_serialize_inner() {
            return <Self as FFT64VmpDefault<Self>>::vmp_apply_dft_to_dft_accumulate_with_kernel_default::<FFT64Neon>(
                module,
                res,
                a,
                b,
...
unsafe impl HalConvolutionImpl<FFT64NeonRayon> for FFT64NeonRayon {
    poulpy_cpu_ref::hal_impl_convolution!(FFT64ConvolutionDefault);
}
unsafe impl HalVecZnxBigImpl<FFT64NeonRayon> for FFT64NeonRayon {
    #[inline(always)]
    fn vec_znx_big_from_small_backend(
        ...
        <FFT64Neon as HalVecZnxBigImpl<FFT64Neon>>::vec_znx_big_from_small_backend(&mut base_big_mut(res), res_col, a, a_col)
    }
    ...
    fn vec_znx_big_add_normal_backend(
        ...
        <FFT64Neon as HalVecZnxBigImpl<FFT64Neon>>::vec_znx_big_add_normal_backend(
            base_module(module),
            res_base2k,
    ...
    fn vec_znx_big_add_into(
        ...
        <FFT64Neon as HalVecZnxBigImpl<FFT64Neon>>::vec_znx_big_add_into(
            base_module(module),
            &mut base_big_mut(res),
            res_col,
            &base_big_ref(a),
    ...
    fn vec_znx_big_add_assign(
        ...
        <FFT64Neon as HalVecZnxBigImpl<FFT64Neon>>::vec_znx_big_add_assign(
            base_module(module),
            &mut base_big_mut(res),
            res_col,
            &base_big_ref(a),
    ...
    fn vec_znx_big_add_small_into_backend(
        ...
        <FFT64Neon as HalVecZnxBigImpl<FFT64Neon>>::vec_znx_big_add_small_into_backend(
            base_module(module),
            &mut base_big_mut(res),
            res_col,
            &base_big_ref(a),
    ...
    fn vec_znx_big_add_small_assign(
        ...
        <FFT64Neon as HalVecZnxBigImpl<FFT64Neon>>::vec_znx_big_add_small_assign(
            base_module(module),
            &mut base_big_mut(res),
            res_col,
            a,
    ...
    fn vec_znx_big_sub(
        ...
        <FFT64Neon as HalVecZnxBigImpl<FFT64Neon>>::vec_znx_big_sub(
            base_module(module),
            &mut base_big_mut(res),
            res_col,
            &base_big_ref(a),
    ...
    fn vec_znx_big_sub_assign(
        ...
        <FFT64Neon as HalVecZnxBigImpl<FFT64Neon>>::vec_znx_big_sub_assign(
            base_module(module),
            &mut base_big_mut(res),
            res_col,
            &base_big_ref(a),
    ...
    fn vec_znx_big_sub_negate_assign(
        ...
        <FFT64Neon as HalVecZnxBigImpl<FFT64Neon>>::vec_znx_big_sub_negate_assign(
            base_module(module),
            &mut base_big_mut(res),
            res_col,
            &base_big_ref(a),
    ...
    fn vec_znx_big_sub_small_a_backend(
        ...
        <FFT64Neon as HalVecZnxBigImpl<FFT64Neon>>::vec_znx_big_sub_small_a_backend(
            base_module(module),
            &mut base_big_mut(res),
            res_col,
            a,
    ...
    fn vec_znx_big_sub_small_assign(
        ...
        <FFT64Neon as HalVecZnxBigImpl<FFT64Neon>>::vec_znx_big_sub_small_assign(
            base_module(module),
            &mut base_big_mut(res),
            res_col,
            a,
    ...
    fn vec_znx_big_sub_small_b_backend(
        ...
        <FFT64Neon as HalVecZnxBigImpl<FFT64Neon>>::vec_znx_big_sub_small_b_backend(
            base_module(module),
            &mut base_big_mut(res),
            res_col,
            &base_big_ref(a),
    ...
    fn vec_znx_big_sub_small_negate_assign(
        ...
        <FFT64Neon as HalVecZnxBigImpl<FFT64Neon>>::vec_znx_big_sub_small_negate_assign(
            base_module(module),
            &mut base_big_mut(res),
            res_col,
            a,
    ...
    fn vec_znx_big_inner_sum_backend(
        ...
        <FFT64Neon as HalVecZnxBigImpl<FFT64Neon>>::vec_znx_big_inner_sum_backend(
            base_module(module),
            &mut base_big_mut(res),
            res_col,
            res_coeff,
    ...
    fn vec_znx_big_col_weighted_sum(
        ...
        <FFT64Neon as HalVecZnxBigImpl<FFT64Neon>>::vec_znx_big_col_weighted_sum(
            base_module(module),
            &mut base_big_mut(res),
            res_col,
            a,
    ...
    fn vec_znx_scalar_product(
        ...
        <FFT64Neon as HalVecZnxBigImpl<FFT64Neon>>::vec_znx_scalar_product(
            base_module(module),
            &mut base_big_mut(res),
            res_col,
            a,
    ...
    fn vec_znx_big_negate(
        ...
        <FFT64Neon as HalVecZnxBigImpl<FFT64Neon>>::vec_znx_big_negate(
            base_module(module),
            &mut base_big_mut(res),
            res_col,
            &base_big_ref(a),
    ...
    fn vec_znx_big_negate_assign(module: &Module<Self>, res: &mut VecZnxBigBackendMut<'_, Self>, res_col: usize) {
        <FFT64Neon as HalVecZnxBigImpl<FFT64Neon>>::vec_znx_big_negate_assign(
            base_module(module),
            &mut base_big_mut(res),
            res_col,
        )
    ...
    fn vec_znx_big_normalize_tmp_bytes(module: &Module<Self>) -> usize {
        <FFT64Neon as HalVecZnxBigImpl<FFT64Neon>>::vec_znx_big_normalize_tmp_bytes(base_module(module))
    }
    ...
    fn vec_znx_big_normalize(
        ...
        let mut scratch = scratch.borrow().into_backend::<FFT64Neon>();
        <FFT64Neon as HalVecZnxBigImpl<FFT64Neon>>::vec_znx_big_normalize(
            base_module(module),
            res,
            res_base2k,
            res_offset,
    ...
    fn vec_znx_big_automorphism(
        ...
        <FFT64Neon as HalVecZnxBigImpl<FFT64Neon>>::vec_znx_big_automorphism(
            base_module(module),
            k,
            &mut base_big_mut(res),
            res_col,
    ...
    fn vec_znx_big_automorphism_assign_tmp_bytes(module: &Module<Self>) -> usize {
        <FFT64Neon as HalVecZnxBigImpl<FFT64Neon>>::vec_znx_big_automorphism_assign_tmp_bytes(base_module(module))
    }
    ...
    fn vec_znx_big_automorphism_assign(
        ...
        let mut scratch = scratch.borrow().into_backend::<FFT64Neon>();
        <FFT64Neon as HalVecZnxBigImpl<FFT64Neon>>::vec_znx_big_automorphism_assign(
            base_module(module),
            k,
            &mut base_big_mut(res),
            res_col,
...
unsafe impl HalSvpImpl<FFT64NeonRayon> for FFT64NeonRayon {
    poulpy_cpu_ref::hal_impl_svp!(FFT64SvpDefault);
}
unsafe impl HalVecZnxDftImpl<FFT64NeonRayon> for FFT64NeonRayon {
    #[inline(always)]
    fn vec_znx_dft_apply(
        ...
        if !parallel_limb_tasks(res.size()) {
            return <FFT64Neon as HalVecZnxDftImpl<FFT64Neon>>::vec_znx_dft_apply(
                base_module(module),
                step,
                offset,
        ...
        res.raw_mut().par_chunks_mut(n * cols).enumerate().for_each(|(j, group)| {
            ...
            if limb < a_size {
                <FFT64Neon as ReimArith>::reim_from_znx(dst, a.at(a_col, limb));
                <FFT64Neon as ReimFFTExecute<ReimFFTTable<f64>, f64>>::reim_dft_execute(table, dst);
            } else {
                <FFT64Neon as ReimArith>::reim_zero(dst);
            }
        });
    }
    ...
    fn vec_znx_idft_apply_tmp_bytes(module: &Module<Self>) -> usize {
        <FFT64Neon as HalVecZnxDftImpl<FFT64Neon>>::vec_znx_idft_apply_tmp_bytes(base_module(module))
    }
    ...
    fn vec_znx_idft_apply(
        ...
        if !parallel_limb_tasks(res.size()) {
            let mut scratch = scratch.borrow().into_backend::<FFT64Neon>();
            return <FFT64Neon as HalVecZnxDftImpl<FFT64Neon>>::vec_znx_idft_apply(
                base_module(module),
                &mut base_big_mut(res),
                res_col,
                &base_dft_ref(a),
        ...
        res.raw_mut().par_chunks_mut(n * res_cols).enumerate().for_each(|(j, group)| {
            ...
            if j < min_size {
                ...
                <FFT64Neon as ReimArith>::reim_copy(dst_f64, src);
                <FFT64Neon as ReimFFTExecute<ReimIFFTTable<f64>, f64>>::reim_dft_execute(table, dst_f64);
                <FFT64Neon as ReimArith>::reim_to_znx_assign(dst_f64, divisor);
            } else {
                <FFT64Neon as ZnxZero>::znx_zero(dst);
            }
        });
    }
    ...
    fn vec_znx_idft_apply_tmpa(
        ...
        if !parallel_limb_tasks(res.size()) {
            return <FFT64Neon as HalVecZnxDftImpl<FFT64Neon>>::vec_znx_idft_apply_tmpa(
                base_module(module),
                &mut base_big_mut(res),
                res_col,
                &mut base_dft_mut(a),
        ...
        res_active
            ...
            .for_each(|(res_group, a_group)| {
                ...
                <FFT64Neon as ReimFFTExecute<ReimIFFTTable<f64>, f64>>::reim_dft_execute(table, src);
                <FFT64Neon as ReimArith>::reim_to_znx(dst, divisor, src);
            });
        res_zero
            .par_chunks_mut(n * res_cols)
            .for_each(|group| <FFT64Neon as ZnxZero>::znx_zero(&mut group[n * res_col..][..n]));
    }
    ...
    fn vec_znx_dft_add_into(
        ...
        <FFT64Neon as HalVecZnxDftImpl<FFT64Neon>>::vec_znx_dft_add_into(
            base_module(module),
            &mut base_dft_mut(res),
            res_col,
            &base_dft_ref(a),
    ...
    fn vec_znx_dft_add_scaled_assign(
        ...
        <FFT64Neon as HalVecZnxDftImpl<FFT64Neon>>::vec_znx_dft_add_scaled_assign(
            base_module(module),
            &mut base_dft_mut(res),
            res_col,
            &base_dft_ref(a),
    ...
    fn vec_znx_dft_add_assign(
        ...
        <FFT64Neon as HalVecZnxDftImpl<FFT64Neon>>::vec_znx_dft_add_assign(
            base_module(module),
            &mut base_dft_mut(res),
            res_col,
            &base_dft_ref(a),
    ...
    fn vec_znx_dft_sub(
        ...
        <FFT64Neon as HalVecZnxDftImpl<FFT64Neon>>::vec_znx_dft_sub(
            base_module(module),
            &mut base_dft_mut(res),
            res_col,
            &base_dft_ref(a),
    ...
    fn vec_znx_dft_sub_assign(
        ...
        <FFT64Neon as HalVecZnxDftImpl<FFT64Neon>>::vec_znx_dft_sub_assign(
            base_module(module),
            &mut base_dft_mut(res),
            res_col,
            &base_dft_ref(a),
    ...
    fn vec_znx_dft_sub_negate_assign(
        ...
        <FFT64Neon as HalVecZnxDftImpl<FFT64Neon>>::vec_znx_dft_sub_negate_assign(
            base_module(module),
            &mut base_dft_mut(res),
            res_col,
            &base_dft_ref(a),
    ...
    fn vec_znx_dft_copy(
        ...
        <FFT64Neon as HalVecZnxDftImpl<FFT64Neon>>::vec_znx_dft_copy(
            base_module(module),
            step,
            offset,
            &mut base_dft_mut(res),
    ...
    fn vec_znx_dft_zero(module: &Module<Self>, res: &mut VecZnxDftBackendMut<'_, Self>, res_col: usize) {
        <FFT64Neon as HalVecZnxDftImpl<FFT64Neon>>::vec_znx_dft_zero(base_module(module), &mut base_dft_mut(res), res_col)
    }

    type AutomorphismPlan = <FFT64Neon as HalVecZnxDftImpl<FFT64Neon>>::AutomorphismPlan;

    #[inline(always)]
    fn vec_znx_dft_automorphism_plan(module: &Module<Self>, p: i64) -> Self::AutomorphismPlan {
        <FFT64Neon as HalVecZnxDftImpl<FFT64Neon>>::vec_znx_dft_automorphism_plan(base_module(module), p)
    }
...
mod tests {
    ...
    fn coefficient_add_matches_wrapping_arithmetic() {
        ...
        <FFT64NeonRayon as ZnxAdd>::znx_add(&mut actual, &a, &b);
