...
///
/// `NTT4x30Portable` is a zero-sized marker type that selects the reference NTT4x30 CPU backend
...
/// # Thread safety
///
/// `NTT4x30Portable` is `Send + Sync` (derived from being a zero-sized, field-less struct).
/// The `Module<NTT4x30Portable>` that holds the NTT tables is also `Send + Sync`, so modules can
/// be shared across threads.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct NTT4x30Portable;
