...
fn test_convolution_by_const_fft64_ref() {
    let module: Module<FFT64Portable> = Module::<FFT64Portable>::new(8);
...
fn test_convolution_by_const_ntt4x30_ref() {
    let module: Module<NTT4x30Portable> = Module::<NTT4x30Portable>::new(8);
...
cross_backend_test_suite! {
    mod vec_znx,
    backend_ref =  crate::FFT64Portable,
...
fn test_vec_znx_rsh_assign_multi_limb_matches_rsh() {
    ...
    let n = 8usize;
    let module: Module<NTT4x30Portable> = Module::<NTT4x30Portable>::new(n as u64);
    let module_host: Module<HostBytesBackend> = Module::<HostBytesBackend>::new(n as u64);
    let mut scratch: ScratchOwned<NTT4x30Portable> = ScratchOwned::alloc(module.vec_znx_rsh_tmp_bytes(4));
    let base2k = 52usize;
    ...
    for size in [2usize, 3, 4] {
        for k in [60usize, 90, 105, 116] {
            ...
            let a_be = upload_vec_znx::<NTT4x30Portable>(&a);
            let mut want_be = upload_vec_znx::<NTT4x30Portable>(&module_host.vec_znx_alloc(1, size));
            module.vec_znx_rsh(
                base2k,
                k,
                &mut vec_znx_backend_mut::<NTT4x30Portable>(&mut want_be),
                0,
                &vec_znx_backend_ref::<NTT4x30Portable>(&a_be),
                0,
                &mut scratch.borrow(),
            );
            let mut got_be = upload_vec_znx::<NTT4x30Portable>(&a);
            module.vec_znx_rsh_assign(
                base2k,
                k,
                &mut vec_znx_backend_mut::<NTT4x30Portable>(&mut got_be),
                0,
                &mut scratch.borrow(),
            );
            assert_eq!(
...
fn assert_f64_word_containers_are_eq() {
    fn requires_eq<T: Eq>() {}
    requires_eq::<poulpy_hal::layouts::VecZnxDftOwned<crate::FFT64Portable>>();
    requires_eq::<poulpy_hal::layouts::VecZnxBigOwned<crate::FFT64Portable>>();
    requires_eq::<poulpy_hal::layouts::SvpPPolOwned<crate::FFT64Portable>>();
    requires_eq::<poulpy_hal::layouts::VmpPMatOwned<crate::FFT64Portable>>();
}
...
fn test_gglwe_product_dft_selected_fft64_ref() {
    poulpy_core::test_suite::parity::test_gglwe_product_dft_selected(&Module::<FFT64Portable>::new(64), 12);
}

#[cfg(feature = "enable-core")]
#[test]
fn test_gglwe_product_dft_selected_ntt4x30_ref() {
    poulpy_core::test_suite::parity::test_gglwe_product_dft_selected(&Module::<NTT4x30Portable>::new(64), 12);
}
...
poulpy_core::core_parity_test_suite! {
    ...
    backend_ref = crate::NTT4x30Portable,
...
fn test_vec_znx_big_normalize_limb_bounds_fft64_ref() {
    vec_znx_big_normalize_limb_bounds(&Module::<FFT64Portable>::new(8));
}

#[test]
fn test_vec_znx_big_normalize_limb_bounds_ntt4x30_ref() {
    vec_znx_big_normalize_limb_bounds(&Module::<NTT4x30Portable>::new(8));
}
...
fn test_vec_znx_big_normalize_input_bound_integer() {
    ...
    for a_base2k in 1..=62 {
        for res_base2k in 1..=62 {
            for a_size in 0..=8 {
                let mut input = VecZnxBig::<Vec<u8>, i128, NTT4x30Portable>::from_data(vec![0; 16 * N * a_size], N, 1, a_size);
                for j in 0..a_size {
                    for i in 0..N {
                        random ^= random << 17;
                        random ^= random >> 29;
                ...
                for res_size in 1..=8 {
                    ...
                    for offset in [
                        ...
                        vec_znx_normalize::<FFT64Portable>(
                            &mut <VecZnx<Vec<u8>, i64> as VecZnxToBackendMut<FFT64Portable>>::to_backend_mut(&mut small_output),
                            res_base2k,
                            res_size * res_base2k,
                            offset,
                            0,
                            &<VecZnx<Vec<u8>, i64> as VecZnxToBackendRef<FFT64Portable>>::to_backend_ref(&small_input),
                            a_base2k,
                            0,
                            &mut [0i64; 3 * N],
                        );
...
fn test_normalize_exact_canonical_precision() {
    ...
    for &(values, a_base2k, res_base2k, res_k, offset, expected) in cases {
        ...
        let mut big = VecZnxBig::<Vec<u8>, i128, NTT4x30Portable>::from_data(vec![0; 16 * values.len()], 1, 1, values.len());
        ...
        if a_base2k <= 62 && res_base2k <= 62 {
            vec_znx_normalize::<FFT64Portable>(
                &mut <VecZnx<Vec<u8>, i64> as VecZnxToBackendMut<FFT64Portable>>::to_backend_mut(&mut output),
                res_base2k,
                res_k,
                offset,
                0,
                &<VecZnx<Vec<u8>, i64> as VecZnxToBackendRef<FFT64Portable>>::to_backend_ref(&small),
                a_base2k,
                0,
                &mut [0; 3],
            );
            ...
            vec_znx_normalize_assign::<FFT64Portable>(
                ...
                res_k,
                0,
                &mut <VecZnx<Vec<u8>, i64> as VecZnxToBackendMut<FFT64Portable>>::to_backend_mut(&mut small),
                0,
                &mut [0],
            );
...
fn test_vec_znx_big_normalize_assign_and_ranges() {
    ...
    let mut regression_input = VecZnxBig::<Vec<u8>, i128, NTT4x30Portable>::from_data(vec![0; 16], 1, 1, 1);
    ...
    for a_base2k in [1, 2, 17, 50, 62] {
        for res_base2k in [1, 2, 19, 51, 62] {
            for offset in [i64::MIN, -400, -63, -1, 0, 1, 63, 400, i64::MAX] {
                let mut input = VecZnxBig::<Vec<u8>, i128, NTT4x30Portable>::from_data(vec![0; 16 * N * 3], N, 1, 3);
                for j in 0..3 {
                    for i in 0..N {
...
fn test_vec_znx_big_normalize_wide_radices() {
    ...
    for a_base2k in [1, 63, 64, 65, 126, 127] {
        for res_base2k in [1, 17, 63, 64] {
            for size in 1..=3 {
                let mut input = VecZnxBig::<Vec<u8>, i128, NTT4x30Portable>::from_data(vec![0; 32 * size], 2, 1, size);
                for j in 0..size {
                    input
                        .at_mut(0, j)
...
mod canonical_precision_tests {
    ...
    fn check(a: &[Vec<i128>], ka: usize, kr: usize, k: usize, size: usize, offset: i64, ranges: bool) {
        ...
        let mut wide = VecZnxBig::<Vec<u8>, i128, NTT4x30Portable>::from_data(vec![0; 16 * N * 2 * a.len()], N, 2, a.len());
        ...
        if narrow {
            let src = <VecZnx<Vec<u8>, i64> as VecZnxToBackendRef<FFT64Portable>>::to_backend_ref(&input);
            if ranges {
        ...
        if narrow && ka == kr && size == a.len() && offset == 0 {
            if ranges {
                ...
                vec_znx_normalize_assign::<FFT64Portable>(
                    ...
                    k,
                    0,
                    &mut <VecZnx<Vec<u8>, i64> as VecZnxToBackendMut<FFT64Portable>>::to_backend_mut(&mut input),
                    1,
                    &mut [73; N],
                );
            }
