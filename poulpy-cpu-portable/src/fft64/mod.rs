...
//! implements every OEP trait for the [`FFT64Portable`] backend by delegating to the reference
...
///
/// `FFT64Portable` is a zero-sized marker type that selects the reference CPU backend
...
/// # Thread safety
///
/// `FFT64Portable` is `Send + Sync` (derived from being a zero-sized, field-less struct).
