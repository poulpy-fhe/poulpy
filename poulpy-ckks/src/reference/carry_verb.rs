...
//!
//! [`CKKSAddDefault`](crate::reference::CKKSAddDefault) and
//! [`CKKSSubDefault`](crate::reference::CKKSSubDefault) are exact mirrors: same operand alignment, same metadata stamping, same plaintext facades — only the core GLWE verb and its coefficient-wise HAL hooks differ.
...
macro_rules! ckks_carry_verb_default {
    (
        ...
        ::paste::paste! {
            ...
            #[doc = ""]
            #[doc = "Generated by `ckks_carry_verb_default!` — the add and sub families share one body (see `reference::carry_verb`)."]
            pub trait $Trait<BE: Backend> {
                ...
                {
                    self.[<ckks_ $verb _into_unnormalized_default>](dst, a, b, scratch)?;
                ...
                where
                    ...
                    A: GLWEToBackendRef<BE> + CKKSInfos,
                {
                    self.[<ckks_ $verb _assign_unnormalized_default>](dst, a, scratch)?;
                    self.glwe_normalize_assign(dst, scratch);
                    Ok(())
                }
