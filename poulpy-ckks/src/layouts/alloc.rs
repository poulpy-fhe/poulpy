...
pub trait CKKSModuleAlloc<BE: Backend>: ModuleCoreAlloc<OwnedBuf = BE::OwnedBuf, ZnxWord = BE::ZnxWord> {
    ...
    /// `2 * slots`, raised to the backend's minimum sparse degree and capped at
    /// the ring degree. Its value under the ring embedding is `M(X^(N/n))` for
    /// the degree-`n` polynomial `M` it stores; `slots == N/2` gives the dense
    /// plaintext.
        ...
        Self: GetDegree,
    {
        ...
        let n = (2 * slots).max(BE::MIN_SPARSE_DEGREE).min(ring);
