...
mod tests {
    ...
    fn compact_plaintext_degree_is_twice_the_slots_clamped_to_the_floor_and_the_ring() {
        ...
        assert_eq!(
            ...
            <HostBytesBackend as Backend>::MIN_SPARSE_DEGREE
        );
        // 32 slots: the dense plaintext, degree N.
        assert_eq!(module.ckks_pt_vec_alloc_compact(32, base2k, k).n().as_usize(), 64);
        // A degree-8 module: never above the ring degree.
        let tiny = Module::<HostBytesBackend>::new(8);
        assert_eq!(tiny.ckks_pt_vec_alloc_compact(1, base2k, k).n().as_usize(), 8);
    }
