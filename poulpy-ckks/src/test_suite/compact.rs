...
use crate::{
    ...
    api::{
        CKKSAddOps, CKKSEncodingHostOps, CKKSLinearTransformationOps, CKKSMulOps, CKKSSubOps, LinearTransformation,
...
/// the plaintext is a degree-`N/4` object (`2 * slots`) above the CPU floor at
/// the suite's `N = 256`.
...
/// `ct + compact` and `ct - compact` decrypt to exactly what `ct + dense` and
/// `ct - dense` decrypt to, and to the expected values within the suite precision.
pub fn test_compact_plaintext_add_sub<BE, F, E>(
    params: CKKSTestParams,
    module: &Module<BE>,
    host_module: &Module<HostBytesBackend>,
) where
    ...
    E: NegacyclicFFT<F> + NegacyclicFFTNew<F>,
{
    ...
    // The raw multiply takes a dense plaintext only: a compact one is an error, not a panic.
    let mut product = alloc_ct(&params, module, params.k);
    assert!(
        module
            .ckks_mul_pt_vec_into(&mut product, &ct, &compact, &mut scratch.borrow())
            .is_err(),
        "ckks_mul_pt_vec_into accepted a compact plaintext"
    );
    // A degree that is not a power-of-two divisor is an error on the add too.
...
/// `dec(lt(enc(a), B)) ≈ B·a` with compact diagonals, resident path; the
/// prepared slots carry the compact degree.
