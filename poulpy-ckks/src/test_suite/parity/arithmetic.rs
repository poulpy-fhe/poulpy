        ...
        for (sparse, slots) in [(0, SlotsKind::Complex), (2, SlotsKind::Real)] {
            for delta_difference in [0, 3] {
                ...
                ($method:ident, $query:ident, $initial:expr, [$($arg:expr),*]) => {{
                    ...
                    let out = with_scratch::<B,_>(bytes, |scratch| out.normalize(module, scratch)).unwrap();
                    results.push((stringify!($method), snapshot::<B,_>(&out)));
                }};
            }
...
where
    ...
    Module<BR>: GLWETensorKeyPreparedFactory<BR>,
    Module<BT>: GLWETensorKeyPreparedFactory<BT>,
{
    let _scalar = std::marker::PhantomData::<F>;
...
where
    ...
    Module<BR>: GLWEAutomorphismKeyPreparedFactory<BR>,
    Module<BT>: GLWEAutomorphismKeyPreparedFactory<BT>,
{
    let _scalar = std::marker::PhantomData::<F>;
