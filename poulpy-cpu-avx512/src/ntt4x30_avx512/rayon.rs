...
poulpy_hal::impl_backend_from!(NTT4x30Avx512RayonBackend<R>, NTT4x30Avx512Backend<R>, RayonTaskExecutor; R: CpuRing);

fn base_module<R: CpuRing>(module: &Module<NTT4x30Avx512RayonBackend<R>>) -> &Module<NTT4x30Avx512Backend<R>> {
    module.reinterpret()
}

fn base_dft_ref<'a, R: CpuRing>(
    a: &'a VecZnxDftBackendRef<'_, NTT4x30Avx512RayonBackend<R>>,
) -> VecZnxDftBackendRef<'a, NTT4x30Avx512Backend<R>> {
    VecZnxDft::from_shape(&**a.data(), a.shape())
}

fn base_znx_ref<'a, R: CpuRing>(
    a: &'a VecZnxBackendRef<'_, NTT4x30Avx512RayonBackend<R>>,
) -> VecZnxBackendRef<'a, NTT4x30Avx512Backend<R>> {
    VecZnx::from_shape(&**a.data(), a.shape())
}

fn base_znx_mut<'a, R: CpuRing>(
    a: &'a mut VecZnxBackendMut<'_, NTT4x30Avx512RayonBackend<R>>,
) -> VecZnxBackendMut<'a, NTT4x30Avx512Backend<R>> {
    let shape = a.shape();
    VecZnx::from_shape(&mut **a.data_mut(), shape)
}

fn base_scalar_ref<'a, R: CpuRing>(
    a: &'a ScalarZnxBackendRef<'_, NTT4x30Avx512RayonBackend<R>>,
) -> ScalarZnxBackendRef<'a, NTT4x30Avx512Backend<R>> {
    ScalarZnx::from_data(&**a.data(), a.n(), a.cols())
}

fn base_svp_ref<'a, R: CpuRing>(
    a: &'a SvpPPolBackendRef<'_, NTT4x30Avx512RayonBackend<R>>,
) -> SvpPPolBackendRef<'a, NTT4x30Avx512Backend<R>> {
    SvpPPol::from_data(&**a.data(), a.n(), a.cols(), a.hint())
}

fn base_svp_mut<'a, R: CpuRing>(
    a: &'a mut SvpPPolBackendMut<'_, NTT4x30Avx512RayonBackend<R>>,
) -> SvpPPolBackendMut<'a, NTT4x30Avx512Backend<R>> {
    let (n, cols, hint) = (a.n(), a.cols(), a.hint());
    SvpPPol::from_data(&mut **a.data_mut(), n, cols, hint)
}

pub(crate) fn base_dft_mut<'a, R: CpuRing>(
    a: &'a mut VecZnxDftBackendMut<'_, NTT4x30Avx512RayonBackend<R>>,
) -> VecZnxDftBackendMut<'a, NTT4x30Avx512Backend<R>> {
    let shape = a.shape();
    VecZnxDft::from_shape(&mut **a.data_mut(), shape)
}

fn base_big_mut<'a, R: CpuRing>(
    a: &'a mut VecZnxBigBackendMut<'_, NTT4x30Avx512RayonBackend<R>>,
) -> VecZnxBigBackendMut<'a, NTT4x30Avx512Backend<R>> {
    let shape = a.shape();
    VecZnxBig::from_shape(&mut **a.data_mut(), shape)
}

fn base_big_ref<'a, R: CpuRing>(
    a: &'a poulpy_hal::layouts::VecZnxBigBackendRef<'_, NTT4x30Avx512RayonBackend<R>>,
) -> poulpy_hal::layouts::VecZnxBigBackendRef<'a, NTT4x30Avx512Backend<R>> {
    VecZnxBig::from_shape(&**a.data(), a.shape())
}

fn base_vmp_ref<'a, R: CpuRing>(
    a: &'a VmpPMatBackendRef<'_, NTT4x30Avx512RayonBackend<R>>,
) -> VmpPMatBackendRef<'a, NTT4x30Avx512Backend<R>> {
    VmpPMat::from_data(&**a.data(), a.n(), a.rows(), a.cols_in(), a.cols_out(), a.size(), a.hint())
}

fn base_vmp_mut<'a, R: CpuRing>(
    a: &'a mut VmpPMatBackendMut<'_, NTT4x30Avx512RayonBackend<R>>,
) -> VmpPMatBackendMut<'a, NTT4x30Avx512Backend<R>> {
...
pub(crate) fn base_cnv_l_ref<'a, R: CpuRing>(
    a: &'a CnvPVecLBackendRef<'_, NTT4x30Avx512RayonBackend<R>>,
) -> CnvPVecLBackendRef<'a, NTT4x30Avx512Backend<R>> {
    CnvPVecL::from_data(&**a.data(), a.n(), a.cols(), a.size(), a.hint())
}

fn base_cnv_l_mut<'a, R: CpuRing>(
    a: &'a mut CnvPVecLBackendMut<'_, NTT4x30Avx512RayonBackend<R>>,
) -> CnvPVecLBackendMut<'a, NTT4x30Avx512Backend<R>> {
...
pub(crate) fn base_cnv_r_ref<'a, R: CpuRing>(
    a: &'a CnvPVecRBackendRef<'_, NTT4x30Avx512RayonBackend<R>>,
) -> CnvPVecRBackendRef<'a, NTT4x30Avx512Backend<R>> {
    CnvPVecR::from_data(&**a.data(), a.n(), a.cols(), a.size(), a.hint())
}

fn base_cnv_r_mut<'a, R: CpuRing>(
    a: &'a mut CnvPVecRBackendMut<'_, NTT4x30Avx512RayonBackend<R>>,
) -> CnvPVecRBackendMut<'a, NTT4x30Avx512Backend<R>> {
    let (n, cols, size, hint) = (a.n(), a.cols(), a.size(), a.hint());
...
macro_rules! parallel_binary {
    ($trait:ident, $method:ident) => {
        impl<R: CpuRing> $trait for NTT4x30Avx512RayonBackend<R> {
            fn $method(res: &mut [i64], a: &[i64], b: &[i64]) {
                let Some(chunk) = parallel_chunk_len::<Self>(res.len()) else {
                    return <NTT4x30Avx512Backend<R> as $trait>::$method(res, a, b);
                };
                res.par_chunks_mut(chunk)
                    .zip(a.par_chunks(chunk))
                    .zip(b.par_chunks(chunk))
                    .for_each(|((res, a), b)| <NTT4x30Avx512Backend<R> as $trait>::$method(res, a, b));
            }
        }
    };
}
    ...
    ($trait:ident, $method:ident) => {
        impl<R: CpuRing> $trait for NTT4x30Avx512RayonBackend<R> {
            fn $method(res: &mut [i64], a: &[i64]) {
                let Some(chunk) = parallel_chunk_len::<Self>(res.len()) else {
                    return <NTT4x30Avx512Backend<R> as $trait>::$method(res, a);
                };
                res.par_chunks_mut(chunk)
                    .zip(a.par_chunks(chunk))
                    .for_each(|(res, a)| <NTT4x30Avx512Backend<R> as $trait>::$method(res, a));
            }
        }
    };
}
    ...
    ($trait:ident, $method:ident) => {
        impl<R: CpuRing> $trait for NTT4x30Avx512RayonBackend<R> {
            fn $method(res: &mut [i64]) {
                let Some(chunk) = parallel_chunk_len::<Self>(res.len()) else {
                    return <NTT4x30Avx512Backend<R> as $trait>::$method(res);
                };
                res.par_chunks_mut(chunk)
                    .for_each(|res| <NTT4x30Avx512Backend<R> as $trait>::$method(res));
            }
        }
    };
}
    ...
    ($trait:ident, $method:ident) => {
        impl<R: CpuRing> $trait for NTT4x30Avx512RayonBackend<R> {
            fn $method(k: i64, res: &mut [i64], a: &[i64]) {
                let Some(chunk) = parallel_chunk_len::<Self>(res.len()) else {
                    return <NTT4x30Avx512Backend<R> as $trait>::$method(k, res, a);
                };
                res.par_chunks_mut(chunk)
                    .zip(a.par_chunks(chunk))
                    .for_each(|(res, a)| <NTT4x30Avx512Backend<R> as $trait>::$method(k, res, a));
            }
        }
    };
}
...
macro_rules! forward_znx {
    ($trait:ident, $method:ident($($arg:ident: $ty:ty),* $(,)?)) => {
        impl<R: CpuRing> $trait for NTT4x30Avx512RayonBackend<R> {
            #[inline(always)]
            fn $method($($arg: $ty),*) {
                <NTT4x30Avx512Backend<R> as $trait>::$method($($arg),*)
            }
        }
    };
}
    ...
    ($trait:ident, $method:ident($($arg:ident: $ty:ty),* $(,)?)) => {
        impl<R: CpuRing> $trait for NTT4x30Avx512RayonBackend<R> {
            ...
            fn $method<const OVERWRITE: bool>($($arg: $ty),*) {
                <NTT4x30Avx512Backend<R> as $trait>::$method::<OVERWRITE>($($arg),*)
            }
        }
    };
}
...
impl<R: CpuRing> ZnxMulPowerOfTwoAssign for NTT4x30Avx512RayonBackend<R> {
    fn znx_mul_power_of_two_assign(k: i64, res: &mut [i64]) {
        let Some(chunk) = parallel_chunk_len::<Self>(res.len()) else {
            return <NTT4x30Avx512Backend<R> as ZnxMulPowerOfTwoAssign>::znx_mul_power_of_two_assign(k, res);
        };
        res.par_chunks_mut(chunk)
            .for_each(|res| <NTT4x30Avx512Backend<R> as ZnxMulPowerOfTwoAssign>::znx_mul_power_of_two_assign(k, res));
    }
}
...
impl<R: CpuRing> ZnxExtractDigitAddMul for NTT4x30Avx512RayonBackend<R> {
    #[inline(always)]
    fn znx_extract_digit_addmul(base2k: usize, lsh: usize, res: &mut [i64], src: &mut [i64]) {
        <NTT4x30Avx512Backend<R> as ZnxExtractDigitAddMul>::znx_extract_digit_addmul(base2k, lsh, res, src);
    }
}

impl<R: CpuRing> poulpy_cpu_ref::reference::normalization::I64NormalizeOps for NTT4x30Avx512RayonBackend<R> {
    #[inline(always)]
    fn znx_normalize_floor<const CARRY_IN: bool, const ROUND: bool>(base2k: usize, lsh: usize, a: &[i64], carry: &mut [i64]) {
        <NTT4x30Avx512Backend<R> as poulpy_cpu_ref::reference::normalization::I64NormalizeOps>::znx_normalize_floor::<
            CARRY_IN,
            ROUND,
        >(base2k, lsh, a, carry)
    }
    ...
    fn znx_normalize_round<const CARRY_IN: bool, const PAD: bool>(
        ...
        <NTT4x30Avx512Backend<R> as poulpy_cpu_ref::reference::normalization::I64NormalizeOps>::znx_normalize_round::<CARRY_IN, PAD>(
    ...
    fn znx_normalize_round_assign<const CARRY_IN: bool>(
        ...
        <NTT4x30Avx512Backend<R> as poulpy_cpu_ref::reference::normalization::I64NormalizeOps>::znx_normalize_round_assign::<
            CARRY_IN,
    ...
    fn znx_extract_digit_mul(base2k: usize, lsh: usize, res: &mut [i64], src: &mut [i64]) {
        <NTT4x30Avx512Backend<R> as poulpy_cpu_ref::reference::normalization::I64NormalizeOps>::znx_extract_digit_mul(
            base2k, lsh, res, src,
        );
    }
    ...
    fn znx_extract_digit_addmul_normalize<const OVERWRITE: bool>(
        ...
        <NTT4x30Avx512Backend<R> as poulpy_cpu_ref::reference::normalization::I64NormalizeOps>::znx_extract_digit_addmul_normalize::<
            OVERWRITE,
...
impl<R: CpuRing> NttDFTExecute<NttTable<Primes30, R>> for NTT4x30Avx512RayonBackend<R> {
    fn ntt_dft_execute(table: &NttTable<Primes30, R>, data: &mut [u64]) {
        <NTT4x30Avx512Backend<R> as NttDFTExecute<NttTable<Primes30, R>>>::ntt_dft_execute(table, data)
    }
}

impl<R: CpuRing> NttDFTExecute<NttTableInv<Primes30, R>> for NTT4x30Avx512RayonBackend<R> {
    fn ntt_dft_execute(table: &NttTableInv<Primes30, R>, data: &mut [u64]) {
        <NTT4x30Avx512Backend<R> as NttDFTExecute<NttTableInv<Primes30, R>>>::ntt_dft_execute(table, data)
    }
}

impl<R: CpuRing> NttFromZnx64 for NTT4x30Avx512RayonBackend<R> {
    fn ntt_from_znx64(res: &mut [u64], a: &[i64]) {
        <NTT4x30Avx512Backend<R> as NttFromZnx64>::ntt_from_znx64(res, a)
    }
}

impl<R: CpuRing> NttToZnx128 for NTT4x30Avx512RayonBackend<R> {
    fn ntt_to_znx128(res: &mut [i128], divisor_is_n: usize, a: &[u64]) {
        <NTT4x30Avx512Backend<R> as NttToZnx128>::ntt_to_znx128(res, divisor_is_n, a)
    }
}
    ...
    ($trait:ident, $method:ident) => {
        impl<R: CpuRing> $trait for NTT4x30Avx512RayonBackend<R> {
            fn $method(res: &mut [u64], a: &[u64], b: &[u64]) {
                <NTT4x30Avx512Backend<R> as $trait>::$method(res, a, b)
            }
        }
    };
}
    ...
    ($trait:ident, $method:ident) => {
        impl<R: CpuRing> $trait for NTT4x30Avx512RayonBackend<R> {
            fn $method(res: &mut [u64], a: &[u64]) {
                <NTT4x30Avx512Backend<R> as $trait>::$method(res, a)
            }
        }
    };
}
...
impl<R: CpuRing> NttNegateAssign for NTT4x30Avx512RayonBackend<R> {
    fn ntt_negate_assign(res: &mut [u64]) {
        <NTT4x30Avx512Backend<R> as NttNegateAssign>::ntt_negate_assign(res)
    }
}
impl<R: CpuRing> NttZero for NTT4x30Avx512RayonBackend<R> {
    fn ntt_zero(res: &mut [u64]) {
        <NTT4x30Avx512Backend<R> as NttZero>::ntt_zero(res)
    }
}
forward_ntt_assign!(NttCopy, ntt_copy);

impl<R: CpuRing> NttMulBbb for NTT4x30Avx512RayonBackend<R> {
    fn ntt_mul_bbb(meta: &BbbMeta<Primes30>, ell: usize, res: &mut [u64], a: &[u64], b: &[u64]) {
        <NTT4x30Avx512Backend<R> as NttMulBbb>::ntt_mul_bbb(meta, ell, res, a, b)
    }
}
impl<R: CpuRing> NttMulBbc for NTT4x30Avx512RayonBackend<R> {
    fn ntt_mul_bbc(meta: &BbcMeta<Primes30>, ell: usize, res: &mut [u64], a: &[u32], b: &[u32]) {
        <NTT4x30Avx512Backend<R> as NttMulBbc>::ntt_mul_bbc(meta, ell, res, a, b)
    }
}
impl<R: CpuRing> NttCFromB for NTT4x30Avx512RayonBackend<R> {
    fn ntt_c_from_b(n: usize, res: &mut [u32], a: &[u64]) {
        <NTT4x30Avx512Backend<R> as NttCFromB>::ntt_c_from_b(n, res, a)
    }
}
impl<R: CpuRing> NttMulBbc1ColX2 for NTT4x30Avx512RayonBackend<R> {
    fn ntt_mul_bbc_1col_x2(meta: &BbcMeta<Primes30>, ell: usize, res: &mut [u64], a: &[u32], b: &[u32]) {
        <NTT4x30Avx512Backend<R> as NttMulBbc1ColX2>::ntt_mul_bbc_1col_x2(meta, ell, res, a, b)
    }
    fn ntt_mul_bbc_tile4_x2(meta: &BbcMeta<Primes30>, len: usize, res: &mut [u64], a: &[u32], b: &[u32]) {
        <NTT4x30Avx512Backend<R> as NttMulBbc1ColX2>::ntt_mul_bbc_tile4_x2(meta, len, res, a, b)
    }
}
impl<R: CpuRing> NttMulBbc2ColsX2 for NTT4x30Avx512RayonBackend<R> {
    fn ntt_mul_bbc_2cols_x2(meta: &BbcMeta<Primes30>, ell: usize, res: &mut [u64], a: &[u32], b: &[u32]) {
        <NTT4x30Avx512Backend<R> as NttMulBbc2ColsX2>::ntt_mul_bbc_2cols_x2(meta, ell, res, a, b)
    }
}
impl<R: CpuRing> NttExtract1BlkContiguous for NTT4x30Avx512RayonBackend<R> {
    fn ntt_extract_1blk_contiguous(n: usize, rows: usize, blk: usize, dst: &mut [u64], src: &[u64]) {
        <NTT4x30Avx512Backend<R> as NttExtract1BlkContiguous>::ntt_extract_1blk_contiguous(n, rows, blk, dst, src)
    }
}
impl<R: CpuRing> NttPackLeft1BlkX2 for NTT4x30Avx512RayonBackend<R> {
    fn ntt_pack_left_1blk_x2(dst: &mut [u32], a: &[u64], rows: usize, stride: usize, blk: usize) {
        <NTT4x30Avx512Backend<R> as NttPackLeft1BlkX2>::ntt_pack_left_1blk_x2(dst, a, rows, stride, blk)
    }
}
impl<R: CpuRing> NttPackRight1BlkX2 for NTT4x30Avx512RayonBackend<R> {
    fn ntt_pack_right_1blk_x2(dst: &mut [u32], a: &[u32], rows: usize, stride: usize, blk: usize) {
        <NTT4x30Avx512Backend<R> as NttPackRight1BlkX2>::ntt_pack_right_1blk_x2(dst, a, rows, stride, blk)
    }
}
impl<R: CpuRing> NttPairwisePackLeft1BlkX2 for NTT4x30Avx512RayonBackend<R> {
    fn ntt_pairwise_pack_left_1blk_x2(dst: &mut [u32], a: &[u64], b: &[u64], rows: usize, stride: usize, blk: usize) {
        <NTT4x30Avx512Backend<R> as NttPairwisePackLeft1BlkX2>::ntt_pairwise_pack_left_1blk_x2(dst, a, b, rows, stride, blk)
    }
}
impl<R: CpuRing> NttPairwisePackRight1BlkX2 for NTT4x30Avx512RayonBackend<R> {
    fn ntt_pairwise_pack_right_1blk_x2(dst: &mut [u32], a: &[u32], b: &[u32], rows: usize, stride: usize, blk: usize) {
        <NTT4x30Avx512Backend<R> as NttPairwisePackRight1BlkX2>::ntt_pairwise_pack_right_1blk_x2(dst, a, b, rows, stride, blk)
    }
}
...
impl<R: CpuRing> I128BigOps for NTT4x30Avx512RayonBackend<R> {
...
impl<R: CpuRing> I128NormalizeOps for NTT4x30Avx512RayonBackend<R> {
    #[inline(always)]
    fn nfc_add_small_carry(carry: &mut [i128], a: &[i64]) {
        <NTT4x30Avx512Backend<R> as I128NormalizeOps>::nfc_add_small_carry(carry, a)
    }

    #[inline(always)]
    fn znx_extract_digit_addmul_i128(base2k: usize, lsh: usize, res: &mut [i64], src: &mut [i128]) {
        <NTT4x30Avx512Backend<R> as I128NormalizeOps>::znx_extract_digit_addmul_i128(base2k, lsh, res, src)
    }

    #[inline(always)]
    fn nfc_normalize_floor<const CARRY_IN: bool, const ROUND: bool>(base2k: usize, lsh: usize, a: &[i128], carry: &mut [i128]) {
        <NTT4x30Avx512Backend<R> as I128NormalizeOps>::nfc_normalize_floor::<CARRY_IN, ROUND>(base2k, lsh, a, carry)
    }
    ...
    fn nfc_normalize_round<const CARRY_IN: bool, const PAD: bool>(
        ...
        <NTT4x30Avx512Backend<R> as I128NormalizeOps>::nfc_normalize_round::<CARRY_IN, PAD>(base2k, lsh, padding, res, a, carry)
    }

    const FUSE_NORMALIZE: bool =
        <NTT4x30Avx512Backend<R> as poulpy_cpu_ref::reference::ntt4x30::I128NormalizeOps>::FUSE_NORMALIZE;

    fn znx_extract_digit_mul_i128(base2k: usize, lsh: usize, res: &mut [i64], src: &mut [i128]) {
        <NTT4x30Avx512Backend<R> as poulpy_cpu_ref::reference::ntt4x30::I128NormalizeOps>::znx_extract_digit_mul_i128(
            base2k, lsh, res, src,
        )
    }
    ...
    fn znx_extract_digit_addmul_normalize_i128<const OVERWRITE: bool>(
        ...
        <NTT4x30Avx512Backend<R> as poulpy_cpu_ref::reference::ntt4x30::I128NormalizeOps>::znx_extract_digit_addmul_normalize_i128::<
            OVERWRITE,
        >(base2k, lsh, res_base2k, res, src, carry)
    }

    fn nfc_middle_step(base2k: usize, lsh: usize, res: &mut [i64], a: &[i128], carry: &mut [i128]) {
        <NTT4x30Avx512Backend<R> as I128NormalizeOps>::nfc_middle_step(base2k, lsh, res, a, carry)
    }
    fn nfc_middle_step_into<O: AssignOp>(base2k: usize, lsh: usize, res: &mut [i64], a: &[i128], carry: &mut [i128]) {
        <NTT4x30Avx512Backend<R> as I128NormalizeOps>::nfc_middle_step_into::<O>(base2k, lsh, res, a, carry)
    }
    fn nfc_middle_step_assign(base2k: usize, lsh: usize, res: &mut [i64], carry: &mut [i128]) {
        <NTT4x30Avx512Backend<R> as I128NormalizeOps>::nfc_middle_step_assign(base2k, lsh, res, carry)
    }
    fn nfc_final_step_assign(base2k: usize, lsh: usize, res: &mut [i64], carry: &mut [i128]) {
        <NTT4x30Avx512Backend<R> as I128NormalizeOps>::nfc_final_step_assign(base2k, lsh, res, carry)
    }
    fn nfc_final_step_into<O: AssignOp>(base2k: usize, lsh: usize, res: &mut [i64], carry: &mut [i128]) {
        <NTT4x30Avx512Backend<R> as I128NormalizeOps>::nfc_final_step_into::<O>(base2k, lsh, res, carry)
    }
}

impl<R: CpuRing> BigWordHadamardProduct for NTT4x30Avx512RayonBackend<R> {
    fn big_word_hadamard_product(res: &mut [i128], a: &[i64], b: &[i64]) {
        <Self as I128BigOps>::i128_hadamard_product_i64(res, a, b)
    }
}

unsafe impl<R: CpuRing> HalVecZnxImpl for NTT4x30Avx512RayonBackend<R> {
    ...
    fn vec_znx_normalize(
        ...
        poulpy_cpu_rayon::normalize::vec_znx_normalize_par::<NTT4x30Avx512Backend<R>, Self>(
    ...
    fn vec_znx_normalize_assign(
        ...
        poulpy_cpu_rayon::normalize::vec_znx_normalize_assign_par::<NTT4x30Avx512Backend<R>, Self>(
            base2k, k, a_offset, a, a_col, carry,
        );
    }
}
unsafe impl<R: CpuRing> HalModuleImpl for NTT4x30Avx512RayonBackend<R> {
    poulpy_cpu_ref::hal_impl_module!(NTT4x30ModuleDefault);
}
unsafe impl<R: CpuRing> HalVmpImpl for NTT4x30Avx512RayonBackend<R> {
    ...
    fn vmp_prepare(
        ...
        super::vmp::vmp_prepare_avx_pm(base_module::<R>(module), &mut base_vmp_mut::<R>(res), a, tmp);
    }
    ...
    fn vmp_apply_dft_to_dft(
        ...
        if RayonTaskExecutor::should_serialize_inner() {
            super::vmp::vmp_apply_dft_to_dft_avx::<SerialTaskExecutor, _>(
            ...
            super::vmp::vmp_apply_dft_to_dft_avx::<RayonTaskExecutor, _>(
                base_module::<R>(module),
                &mut base_dft_mut::<R>(res),
                &base_dft_ref::<R>(a),
                &base_vmp_ref::<R>(b),
    ...
    fn vmp_apply_dft_to_dft_add(
        ...
        let (tmp, _) = crate::hal_impl::take_host_typed::<Self, u64>(scratch.borrow(), bytes / size_of::<u64>());
        if RayonTaskExecutor::should_serialize_inner() {
            super::vmp::vmp_apply_dft_to_dft_add_avx::<SerialTaskExecutor, _>(
                base_module::<R>(module),
                &mut base_dft_mut::<R>(res),
                &base_dft_ref::<R>(a),
                &base_vmp_ref::<R>(b),
            ...
            super::vmp::vmp_apply_dft_to_dft_add_avx::<RayonTaskExecutor, _>(
                base_module::<R>(module),
                &mut base_dft_mut::<R>(res),
                &base_dft_ref::<R>(a),
                &base_vmp_ref::<R>(b),
    ...
    fn vmp_extract_selected_rows(
        ...
        NTT4x30Avx512Backend::<R>::vmp_extract_selected_rows(
            base_module::<R>(module),
...
unsafe impl<R: CpuRing> poulpy_core::oep::GGLWEProductDigitsStridedImpl for NTT4x30Avx512RayonBackend<R> {
    ...
    fn gglwe_product_digits_strided(
        ...
        let (tmp, _) = crate::hal_impl::take_host_typed::<Self, u64>(scratch.borrow(), bytes / size_of::<u64>());
        super::vmp::vmp_apply_dft_to_dft_digits_strided_avx::<RayonTaskExecutor, _>(
            base_module::<R>(module),
            &mut base_dft_mut::<R>(res),
            &base_dft_ref::<R>(a),
            dsize,
...
pub(crate) fn vmp_apply_digits_strided_known_zero_prefix<R: CpuRing>(
    module: &Module<NTT4x30Avx512RayonBackend<R>>,
    res: &mut VecZnxDftBackendMut<'_, NTT4x30Avx512RayonBackend<R>>,
    a: &VecZnxDftBackendRef<'_, NTT4x30Avx512RayonBackend<R>>,
    dsize: usize,
    zero_prefix: usize,
    product_limbs: usize,
    pmat: &VmpPMatBackendRef<'_, NTT4x30Avx512RayonBackend<R>>,
    scratch: &mut ScratchArena<'_, NTT4x30Avx512RayonBackend<R>>,
) {
    let bytes =
        <NTT4x30Avx512RayonBackend<R> as poulpy_core::oep::GGLWEProductDigitsStridedImpl>::gglwe_product_digits_strided_tmp_bytes(
            module,
            res.size(),
            a.cols(),
            a.size(),
    ...
    let (tmp, _) =
        crate::hal_impl::take_host_typed::<NTT4x30Avx512RayonBackend<R>, u64>(scratch.borrow(), bytes / size_of::<u64>());
    super::vmp::vmp_apply_dft_to_dft_digits_strided_avx_known_zero_prefix::<RayonTaskExecutor, _>(
        base_module::<R>(module),
        &mut base_dft_mut::<R>(res),
        &base_dft_ref::<R>(a),
        dsize,
...
unsafe impl<R: CpuRing> HalConvolutionImpl for NTT4x30Avx512RayonBackend<R> {
    ...
    fn cnv_prepare_left(
        ...
        let (tmp, _) = crate::hal_impl::take_host_typed::<Self, u64>(scratch.borrow(), bytes / size_of::<u64>());
        super::convolution::cnv_prepare_left::<RayonTaskExecutor, _>(
            base_module::<R>(module),
    ...
    fn cnv_prepare_right(
        ...
        let (tmp, _) = crate::hal_impl::take_host_typed::<Self, u64>(scratch.borrow(), bytes / size_of::<u64>());
        super::convolution::cnv_prepare_right::<RayonTaskExecutor, _>(
            base_module::<R>(module),
    ...
    fn cnv_apply_dft_tmp_bytes(
        ...
        NTT4x30Avx512Backend::<R>::cnv_apply_dft_tmp_bytes(base_module::<R>(_module), _cnv_offset, res_size, a_size, b_size)
    }
    ...
    fn cnv_by_const_apply_tmp_bytes(
        ...
        NTT4x30Avx512Backend::<R>::cnv_by_const_apply_tmp_bytes(base_module::<R>(module), cnv_offset, res_size, a_size, b_size)
    }
    ...
    fn cnv_apply_dft(
        ...
        unsafe {
            super::convolution::cnv_apply_dft::<RayonTaskExecutor, _>(
                base_module::<R>(module),
                cnv_offset,
                &mut base_dft_mut::<R>(res),
                res_col,
    ...
    fn cnv_apply_dft_add(
        ...
        unsafe {
            super::convolution::cnv_apply_dft_add::<RayonTaskExecutor, _>(
                base_module::<R>(module),
                cnv_offset,
                &mut base_dft_mut::<R>(res),
                res_col,
    ...
    fn cnv_pairwise_apply_dft_tmp_bytes(
        ...
        NTT4x30Avx512Backend::<R>::cnv_pairwise_apply_dft_tmp_bytes(
            base_module::<R>(_module),
            _cnv_offset,
            res_size,
            a_size,
            b_size,
    ...
    fn cnv_pairwise_apply_dft(
        ...
        unsafe {
            super::convolution::cnv_pairwise_apply_dft::<RayonTaskExecutor, _>(
                base_module::<R>(module),
                cnv_offset,
                &mut base_dft_mut::<R>(res),
                res_col,
    ...
    fn cnv_prepare_self(
        ...
        let (tmp, _) = crate::hal_impl::take_host_typed::<Self, u64>(scratch.borrow(), bytes / size_of::<u64>());
        super::convolution::cnv_prepare_self::<RayonTaskExecutor, _>(
            base_module::<R>(module),
...
unsafe impl<R: CpuRing> HalVecZnxBigImpl for NTT4x30Avx512RayonBackend<R> {
    ...
    fn vec_znx_big_normalize(
        ...
        poulpy_cpu_rayon::normalize::ntt4x30_vec_znx_big_normalize_par::<NTT4x30Avx512Backend<R>, Self>(
            res,
            res_base2k,
            res_k,
            res_offset,
...
unsafe impl<R: CpuRing> HalSvpImpl for NTT4x30Avx512RayonBackend<R> {
    fn svp_prepare(
        module: &Module<Self>,
        ...
        NTT4x30Avx512Backend::<R>::svp_prepare(
            base_module::<R>(module),
    ...
    fn svp_ppol_copy(
        ...
        NTT4x30Avx512Backend::<R>::svp_ppol_copy(
            base_module::<R>(module),
            &mut base_svp_mut::<R>(res),
            res_col,
            &base_svp_ref::<R>(a),
    ...
    fn svp_apply_dft(
        ...
        let mut base_scratch = scratch.borrow().into_backend::<NTT4x30Avx512Backend<R>>();
        NTT4x30Avx512Backend::<R>::svp_apply_dft(
            base_module::<R>(module),
            &mut base_dft_mut::<R>(res),
            res_col,
            &base_svp_ref::<R>(a),
    ...
    fn svp_apply_dft_to_dft(
        ...
        NTT4x30Avx512Backend::<R>::svp_apply_dft_to_dft(
            base_module::<R>(module),
            &mut base_dft_mut::<R>(res),
            res_col,
            &base_svp_ref::<R>(a),
    ...
    fn svp_apply_dft_to_dft_assign(
        ...
        NTT4x30Avx512Backend::<R>::svp_apply_dft_to_dft_assign(
            base_module::<R>(module),
            &mut base_dft_mut::<R>(res),
            res_col,
            &base_svp_ref::<R>(a),
...
unsafe impl<R: CpuRing> HalVecZnxDftImpl for NTT4x30Avx512RayonBackend<R> {
    fn vec_znx_idft_normalize_consume_tmp_bytes(module: &Module<Self>, res_size: usize, a_size: usize) -> usize {
        NTT4x30Avx512Backend::<R>::vec_znx_idft_normalize_consume_tmp_bytes(base_module::<R>(module), res_size, a_size)
    }
    ...
    fn vec_znx_idft_normalize_consume(
        ...
        if let Some((add, add_col)) = addend {
            ...
            NTT4x30Avx512Backend::<R>::vec_znx_idft_normalize_consume(
                base_module::<R>(module),
                &mut base_res,
                res_base2k,
                res_k,
    ...
    fn vec_znx_dft_apply(
        ...
        if !parallel_limb_tasks(res.size()) {
            return NTT4x30Avx512Backend::<R>::vec_znx_dft_apply(
                base_module::<R>(module),
                step,
                offset,
                &mut base_dft_mut::<R>(res),
        ...
        poulpy_hal::layouts::check_degree::<NTT4x30Avx512Backend<R>>(module.n(), res.n());
        assert!(a.n() == res.n(), "vec_znx_dft_apply: a.n() != res.n()");
        let n = res.n();
        let cols = res.cols();
        let a_size = a.size();
        let module = base_module::<R>(module);
    ...
    fn vec_znx_idft_apply_tmp_bytes(module: &Module<Self>) -> usize {
        NTT4x30Avx512Backend::<R>::vec_znx_idft_apply_tmp_bytes(base_module::<R>(module)).max(
    ...
    fn vec_znx_idft_apply(
        ...
        if !parallel_limb_tasks(res.size()) {
            let mut base_scratch = scratch.borrow().into_backend::<NTT4x30Avx512Backend<R>>();
            return NTT4x30Avx512Backend::<R>::vec_znx_idft_apply(
                base_module::<R>(module),
                &mut base_big_mut::<R>(res),
                res_col,
                &base_dft_ref::<R>(a),
    ...
    fn vec_znx_idft_apply_tmpa(
        ...
        if !parallel_limb_tasks(res.size()) {
            return NTT4x30Avx512Backend::<R>::vec_znx_idft_apply_tmpa(
                base_module::<R>(module),
                &mut base_big_mut::<R>(res),
                res_col,
    ...
    fn vec_znx_dft_add(
        ...
        NTT4x30Avx512Backend::<R>::vec_znx_dft_add(
            base_module::<R>(module),
            &mut base_dft_mut::<R>(res),
            res_col,
            &base_dft_ref::<R>(a),
    ...
    fn vec_znx_dft_add_assign(
        ...
        NTT4x30Avx512Backend::<R>::vec_znx_dft_add_assign(
            base_module::<R>(module),
            &mut base_dft_mut::<R>(res),
            res_col,
            &base_dft_ref::<R>(a),
    ...
    fn vec_znx_dft_sub(
        ...
        NTT4x30Avx512Backend::<R>::vec_znx_dft_sub(
            base_module::<R>(module),
            &mut base_dft_mut::<R>(res),
            res_col,
            &base_dft_ref::<R>(a),
    ...
    fn vec_znx_dft_sub_assign(
        ...
        NTT4x30Avx512Backend::<R>::vec_znx_dft_sub_assign(
            base_module::<R>(module),
            &mut base_dft_mut::<R>(res),
            res_col,
            &base_dft_ref::<R>(a),
    ...
    fn vec_znx_dft_sub_negate_assign(
        ...
        NTT4x30Avx512Backend::<R>::vec_znx_dft_sub_negate_assign(
            base_module::<R>(module),
            &mut base_dft_mut::<R>(res),
            res_col,
            &base_dft_ref::<R>(a),
    ...
    fn vec_znx_dft_copy(
        ...
        NTT4x30Avx512Backend::<R>::vec_znx_dft_copy(
            base_module::<R>(module),
            step,
            offset,
            &mut base_dft_mut::<R>(res),
    ...
    fn vec_znx_dft_zero(module: &Module<Self>, res: &mut VecZnxDftBackendMut<'_, Self>, res_col: usize) {
        NTT4x30Avx512Backend::<R>::vec_znx_dft_zero(base_module::<R>(module), &mut base_dft_mut::<R>(res), res_col)
    }

    type AutomorphismPlan = <NTT4x30Avx512Backend<R> as HalVecZnxDftImpl>::AutomorphismPlan;

    fn vec_znx_dft_automorphism_plan(module: &Module<Self>, n: usize, p: i64) -> Self::AutomorphismPlan {
        NTT4x30Avx512Backend::<R>::vec_znx_dft_automorphism_plan(base_module::<R>(module), n, p)
    }
    ...
    fn vec_znx_dft_automorphism_add_with_plan(
        ...
        let _ = scratch;
        if RayonTaskExecutor::should_serialize_inner() {
            super::vec_znx_dft::vec_znx_dft_automorphism_add::<SerialTaskExecutor, _>(
                plan,
                &mut base_dft_mut::<R>(res),
                res_col,
                &base_dft_ref::<R>(a),
            ...
            super::vec_znx_dft::vec_znx_dft_automorphism_add::<RayonTaskExecutor, _>(
                plan,
                &mut base_dft_mut::<R>(res),
                res_col,
                &base_dft_ref::<R>(a),
...
impl<R: CpuRing> poulpy_hal::execution::ScratchWorkers for NTT4x30Avx512RayonBackend<R> {
...
impl<R: CpuRing> poulpy_cpu_rayon::RayonTuning for NTT4x30Avx512RayonBackend<R> {
