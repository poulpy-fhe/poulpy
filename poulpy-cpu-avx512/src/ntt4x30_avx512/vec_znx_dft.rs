...
pub(crate) fn dft_limb(module: &Module<NTT4x30Avx512>, dst: &mut [u32], src: Option<&[i64]>, tmp: &mut [u64]) {
    if let Some(src) = src {
...
pub(crate) fn idft_limb(module: &Module<NTT4x30Avx512>, dst: &mut [i128], src: &[u32], tmp: &mut [u64]) {
    let n = module.n();
