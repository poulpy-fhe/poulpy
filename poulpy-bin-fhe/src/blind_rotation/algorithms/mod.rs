    ...
    BE: Backend<Location = Host, ZnxWord = i64>,
{
    ...
    BE::copy_view_to_host(&BE::region_ref(DataView::data(lwe.body()), 0, body_bytes), &mut body_host);
    BE::copy_view_to_host(&BE::region_ref(DataView::data(lwe.mask()), 0, mask_bytes), &mut mask_host);
