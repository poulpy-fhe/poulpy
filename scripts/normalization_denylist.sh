...
check "from_data_like"         8 "state-forwarding reborrow; oep-only since PR 2 (backend kernel plumbing)"
check "map_data_mut"           4 "state-forwarding reborrow; oep-only since PR 2 (backend kernel plumbing)"
...
check "kernel_words_mut"      86 "sealed kernel capability uses (backend kernels + harness wrapper); audited per spec §9.1"
